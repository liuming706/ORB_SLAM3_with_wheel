function(z_vcpkg_meson_set_proglist_variables config_type)
  ...
  foreach(prog IN LISTS proglist)
    if(VCPKG_DETECTED_CMAKE_${prog})
      if(meson_${prog})
        ...
        set("${var_to_set}"
            "${meson_${prog}} = ['${VCPKG_DETECTED_CMAKE_${prog}}']"
            PARENT_SCOPE)
      elseif(${prog} STREQUAL AR
             AND VCPKG_DETECTED_CMAKE_STATIC_LINKER_FLAGS_${config_type})
        # Probably need to move AR somewhere else
        string(TOLOWER "${prog}" proglower)
        z_vcpkg_meson_convert_compiler_flags_to_list(
          ar_flags "${VCPKG_DETECTED_CMAKE_STATIC_LINKER_FLAGS_${config_type}}")
        ...
        string(TOLOWER "${prog}" proglower)
        set("${var_to_set}"
            "${proglower} = ['${VCPKG_DETECTED_CMAKE_${prog}}']"
            PARENT_SCOPE)
      endif()
    endif()
  endforeach()
  ...
  foreach(prog IN LISTS compilers)
    if(VCPKG_DETECTED_CMAKE_${prog}_COMPILER)
      ...
      if(meson_${prog})
        set("${var_to_set}"
            "${meson_${prog}} = ['${VCPKG_DETECTED_CMAKE_${prog}_COMPILER}']"
            PARENT_SCOPE)
        if(DEFINED VCPKG_DETECTED_CMAKE_${prog}_COMPILER_ID
           AND NOT VCPKG_DETECTED_CMAKE_${prog}_COMPILER_ID MATCHES
               "^(GNU|Intel)$")
          string(TOUPPER "MESON_${prog}_LD" var_to_set)
          set(${var_to_set}
              "${meson_${prog}}_ld = ['${VCPKG_DETECTED_CMAKE_LINKER}']"
              PARENT_SCOPE)
        endif()
      elseif(${prog} MATCHES RC
             AND VCPKG_DETECTED_CMAKE_RC_FLAGS_${config_type})
        z_vcpkg_meson_convert_compiler_flags_to_list(
          rc_flags "${VCPKG_DETECTED_CMAKE_RC_FLAGS_${config_type}}")
        list(PREPEND rc_flags "${VCPKG_DETECTED_CMAKE_${prog}_COMPILER}")
        z_vcpkg_meson_convert_list_to_python_array(rc_flags ${rc_flags})
        set("${var_to_set}"
            "${meson_${prog}} = ${rc_flags}"
            PARENT_SCOPE)
      else()
        string(TOLOWER "${prog}" proglower)
        set("${var_to_set}"
            "${proglower} = ['${VCPKG_DETECTED_CMAKE_${prog}_COMPILER}']"
            PARENT_SCOPE)
        if(DEFINED VCPKG_DETECTED_CMAKE_${prog}_COMPILER_ID
           AND NOT VCPKG_DETECTED_CMAKE_${prog}_COMPILER_ID MATCHES
               "^(GNU|Intel)$")
          string(TOUPPER "MESON_${prog}_LD" var_to_set)
          set(${var_to_set}
              "${proglower}_ld = ['${VCPKG_DETECTED_CMAKE_LINKER}']"
              PARENT_SCOPE)
        endif()
      endif()
    endif()
...
function(z_vcpkg_meson_convert_compiler_flags_to_list out_var compiler_flags)
  ...
  string(REGEX REPLACE [=[( +|^)((\"(\\"|[^"])+"|\\"|\\ |[^ ])+)]=] ";\\2"
                       tmp_var "${tmp_var}")
  vcpkg_list(POP_FRONT tmp_var) # The first element is always empty due to the
                                # above replacement
  list(TRANSFORM tmp_var STRIP) # Strip leading trailing whitespaces from each
                                # element in the list.
  set("${out_var}"
      "${tmp_var}"
      PARENT_SCOPE)
...
function(z_vcpkg_meson_set_flags_variables config_type)
  ...
  foreach(lang IN LISTS arg_LANGUAGES)
    z_vcpkg_meson_convert_compiler_flags_to_list(
      ${lang}flags "${VCPKG_DETECTED_CMAKE_${lang}_FLAGS_${config_type}}")
    ...
    set(linker_flags
        "${VCPKG_DETECTED_CMAKE_SHARED_LINKER_FLAGS_${config_type}}")
    ...
    string(APPEND MESON_${lang_mapping}FLAGS
           "${langlower}_link_args = ${linker_flags}")
    set(MESON_${lang_mapping}FLAGS
        "${MESON_${lang_mapping}FLAGS}"
        PARENT_SCOPE)
  endforeach()
endfunction()
...
function(z_vcpkg_get_build_and_host_system build_system host_system is_cross
  ...
  if(CMAKE_HOST_WIN32)
    ...
    elseif(build_arch MATCHES "^(ARM|arm)$")
      ...
      if(NOT DEFINED VCPKG_MESON_CROSS_FILE OR NOT DEFINED
                                               VCPKG_MESON_NATIVE_FILE)
        message(
          WARNING
            "Unsupported build architecture ${build_arch}! Please set VCPKG_MESON_(CROSS|NATIVE)_FILE to a meson file containing the build_machine entry!"
        )
      endif()
  ...
  elseif(CMAKE_HOST_UNIX)
    ...
    # Show real machine architecture to visually understand whether we are in a
    # native Apple Silicon terminal or running under Rosetta emulation
    ...
    elseif(MACHINE MATCHES "loongarch64")
      ...
      if(NOT DEFINED VCPKG_MESON_CROSS_FILE OR NOT DEFINED
        ...
        message(
          WARNING
            "Unhandled machine: ${MACHINE}! Please set VCPKG_MESON_(CROSS|NATIVE)_FILE to a meson file containing the build_machine entry!"
        )
      endif()
      set(build_unknown TRUE)
    endif()
    ...
    if(NOT DEFINED VCPKG_MESON_CROSS_FILE OR NOT DEFINED
      ...
      message(
        WARNING
          "Failed to detect the build architecture! Please set VCPKG_MESON_(CROSS|NATIVE)_FILE to a meson file containing the build_machine entry!"
      )
    endif()
    set(build_unknown TRUE)
  endif()
  ...
  elseif(VCPKG_TARGET_ARCHITECTURE MATCHES "wasm32")
    ...
    if(NOT DEFINED VCPKG_MESON_CROSS_FILE OR NOT DEFINED
      ...
      message(
        WARNING
          "Unsupported target architecture ${VCPKG_TARGET_ARCHITECTURE}! Please set VCPKG_MESON_(CROSS|NATIVE)_FILE to a meson file containing the host_machine entry!"
      )
    endif()
...
function(z_vcpkg_meson_setup_extra_windows_variables config_type)
  ...
  set(MESON_VSCRT_LINKAGE
      "b_vscrt = '${crt_type}'"
      PARENT_SCOPE)
  # winlibs
  separate_arguments(c_winlibs NATIVE_COMMAND
                     "${VCPKG_DETECTED_CMAKE_C_STANDARD_LIBRARIES}")
  separate_arguments(cpp_winlibs NATIVE_COMMAND
...
function(vcpkg_configure_meson)
  ...
  if(DEFINED arg_ADDITIONAL_NATIVE_BINARIES OR DEFINED
                                               arg_ADDITIONAL_CROSS_BINARIES)
    message(
      WARNING
        "Options ADDITIONAL_(NATIVE|CROSS)_BINARIES have been deprecated. Only use ADDITIONAL_BINARIES!"
    )
  endif()

  vcpkg_list(APPEND arg_ADDITIONAL_BINARIES ${arg_ADDITIONAL_NATIVE_BINARIES}
             ${arg_ADDITIONAL_CROSS_BINARIES})
  ...
  if(NOT DEFINED VCPKG_BUILD_TYPE OR VCPKG_BUILD_TYPE STREQUAL "debug")
    ...
    set(meson_input_file_${buildname}
        "${CURRENT_BUILDTREES_DIR}/meson-${TARGET_TRIPLET}-${suffix_${buildname}}.log"
    )
  endif()
  ...
  z_vcpkg_get_build_and_host_system(MESON_HOST_MACHINE MESON_BUILD_MACHINE
                                    IS_CROSS)

  if(IS_CROSS)
    # VCPKG_CROSSCOMPILING is not used since it regresses a lot of ports in
    # x64-windows-x triplets For consistency this should proably be changed in
    # the future?
    vcpkg_list(APPEND arg_OPTIONS --native
    ...
    vcpkg_list(APPEND arg_OPTIONS_RELEASE --native
               "${meson_input_file_RELEASE}")
  endif()
  ...
  if(VCPKG_MESON_NATIVE_FILE_DEBUG)
    vcpkg_list(APPEND arg_OPTIONS_DEBUG --native
  ...
  if(VCPKG_MESON_CROSS_FILE_RELEASE)
    vcpkg_list(APPEND arg_OPTIONS_RELEASE --cross
               "${VCPKG_MESON_CROSS_FILE_RELEASE}")
  endif()
  if(VCPKG_MESON_CROSS_FILE_DEBUG)
    vcpkg_list(APPEND arg_OPTIONS_DEBUG --cross
  ...
  vcpkg_list(APPEND arg_OPTIONS --libdir lib) # else meson install into an
                                              # architecture describing folder
  vcpkg_list(
    APPEND
    arg_OPTIONS_DEBUG
    -Ddebug=true
    ...
    --includedir
    ../include)
  vcpkg_list(APPEND arg_OPTIONS_RELEASE -Ddebug=false --prefix
  ...
  if(VCPKG_TARGET_IS_WINDOWS)
    vcpkg_list(
      APPEND
      arg_OPTIONS_DEBUG
      "-Dcmake_prefix_path=['${CURRENT_INSTALLED_DIR}/debug','${CURRENT_INSTALLED_DIR}','${CURRENT_INSTALLED_DIR}/share']"
    )
    vcpkg_list(
      APPEND
      arg_OPTIONS_RELEASE
      "-Dcmake_prefix_path=['${CURRENT_INSTALLED_DIR}','${CURRENT_INSTALLED_DIR}/debug','${CURRENT_INSTALLED_DIR}/share']"
    )
  else()
    vcpkg_list(
      APPEND
    ...
    vcpkg_list(
      APPEND
      arg_OPTIONS_RELEASE
      "-Dcmake_prefix_path=['${CURRENT_INSTALLED_DIR}','${CURRENT_INSTALLED_DIR}/debug']"
    )
  endif()
  ...
  foreach(buildtype IN LISTS buildtypes)
    ...
    configure_file("${SCRIPTS}/buildsystems/meson/meson.template.in"
                   "${meson_input_file_${buildtype}}" @ONLY)

    vcpkg_execute_required_process(
      COMMAND
      ${MESON}
      ${arg_OPTIONS}
      ${arg_OPTIONS_${buildtype}}
      ${arg_SOURCE_PATH}
      WORKING_DIRECTORY
      "${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-${suffix_${buildtype}}"
      LOGNAME
      config-${TARGET_TRIPLET}-${suffix_${buildtype}}
      SAVE_LOG_FILES
