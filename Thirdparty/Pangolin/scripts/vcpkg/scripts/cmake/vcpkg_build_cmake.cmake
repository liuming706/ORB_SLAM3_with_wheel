function(vcpkg_build_cmake)
  ...
  if(Z_VCPKG_CMAKE_BUILD_GUARD)
    message(
      FATAL_ERROR
        "The ${PORT} port already depends on vcpkg-cmake; using both vcpkg-cmake and vcpkg_build_cmake in the same port is unsupported."
    )
  endif()
  ...
  elseif("${Z_VCPKG_CMAKE_GENERATOR}" MATCHES "^Visual Studio")
    vcpkg_list(SET build_param "/p:VCPkgLocalAppDataDisabled=true"
               "/p:UseIntelMKL=No")
  ...
  elseif(Z_VCPKG_CMAKE_GENERATOR STREQUAL "Xcode")
    ...
    message(
      FATAL_ERROR
        "Unrecognized GENERATOR setting from vcpkg_configure_cmake(). Valid generators are: Ninja, Visual Studio, and NMake Makefiles"
    )
  endif()
    ...
    if(NOT DEFINED VCPKG_BUILD_TYPE OR "${VCPKG_BUILD_TYPE}" STREQUAL
      ...
      if(arg_DISABLE_PARALLEL)
        vcpkg_execute_build_process(
          ...
          ${no_parallel_param}
          WORKING_DIRECTORY
          "${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-${short_build_type}"
          LOGNAME
          "${arg_LOGFILE_ROOT}-${TARGET_TRIPLET}-${short_build_type}")
      else()
        vcpkg_execute_build_process(
          COMMAND
          "${CMAKE_COMMAND}"
