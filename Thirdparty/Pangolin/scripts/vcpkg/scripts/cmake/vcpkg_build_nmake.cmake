function(vcpkg_build_nmake)
  ...
  if(arg_NO_DEBUG)
    message(
      WARNING "NO_DEBUG argument to ${CMAKE_CURRENT_FUNCTION} is deprecated")
  endif()
  if(arg_ADD_BIN_TO_PATH)
    message(
      WARNING
        "ADD_BIN_TO_PATH argument to ${CMAKE_CURRENT_FUNCTION} is deprecated - it never did anything"
    )
  endif()
  ...
  if(arg_PREFER_JOM AND VCPKG_CONCURRENCY GREATER "1")
    ...
    if(arg_CL_LANGUAGE
       AND "${VCPKG_DETECTED_CMAKE_${arg_CL_LANGUAGE}_COMPILER_ID}" STREQUAL
           "MSVC")
      string(
        REGEX
        REPLACE " [/-]MP[0-9]* " " "
                VCPKG_COMBINED_${arg_CL_LANGUAGE}_FLAGS_DEBUG
                " ${VCPKG_COMBINED_${arg_CL_LANGUAGE}_FLAGS_DEBUG} /FS")
      string(
        REGEX
        REPLACE " [/-]MP[0-9]* " " "
                VCPKG_COMBINED_${arg_CL_LANGUAGE}_FLAGS_RELEASE
                " ${VCPKG_COMBINED_${arg_CL_LANGUAGE}_FLAGS_RELEASE} /FS")
    endif()
  else()
    ...
    if(NOT DEFINED VCPKG_BUILD_TYPE OR VCPKG_BUILD_TYPE STREQUAL build_type)
      if(build_type STREQUAL "debug")
        ...
        if(NOT arg_CL_LANGUAGE STREQUAL "NONE")
          set(ENV{_CL_}
              "${VCPKG_DETECTED_CMAKE_${arg_CL_LANGUAGE}_FLAGS_DEBUG}")
        endif()
      ...
      if(DEFINED arg_PRERUN_SHELL)
        ...
        vcpkg_execute_required_process(
          COMMAND ${arg_PRERUN_SHELL} WORKING_DIRECTORY
          "${object_dir}${project_subpath}" LOGNAME
          "prerun-${triplet_and_build_type}")
      endif()
      if(DEFINED "${prerun_variable_name}")
        message(STATUS "Prerunning ${triplet_and_build_type}")
        vcpkg_execute_required_process(
          COMMAND ${${prerun_variable_name}} WORKING_DIRECTORY
          "${object_dir}${project_subpath}" LOGNAME
          "prerun-specific-${triplet_and_build_type}")
      endif()
      ...
      if(arg_PREFER_JOM)
        execute_process(
          ...
          OUTPUT_FILE
            "${CURRENT_BUILDTREES_DIR}/${arg_LOGFILE_ROOT}-${triplet_and_build_type}-jom-out.log"
          ERROR_FILE
            "${CURRENT_BUILDTREES_DIR}/${arg_LOGFILE_ROOT}-${triplet_and_build_type}-jom-err.log"
      ...
      if(run_nmake)
        vcpkg_execute_build_process(
          ...
          LOGNAME
          "${arg_LOGFILE_ROOT}-${triplet_and_build_type}${tool_suffix}")
      endif()
