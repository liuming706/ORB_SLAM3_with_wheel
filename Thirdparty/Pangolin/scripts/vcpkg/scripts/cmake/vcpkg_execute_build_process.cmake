set(Z_VCPKG_EXECUTE_BUILD_PROCESS_RETRY_ERROR_MESSAGES
    ...
    " fatal error C1060: "
    # The linker ran out of memory during execution. We will try continuing once
    # more, with parallelism disabled.
    "LINK : fatal error LNK1318:"
    "LINK : fatal error LNK1104:"
    "LINK : fatal error LNK1201:"
    "ld terminated with signal 9"
    "Killed signal terminated program"
    # Multiple threads using the same directory at the same time cause
    # conflicts, will try again.
    "Cannot create parent directory"
    "Cannot write file"
    # Multiple threads caused the wrong order of creating folders and creating
    # files in folders
    "Can't open"
    # `make install` may stumble over concurrency, in particular with `mkdir` on
    # osx.
    "mkdir [^:]*: File exists")
list(JOIN Z_VCPKG_EXECUTE_BUILD_PROCESS_RETRY_ERROR_MESSAGES "|"
     Z_VCPKG_EXECUTE_BUILD_PROCESS_RETRY_ERROR_MESSAGES)

function(vcpkg_execute_build_process)
  cmake_parse_arguments(PARSE_ARGV 0 arg "" "WORKING_DIRECTORY;LOGNAME"
                        "COMMAND;NO_PARALLEL_COMMAND")
  ...
  if(NOT error_code EQUAL "0")
    ...
    elseif(all_contents MATCHES "mt : general error c101008d: ")
      # Antivirus workaround - occasionally files are locked and cause mt.exe to
      # fail
      message(
        STATUS
      ...
      foreach(iteration RANGE 1 3)
        message(
          STATUS
            "Restarting Build ${TARGET_TRIPLET}-${SHORT_BUILDTYPE} because of mt.exe file locking issue. Iteration: ${iteration}"
        )
