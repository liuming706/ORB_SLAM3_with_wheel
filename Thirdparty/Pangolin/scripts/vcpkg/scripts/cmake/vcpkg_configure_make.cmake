macro(z_vcpkg_determine_host_mingw out_var)
  ...
  elseif(host_arch MATCHES "(x|X)86")
    ...
    message(
      FATAL_ERROR
        "Unsupported mingw architecture ${host_arch} in z_vcpkg_determine_autotools_host_cpu!"
    )
  endif()
  unset(host_arch)
endmacro()
...
macro(z_vcpkg_determine_autotools_host_cpu out_var)
  # TODO: the host system processor architecture can differ from the host
  # triplet target architecture
  if(DEFINED ENV{PROCESSOR_ARCHITEW6432})
    set(host_arch $ENV{PROCESSOR_ARCHITEW6432})
  ...
  elseif(host_arch MATCHES "^(ARM|arm)$")
    ...
    message(
      FATAL_ERROR
        "Unsupported host architecture ${host_arch} in z_vcpkg_determine_autotools_host_cpu!"
    )
  endif()
  unset(host_arch)
endmacro()
...
macro(z_vcpkg_determine_autotools_target_cpu out_var)
  ...
  elseif(VCPKG_TARGET_ARCHITECTURE MATCHES "^(ARM|arm)$")
    ...
    message(
      FATAL_ERROR
        "Unsupported VCPKG_TARGET_ARCHITECTURE architecture ${VCPKG_TARGET_ARCHITECTURE} in z_vcpkg_determine_autotools_target_cpu!"
    )
  endif()
endmacro()
...
macro(z_vcpkg_determine_autotools_target_arch_mac out_var)
  ...
  elseif(osx_archs_num GREATER_EQUAL 2)
    ...
    # Better match the arch behavior of config.guess See:
...
macro(z_vcpkg_extract_cpp_flags_and_set_cflags_and_cxxflags flag_suffix)
  string(
    REGEX
      MATCHALL
      "( |^)(-D|-isysroot|--sysroot=|-isystem|-m?[Aa][Rr][Cc][Hh]|--target=|-target) ?[^ ]+"
      CPPFLAGS_${flag_suffix}
      "${VCPKG_DETECTED_CMAKE_C_FLAGS_${flag_suffix}}")
  string(
    REGEX
      MATCHALL
      "( |^)(-D|-isysroot|--sysroot=|-isystem|-m?[Aa][Rr][Cc][Hh]|--target=|-target) ?[^ ]+"
      CXXPPFLAGS_${flag_suffix}
      "${VCPKG_DETECTED_CMAKE_CXX_FLAGS_${flag_suffix}}")
  ...
  if(CPPREGEX)
    string(REGEX REPLACE "(${CPPREGEX})" "" CFLAGS_${flag_suffix}
                         "${VCPKG_DETECTED_CMAKE_C_FLAGS_${flag_suffix}}")
    string(REGEX REPLACE "(${CPPREGEX})" "" CXXFLAGS_${flag_suffix}
                         "${VCPKG_DETECTED_CMAKE_CXX_FLAGS_${flag_suffix}}")
  else()
    set(CFLAGS_${flag_suffix} "${VCPKG_DETECTED_CMAKE_C_FLAGS_${flag_suffix}}")
    set(CXXFLAGS_${flag_suffix}
        "${VCPKG_DETECTED_CMAKE_CXX_FLAGS_${flag_suffix}}")
  endif()
  string(REGEX REPLACE " +" " " CPPFLAGS_${flag_suffix}
                       "${CPPFLAGS_${flag_suffix}}")
  string(REGEX REPLACE " +" " " CFLAGS_${flag_suffix}
                       "${CFLAGS_${flag_suffix}}")
  string(REGEX REPLACE " +" " " CXXFLAGS_${flag_suffix}
                       "${CXXFLAGS_${flag_suffix}}")
  # libtool has and -R option so we need to guard against -RTC by using
  # -Xcompiler while configuring there might be a lot of unknown compiler option
  # warnings due to that just ignore them.
  string(REGEX REPLACE "((-|/)RTC[^ ]+)" "-Xcompiler \\1" CFLAGS_${flag_suffix}
                       "${CFLAGS_${flag_suffix}}")
  string(REGEX REPLACE "((-|/)RTC[^ ]+)" "-Xcompiler \\1"
                       CXXFLAGS_${flag_suffix} "${CXXFLAGS_${flag_suffix}}")
...
macro(z_vcpkg_append_to_configure_environment inoutstring var defaultval)
  # Allows to overwrite settings in custom triplets via the environment on
  # windows
...
function(vcpkg_configure_make)
  ...
  if(arg_USE_WRAPPERS AND arg_NO_WRAPPERS)
    message(
      FATAL_ERROR
        "${CMAKE_CURRENT_FUNCTION} was passed conflicting options USE_WRAPPERS and NO_WRAPPERS. Please remove one of them!"
    )
  endif()
  ...
  if(DEFINED VCPKG_MAKE_BUILD_TRIPLET)
    set(arg_BUILD_TRIPLET ${VCPKG_MAKE_BUILD_TRIPLET}) # Triplet overwrite for
                                                       # crosscompiling
  endif()
  ...
  if(EXISTS "${src_dir}/configure"
     AND EXISTS "${src_dir}/configure.ac"
     AND arg_AUTOCONFIG) # remove configure; rerun autoconf
    set(requires_autoconfig ON)
    file(REMOVE "${SRC_DIR}/configure") # remove possible autodated configure
                                        # scripts
  elseif(EXISTS "${src_dir}/configure" AND NOT arg_SKIP_CONFIGURE) # run
                                                                   # normally;
                                                                   # no autoconf
                                                                   # or autogen
                                                                   # required
  ...
  )# only applies to windows (clang-)cl and lib
    if(arg_AUTOCONFIG)
      set(arg_USE_WRAPPERS ON)
    else()
      # Keep the setting from portfiles. Without autotools we assume a custom
      # configure script which correctly handles cl and lib. Otherwise the port
      # needs to set CC|CXX|AR and probably CPP.
    endif()
  else()
  ...
  # Backup environment variables CCAS CC C CPP CXX FC FF GC LD LF LIBTOOL OBJC
  # OBJCXX R UPC Y
  set(cm_FLAGS
      AR
      AS
  ...
  # FC fotran compiler | FF Fortran 77 compiler LDFLAGS -> pass -L flags LIBS ->
  # pass -l flags

  # Used by gcc/linux
  vcpkg_backup_env_variables(VARS C_INCLUDE_PATH CPLUS_INCLUDE_PATH
  ...
  if(CURRENT_PACKAGES_DIR MATCHES " " OR CURRENT_INSTALLED_DIR MATCHES " ")
    # Don't bother with whitespace. The tools will probably fail and I tried
    # very hard trying to make it work (no success so far)!
    message(
      WARNING
        "Detected whitespace in root directory. Please move the path to one without whitespaces! The required tools do not handle whitespaces correctly and the build will most likely fail"
    )
  ...
  if(VCPKG_TARGET_IS_WINDOWS)
    if(CMAKE_HOST_WIN32)
      ...
      vcpkg_acquire_msys(MSYS_ROOT PACKAGES ${msys_require_packages}
                         ${arg_ADDITIONAL_MSYS_PACKAGES})
    endif()
    if(arg_DETERMINE_BUILD_TRIPLET OR NOT arg_BUILD_TRIPLET)
      z_vcpkg_determine_autotools_host_cpu(
        BUILD_ARCH) # VCPKG_HOST => machine you are building on => --build=
      z_vcpkg_determine_autotools_target_cpu(TARGET_ARCH)
      # --build: the machine you are building on --host: the machine you are
      # building for --target: the machine that CC will produce binaries for
      # https://stackoverflow.com/questions/21990021/how-to-determine-host-value-for-configure-when-using-cross-compiler
      # Only for ports using autotools so we can assume that they follow the
      # common conventions for build/target/host
      if(CMAKE_HOST_WIN32)
        set(arg_BUILD_TRIPLET "--build=${BUILD_ARCH}-pc-mingw32") # This is
                                                                  # required
                                                                  # since we are
                                                                  # running in a
                                                                  # msys
      ...
      )# we don't need to specify the additional flags if we build nativly, this
       # does not hold when we are not on windows
        string(APPEND arg_BUILD_TRIPLET " --host=${TARGET_ARCH}-pc-mingw32"
        )# (Host activates crosscompilation; The name given here is just the
         # prefix of the host tools for the target)
      endif()
      if(VCPKG_TARGET_IS_UWP AND NOT arg_BUILD_TRIPLET MATCHES "--host")
        # Needs to be different from --build to enable cross builds.
        string(APPEND arg_BUILD_TRIPLET
               " --host=${TARGET_ARCH}-unknown-mingw32")
      endif()
    ...
    if(CMAKE_HOST_WIN32)
      ...
      if(arg_USE_WRAPPERS)
        vcpkg_list(APPEND add_to_env "${SCRIPTS}/buildsystems/make_wrapper"
        )# Other required wrappers are also located there
      ...
      message(DEBUG "path_list:${path_list}") # Just to have --trace-expand
                                              # output

      set(find_system_dirs
          "${system_root}/system32" "${system_root}/System32"
          "${system_root}/system32/" "${system_root}/System32/")
      ...
      if(index GREATER_EQUAL "0")
        vcpkg_list(INSERT path_list "${index}" ${add_to_env}
                   "${MSYS_ROOT}/usr/bin")
      else()
        message(
          WARNING
            "Unable to find system32 dir in the PATH variable! Appending required msys paths!"
        )
    ...
    # Remove full filepaths due to spaces and prepend filepaths to PATH
    # (cross-compiling tools are unlikely on path by default)
    set(progs
        ...
        VCPKG_DETECTED_CMAKE_OBJDUMP
        VCPKG_DETECTED_CMAKE_STRIP
        VCPKG_DETECTED_CMAKE_NM
        VCPKG_DETECTED_CMAKE_DLLTOOL
    ...
    if(arg_USE_WRAPPERS)
      z_vcpkg_append_to_configure_environment(
        configure_env CPP "compile ${VCPKG_DETECTED_CMAKE_C_COMPILER} -E")

      z_vcpkg_append_to_configure_environment(
        configure_env CC "compile ${VCPKG_DETECTED_CMAKE_C_COMPILER}")
      if(NOT VCPKG_CROSSCOMPILING)
        z_vcpkg_append_to_configure_environment(
          configure_env CC_FOR_BUILD
          "compile ${VCPKG_DETECTED_CMAKE_C_COMPILER}")
        z_vcpkg_append_to_configure_environment(
          configure_env CPP_FOR_BUILD
          "compile ${VCPKG_DETECTED_CMAKE_C_COMPILER} -E")
        z_vcpkg_append_to_configure_environment(
          configure_env CXX_FOR_BUILD
          "compile ${VCPKG_DETECTED_CMAKE_CXX_COMPILER}")
      else()
        # Silly trick to make configure accept CC_FOR_BUILD but in reallity
        # CC_FOR_BUILD is deactivated.
        z_vcpkg_append_to_configure_environment(
          configure_env CC_FOR_BUILD
          "touch a.out | touch conftest${VCPKG_HOST_EXECUTABLE_SUFFIX} | true")
        z_vcpkg_append_to_configure_environment(
          configure_env CPP_FOR_BUILD
          "touch a.out | touch conftest${VCPKG_HOST_EXECUTABLE_SUFFIX} | true")
        z_vcpkg_append_to_configure_environment(
      ...
      z_vcpkg_append_to_configure_environment(
        configure_env CXX "compile ${VCPKG_DETECTED_CMAKE_CXX_COMPILER}")
      z_vcpkg_append_to_configure_environment(
        configure_env RC "windres-rc ${VCPKG_DETECTED_CMAKE_RC_COMPILER}")
      z_vcpkg_append_to_configure_environment(
        configure_env WINDRES "windres-rc ${VCPKG_DETECTED_CMAKE_RC_COMPILER}")
      if(VCPKG_DETECTED_CMAKE_AR)
        z_vcpkg_append_to_configure_environment(
          configure_env AR "ar-lib ${VCPKG_DETECTED_CMAKE_AR}")
      else()
        z_vcpkg_append_to_configure_environment(configure_env AR
                                                "ar-lib lib.exe -verbose")
      endif()
    else()
      z_vcpkg_append_to_configure_environment(
        configure_env CPP "${VCPKG_DETECTED_CMAKE_C_COMPILER} -E")
      z_vcpkg_append_to_configure_environment(
        configure_env CC "${VCPKG_DETECTED_CMAKE_C_COMPILER}")
      if(NOT VCPKG_CROSSCOMPILING)
        z_vcpkg_append_to_configure_environment(
          configure_env CC_FOR_BUILD "${VCPKG_DETECTED_CMAKE_C_COMPILER}")
        z_vcpkg_append_to_configure_environment(
          configure_env CPP_FOR_BUILD "${VCPKG_DETECTED_CMAKE_C_COMPILER} -E")
        z_vcpkg_append_to_configure_environment(
          configure_env CXX_FOR_BUILD "${VCPKG_DETECTED_CMAKE_CXX_COMPILER}")
      else()
        z_vcpkg_append_to_configure_environment(
          configure_env CC_FOR_BUILD
          "touch a.out | touch conftest${VCPKG_HOST_EXECUTABLE_SUFFIX} | true")
      ...
      z_vcpkg_append_to_configure_environment(
        configure_env CXX "${VCPKG_DETECTED_CMAKE_CXX_COMPILER}")
      z_vcpkg_append_to_configure_environment(
        configure_env RC "${VCPKG_DETECTED_CMAKE_RC_COMPILER}")
      z_vcpkg_append_to_configure_environment(
        configure_env WINDRES "${VCPKG_DETECTED_CMAKE_RC_COMPILER}")
      if(VCPKG_DETECTED_CMAKE_AR)
        z_vcpkg_append_to_configure_environment(configure_env AR
                                                "${VCPKG_DETECTED_CMAKE_AR}")
      else()
    ...
    z_vcpkg_append_to_configure_environment(
      configure_env LD "${VCPKG_DETECTED_CMAKE_LINKER} -verbose")
    if(VCPKG_DETECTED_CMAKE_RANLIB)
      z_vcpkg_append_to_configure_environment(
        configure_env RANLIB "${VCPKG_DETECTED_CMAKE_RANLIB}") # Trick to ignore
                                                               # the RANLIB call
    else()
      z_vcpkg_append_to_configure_environment(configure_env RANLIB ":")
    endif()
    if(VCPKG_DETECTED_CMAKE_OBJDUMP) # Objdump is required to make shared
                                     # libraries. Otherwise define
                                     # lt_cv_deplibs_check_method=pass_all
      z_vcpkg_append_to_configure_environment(
        configure_env OBJDUMP "${VCPKG_DETECTED_CMAKE_OBJDUMP}") # Trick to
                                                                 # ignore the
                                                                 # RANLIB call
    endif()
    if(VCPKG_DETECTED_CMAKE_STRIP) # If required set the ENV variable STRIP in
                                   # the portfile correctly
    ...
    if(VCPKG_DETECTED_CMAKE_NM) # If required set the ENV variable NM in the
                                # portfile correctly
      z_vcpkg_append_to_configure_environment(configure_env NM
                                              "${VCPKG_DETECTED_CMAKE_NM}")
    else()
      # Would be better to have a true nm here! Some symbols (mainly exported
      # variables) get not properly imported with dumpbin as nm and require
      # __declspec(dllimport) for some reason (same problem CMake has with
      # WINDOWS_EXPORT_ALL_SYMBOLS)
      z_vcpkg_append_to_configure_environment(configure_env NM
                                              "dumpbin.exe -symbols -headers")
    endif()
    if(VCPKG_DETECTED_CMAKE_DLLTOOL) # If required set the ENV variable DLLTOOL
                                     # in the portfile correctly
      z_vcpkg_append_to_configure_environment(configure_env DLLTOOL
                                              "${VCPKG_DETECTED_CMAKE_DLLTOOL}")
    else()
      z_vcpkg_append_to_configure_environment(configure_env DLLTOOL
                                              "link.exe -verbose -dll")
    endif()
    z_vcpkg_append_to_configure_environment(
      configure_env CCAS ":") # If required set the ENV variable CCAS in the
                              # portfile correctly
    z_vcpkg_append_to_configure_environment(
      configure_env AS ":") # If required set the ENV variable AS in the
                            # portfile correctly

    foreach(_env IN LISTS arg_CONFIGURE_ENVIRONMENT_VARIABLES)
      z_vcpkg_append_to_configure_environment(configure_env ${_env}
                                              "${${_env}}")
    endforeach()
    debug_message("configure_env: '${configure_env}'")
    # Other maybe interesting variables to control COMPILE This is the command
    # used to actually compile a C source file. The file name is appended to
    # form the complete command line. LINK This is the command used to actually
    # link a C program. CXXCOMPILE The command used to actually compile a C++
    # source file. The file name is appended to form the complete command line.
    # CXXLINK  The command used to actually link a C++ program.

    # Variables not correctly detected by configure. In release builds.
    list(APPEND arg_OPTIONS gl_cv_double_slash_root=yes ac_cv_func_memmove=yes)
    # list(APPEND arg_OPTIONS lt_cv_deplibs_check_method=pass_all) # Just ignore
    # libtool checks
    if(VCPKG_TARGET_ARCHITECTURE MATCHES "^[Aa][Rr][Mm]64$")
      list(APPEND arg_OPTIONS gl_cv_host_cpu_c_abi=no)
      # Currently needed for arm64 because objdump yields: "unrecognised machine
      # type (0xaa64) in Import Library Format archive"
      list(APPEND arg_OPTIONS lt_cv_deplibs_check_method=pass_all)
    elseif(VCPKG_TARGET_ARCHITECTURE MATCHES "^[Aa][Rr][Mm]$")
      # Currently needed for arm because objdump yields: "unrecognised machine
      # type (0x1c4) in Import Library Format archive"
      list(APPEND arg_OPTIONS lt_cv_deplibs_check_method=pass_all)
    endif()
  else()
    # Because OSX dosn't like CMAKE_C(XX)_COMPILER (cc) in CC/CXX and rather
    # wants to have gcc/g++
    ...
    if(NOT VCPKG_CROSSCOMPILING)
      ...
      set(ENV{CC_FOR_BUILD}
          "touch a.out | touch conftest${VCPKG_HOST_EXECUTABLE_SUFFIX} | true")
      set(ENV{CPP_FOR_BUILD}
          "touch a.out | touch conftest${VCPKG_HOST_EXECUTABLE_SUFFIX} | true")
      set(ENV{CXX_FOR_BUILD}
          "touch a.out | touch conftest${VCPKG_HOST_EXECUTABLE_SUFFIX} | true")
    endif()
  ...
  # Some PATH handling for dealing with spaces....some tools will still fail
  # with that! In particular, the libtool install command is unable to install
  # correctly to paths with spaces. CURRENT_INSTALLED_DIR: Pristine native path
  # (unprotected spaces, Windows drive letters) z_vcpkg_installed_path: Native
  # path with escaped space characters z_vcpkg_prefix_path:         Path with
  # unprotected spaces, but drive letters transformed for mingw/msys
  string(REPLACE " " "\\ " z_vcpkg_installed_path "${CURRENT_INSTALLED_DIR}")
  if(CMAKE_HOST_WIN32)
    string(REGEX REPLACE "([a-zA-Z]):/" "/\\1/" z_vcpkg_prefix_path
  ...
  if(VCPKG_TARGET_IS_OSX OR VCPKG_TARGET_IS_IOS)
    if(requires_autoconfig
       AND NOT arg_BUILD_TRIPLET
       OR arg_DETERMINE_BUILD_TRIPLET)
      z_vcpkg_determine_autotools_host_arch_mac(
        BUILD_ARCH) # machine you are building on => --build=
      ...
      # Only for ports using autotools so we can assume that they follow the
      # common conventions for build/target/host
      if(NOT "${TARGET_ARCH}" STREQUAL "${BUILD_ARCH}" OR NOT
                                                          VCPKG_TARGET_IS_OSX
      )# we don't need to specify the additional flags if we build natively.
        set(arg_BUILD_TRIPLET "--host=${TARGET_ARCH}-apple-darwin"
        )# (Host activates crosscompilation; The name given here is just the
         # prefix of the host tools for the target)
      endif()
      debug_message("Using make triplet: ${arg_BUILD_TRIPLET}")
  ...
  if(VCPKG_TARGET_IS_LINUX)
    if(requires_autoconfig
       AND NOT arg_BUILD_TRIPLET
       OR arg_DETERMINE_BUILD_TRIPLET)
      # The regex below takes the prefix from the resulting CMAKE_C_COMPILER
      # variable eg. arm-linux-gnueabihf-gcc set in the common
      # toolchains/linux.cmake This is used via --host as a prefix for all other
      # bin tools as well. Setting the compiler directly via
      # CC=arm-linux-gnueabihf-gcc does not work acording to:
      # https://www.gnu.org/software/autoconf/manual/autoconf-2.65/html_node/Specifying-Target-Triplets.html
      if(VCPKG_DETECTED_CMAKE_C_COMPILER MATCHES "([^\/]*)-gcc$"
         AND CMAKE_MATCH_1)
        set(arg_BUILD_TRIPLET "--host=${CMAKE_MATCH_1}") # (Host activates
                                                         # crosscompilation; The
                                                         # name given here is
                                                         # just the prefix of
                                                         # the host tools for
                                                         # the target)
  ...
  file(RELATIVE_PATH relative_build_path "${CURRENT_BUILDTREES_DIR}"
       "${arg_SOURCE_PATH}/${arg_PROJECT_SUBPATH}")
  ...
  # Used by GCC
  vcpkg_host_path_list(PREPEND ENV{C_INCLUDE_PATH}
                       "${CURRENT_INSTALLED_DIR}/include")
  vcpkg_host_path_list(PREPEND ENV{CPLUS_INCLUDE_PATH}
                       "${CURRENT_INSTALLED_DIR}/include")

  # Flags should be set in the toolchain instead (Setting this up correctly
  # requires a function named vcpkg_determined_cmake_compiler_flags which can
  # also be used to setup CC and CXX etc.)
  if(VCPKG_TARGET_IS_WINDOWS)
    vcpkg_backup_env_variables(VARS _CL_ _LINK_)
    # TODO: Should be CPP flags instead -> rewrite when
    # vcpkg_determined_cmake_compiler_flags defined
    if(VCPKG_TARGET_IS_UWP)
      # Be aware that configure thinks it is crosscompiling due to: error while
      # loading shared libraries: VCRUNTIME140D_APP.dll: cannot open shared
      # object file: No such file or directory IMPORTANT: The only way to pass
      # linker flags through libtool AND the compile wrapper is to use the CL
      # and LINK environment variables !!! (This is due to libtool and compiler
      # wrapper using the same set of options to pass those variables around)
      file(TO_CMAKE_PATH "$ENV{VCToolsInstallDir}" VCToolsInstallDir)
      set(_replacement
          -FU\"${VCToolsInstallDir}/lib/x86/store/references/platform.winmd\")
      string(REPLACE "${_replacement}" "" VCPKG_DETECTED_CMAKE_CXX_FLAGS_DEBUG
                     "${VCPKG_DETECTED_CMAKE_CXX_FLAGS_DEBUG}")
      string(REPLACE "${_replacement}" "" VCPKG_DETECTED_CMAKE_C_FLAGS_DEBUG
                     "${VCPKG_DETECTED_CMAKE_C_FLAGS_DEBUG}")
      string(REPLACE "${_replacement}" ""
                     VCPKG_DETECTED_CMAKE_CXX_FLAGS_RELEASE
                     "${VCPKG_DETECTED_CMAKE_CXX_FLAGS_RELEASE}")
      string(REPLACE "${_replacement}" "" VCPKG_DETECTED_CMAKE_C_FLAGS_RELEASE
                     "${VCPKG_DETECTED_CMAKE_C_FLAGS_RELEASE}")
      # Can somebody please check if CMake's compiler flags for UWP are correct?
      set(ENV{_CL_}
          "$ENV{_CL_} -FU\"${VCToolsInstallDir}/lib/x86/store/references/platform.winmd\""
      )
      set(ENV{_LINK_}
          "$ENV{_LINK_} ${VCPKG_DETECTED_CMAKE_C_STANDARD_LIBRARIES} ${VCPKG_DETECTED_CMAKE_CXX_STANDARD_LIBRARIES}"
      )
    endif()
  endif()
  ...
  if(VCPKG_TARGET_IS_UWP)
    ...
    # Avoid libtool choke: "Warning: linker path does not have real file for
    # library -lWindowsApp." The problem with the choke is that libtool always
    # falls back to built a static library even if a dynamic was requested.
    # Note: Env LIBPATH;LIB are on the search path for libtool by default on
    # windows. It even does unix/dos-short/unix transformation with the path to
    # get rid of spaces.
  endif()
  set(l_prefix)
  ...
  if(arg_AUTOCONFIG
     ...
     AND NOT arg_NO_AUTOCONFIG)
    find_program(AUTORECONF autoreconf)
    if(NOT AUTORECONF)
      message(
        FATAL_ERROR
          "${PORT} requires autoconf from the system package manager (example: \"sudo apt-get install autoconf\")"
      )
    endif()
  ...
  if(requires_autogen)
    ...
    if(CMAKE_HOST_WIN32)
      ...
      vcpkg_execute_required_process(
        COMMAND "./autogen.sh" WORKING_DIRECTORY "${src_dir}" LOGNAME
        "autoconf-${TARGET_TRIPLET}")
    endif()
    message(STATUS "Finished generating configure for ${TARGET_TRIPLET}")
  endif()
  ...
  macro(z_vcpkg_setup_make_linker_flags_vars var_suffix)
    if(VCPKG_LIBRARY_LINKAGE STREQUAL "static")
      set(LINKER_FLAGS_${var_suffix}
          "${VCPKG_DETECTED_CMAKE_STATIC_LINKER_FLAGS_${var_suffix}}")
    else() # dynamic
      set(LINKER_FLAGS_${var_suffix}
          "${VCPKG_DETECTED_CMAKE_SHARED_LINKER_FLAGS_${var_suffix}}")
    endif()
    set(ARFLAGS_${var_suffix}
        "${VCPKG_DETECTED_CMAKE_STATIC_LINKER_FLAGS_${var_suffix}}")
    set(LDFLAGS_${var_suffix}
        "${VCPKG_DETECTED_CMAKE_SHARED_LINKER_FLAGS_${var_suffix}}")
    if(CMAKE_HOST_WIN32 AND VCPKG_DETECTED_CMAKE_C_COMPILER_ID MATCHES
                            [[cl\.exe$]])
      if(NOT vcm_paths_with_spaces)
        string(
          APPEND
          LDFLAGS_${var_suffix}
          " -L${z_vcpkg_installed_path}${path_suffix_${var_suffix}}/lib -L${z_vcpkg_installed_path}${path_suffix_${var_suffix}}/lib/manual-link"
        )
      endif()
      ...
      if(EXISTS "${CURRENT_INSTALLED_DIR}${path_suffix_${var_suffix}}/lib")
        set(link_required_dirs
            "-L${z_vcpkg_installed_path}${path_suffix_${var_suffix}}/lib")
      endif()
      if(EXISTS
         "{CURRENT_INSTALLED_DIR}${path_suffix_${var_suffix}}/lib/manual-link")
        set(link_required_dirs
            "${link_required_dirs} -L${z_vcpkg_installed_path}${path_suffix_${var_suffix}}/lib/manual-link"
        )
      endif()
      ...
      if(link_required_dirs)
        ...
        # ARFLAGS doesn't need -L search paths since it just bundles object
        # files
      endif()
      if(ARFLAGS_${var_suffix})
        # ARFLAGS need to know the command for creating an archive (Maybe needs
        # user customization?) or extract it from CMake via
        # CMAKE_${lang}_ARCHIVE_CREATE ? or from CMAKE_${lang}_${rule} with rule
        # being one of CREATE_SHARED_MODULE CREATE_SHARED_LIBRARY
        # LINK_EXECUTABLE
      ...
      if(VCPKG_TARGET_IS_WINDOWS AND VCPKG_DETECTED_CMAKE_LINKER MATCHES
                                     [[link\.exe$]])
        # Do not touch autotools quirks incoming! -Xlinker is repeated three
        # times because: - libtool script eats -Xlinker - the compile wrapper
        # eats -Xlinker - passing through both tools requires 3 -Xlinker; two
        # being eaten in the first script. passing only through one script will
        # keep one -Xlinker (done in configure) but cl will just ignore those
        # with a warning. (Just like -Xcompiler)
        separate_arguments(LDFLAGS_LIST NATIVE_COMMAND
                           "${LDFLAGS_${var_suffix}}")
        list(JOIN LDFLAGS_LIST " -Xlinker -Xlinker -Xlinker "
             LDFLAGS_${var_suffix})
  ...
  if(NOT DEFINED VCPKG_BUILD_TYPE
     OR VCPKG_BUILD_TYPE STREQUAL "debug"
     AND NOT arg_NO_DEBUG)
  ...
  foreach(current_buildtype IN LISTS all_buildtypes)
    ...
    set(target_dir
        "${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-${short_name_${current_buildtype}}"
    )
    ...
    # set(ENV{CXXFLAGS_FOR_BUILD} "${CXXFLAGS_${current_buildtype}}") -> doesn't
    # exist officially
    ...
    if(ARFLAGS_${current_buildtype} AND NOT (arg_USE_WRAPPERS
                                             AND VCPKG_TARGET_IS_WINDOWS))
      # Target windows with wrappers enabled cannot forward ARFLAGS since it
      # breaks the wrapper
      set(ENV{ARFLAGS} "${ARFLAGS_${current_buildtype}}")
    endif()
    # https://www.gnu.org/software/libtool/manual/html_node/Link-mode.html
    # -avoid-version is handled specially by libtool link mode, this flag is not
    # forwarded to linker, and libtool tries to avoid versioning for shared
    # libraries and no symbolic links are created.
    ...
    foreach(lib_env_var IN LISTS lib_env_vars)
      if(EXISTS
         "${CURRENT_INSTALLED_DIR}${path_suffix_${current_buildtype}}/lib")
        vcpkg_host_path_list(
          PREPEND ENV{${lib_env_var}}
          "${CURRENT_INSTALLED_DIR}${path_suffix_${current_buildtype}}/lib")
      endif()
      if(EXISTS
         "${CURRENT_INSTALLED_DIR}${path_suffix_${current_buildtype}}/lib/manual-link"
      )
        vcpkg_host_path_list(
          PREPEND
          ENV{${lib_env_var}}
    ...
    set(command
        "${base_cmd}"
        -c
        "${configure_env} ./${relative_build_path}/configure ${arg_BUILD_TRIPLET} ${arg_OPTIONS} ${arg_OPTIONS_${current_buildtype}}"
    )

    if(arg_ADD_BIN_TO_PATH)
      set(path_backup $ENV{PATH})
      vcpkg_add_to_path(
        "${CURRENT_INSTALLED_DIR}${path_suffix_${current_buildtype}}/bin")
    endif()
    debug_message("Configure command:'${command}'")
    if(NOT arg_SKIP_CONFIGURE)
      message(
        STATUS
          "Configuring ${TARGET_TRIPLET}-${short_name_${current_buildtype}}")
      vcpkg_execute_required_process(
        COMMAND
        ${command}
        WORKING_DIRECTORY
  ...
  # Restore environment
  vcpkg_restore_env_variables(VARS ${cm_FLAGS} LIB LIBPATH LIBRARY_PATH
                              LD_LIBRARY_PATH)

  set(_VCPKG_PROJECT_SOURCE_PATH
      ${arg_SOURCE_PATH}
      PARENT_SCOPE)
  set(_VCPKG_PROJECT_SUBPATH
      ${arg_PROJECT_SUBPATH}
      PARENT_SCOPE)
  set(_VCPKG_MAKE_NO_DEBUG
      ${arg_NO_DEBUG}
      PARENT_SCOPE)
endfunction()
