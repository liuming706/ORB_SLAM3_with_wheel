function(vcpkg_from_git)
  cmake_parse_arguments(
    PARSE_ARGV 0 "arg" "LFS" "OUT_SOURCE_PATH;URL;REF;FETCH_REF;HEAD_REF;TAG"
    "PATCHES")

  if(DEFINED arg_UNPARSED_ARGUMENTS)
    message(
      WARNING
        "vcpkg_from_git was passed extra arguments: ${arg_UNPARSED_ARGUMENTS}")
  endif()
  if(DEFINED arg_TAG)
    message(
      WARNING
        "The TAG argument to vcpkg_from_git has been deprecated and has no effect."
    )
  endif()
  ...
  if(VCPKG_USE_HEAD_VERSION AND DEFINED arg_HEAD_REF)
    vcpkg_list(SET working_directory_param "WORKING_DIRECTORY"
    ...
    if(NOT DEFINED arg_REF)
      message(
        FATAL_ERROR
          "Package does not specify REF. It must be built using --head.")
    endif()
  ...
  if(NOT EXISTS "${archive}")
    if(_VCPKG_NO_DOWNLOADS)
      message(
        FATAL_ERROR "Downloads are disabled, but '${archive}' does not exist.")
    endif()
    set(do_download ON)
  endif()
  ...
  if(do_download)
    ...
    vcpkg_execute_required_process(
      ...
      "${arg_URL}"
      "${ref_to_fetch}"
      ${git_fetch_shallow_param}
      -n
      WORKING_DIRECTORY
      "${git_working_directory}"
      LOGNAME
      "git-fetch-${TARGET_TRIPLET}")
    if(arg_LFS)
      ...
      vcpkg_execute_required_process(
        ...
        "${git_working_directory}"
        LOGNAME
        "git-lfs-install-${TARGET_TRIPLET}")
      vcpkg_execute_required_process(
        ALLOW_IN_DOWNLOAD_MODE
        COMMAND
        ${GIT}
    ...
    if(VCPKG_USE_HEAD_VERSION)
      ...
      if(error_code)
        message(
          FATAL_ERROR
            "unable to determine FETCH_HEAD after fetching git repository")
      endif()
      ...
      if(error_code)
        message(
          FATAL_ERROR
            "unable to rev-parse ${arg_REF} after fetching git repository")
      endif()
      string(STRIP "${rev_parse_ref}" rev_parse_ref)
      if(NOT "${rev_parse_ref}" STREQUAL "${arg_REF}")
        message(
          FATAL_ERROR
            "REF (${arg_REF}) does not match rev-parse'd reference (${rev_parse_ref})
        [Expected : ( ${arg_REF} )])
