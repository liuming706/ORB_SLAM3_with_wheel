function(z_vcpkg_extract_source_archive_deprecated_mode archive
         working_directory)
...
function(vcpkg_extract_source_archive)
  if(ARGC LESS_EQUAL "2")
    z_vcpkg_deprecation_message(
      "Deprecated form of vcpkg_extract_source_archive used:
    Please use the `vcpkg_extract_source_archive(<out-var> ARCHIVE <archive>)` form."
    )
    if(ARGC EQUAL "0")
      message(
        FATAL_ERROR
          "vcpkg_extract_source_archive requires at least one argument.")
    endif()
    ...
    z_vcpkg_extract_source_archive_deprecated_mode("${archive}"
                                                   "${working_directory}")
    return()
  endif()
  ...
  cmake_parse_arguments(
    ...
    "ARCHIVE;SOURCE_BASE;BASE_DIRECTORY;WORKING_DIRECTORY;REF" "PATCHES")
  ...
  if(DEFINED arg_WORKING_DIRECTORY)
    if(DEFINED arg_BASE_DIRECTORY)
      message(
        FATAL_ERROR "Cannot specify both BASE_DIRECTORY and WORKING_DIRECTORY")
    elseif(NOT IS_ABSOLUTE "${arg_WORKING_DIRECTORY}")
      message(
        FATAL_ERROR
          "WORKING_DIRECTORY (${arg_WORKING_DIRECTORY}) must be an absolute path"
      )
    endif()
    ...
    elseif(IS_ABSOLUTE "${arg_BASE_DIRECTORY}")
      message(
        FATAL_ERROR
          "BASE_DIRECTORY (${arg_BASE_DIRECTORY}) must be a relative path")
    endif()
  ...
  elseif(arg_SOURCE_BASE MATCHES [[\\|/]])
    message(
      FATAL_ERROR "SOURCE_BASE (${arg_SOURCE_BASE}) must not contain slashes")
  endif()
  ...
  # Hash the archive hash along with the patches. Take the first 10 chars of the
  # hash
  file(SHA512 "${arg_ARCHIVE}" patchset_hash)
  foreach(patch IN LISTS arg_PATCHES)
    cmake_path(ABSOLUTE_PATH patch BASE_DIRECTORY "${CURRENT_PORT_DIR}"
  ...
  elseif(NOT _VCPKG_EDITABLE)
    ...
    if(EXISTS "${source_path}")
      message(
        STATUS
          "Cleaning sources at ${source_path}. Use --editable to skip cleaning for the packages you specify."
      )
  ...
  if(arg_NO_REMOVE_ONE_LEVEL)
    ...
    if(NOT archive_directory MATCHES ";" AND IS_DIRECTORY
                                             "${archive_directory}")
      cmake_path(SET temp_source_path "${archive_directory}")
    else()
      message(
        FATAL_ERROR
          "Could not unwrap top level directory from archive. Pass NO_REMOVE_ONE_LEVEL to disable this."
      )
    endif()
  endif()
  ...
  z_vcpkg_apply_patches(SOURCE_PATH "${temp_source_path}" PATCHES
                        ${arg_PATCHES} ${quiet_param})
