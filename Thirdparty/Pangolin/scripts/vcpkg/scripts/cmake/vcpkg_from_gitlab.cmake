...
function(vcpkg_from_gitlab)
  ...
  if(DEFINED arg_UNPARSED_ARGUMENTS)
    message(
      WARNING
        "vcpkg_from_gitlab was passed extra arguments: ${arg_UNPARSED_ARGUMENTS}"
    )
  endif()
  ...
  if(DEFINED arg_AUTHORIZATION_TOKEN)
    set(headers_param "HEADERS"
                      "Authorization: token ${arg_AUTHORIZATION_TOKEN}")
  endif()
  ...
  elseif(arg_REPO MATCHES [[^([^/]*)/([^/]*)/([^/]*)$]]) # 3 elements
    ...
    message(
      FATAL_ERROR
        "REPO (${arg_REPO}) is not a valid repo name. It must be:
  ...
  if(VCPKG_USE_HEAD_VERSION)
    if(DEFINED arg_HEAD_REF)
      ...
      set(working_directory_param "WORKING_DIRECTORY"
                                  "${CURRENT_BUILDTREES_DIR}/src/head")
  ...
  elseif(NOT DEFINED arg_REF)
    message(
      FATAL_ERROR "Package does not specify REF. It must be built using --head."
    )
  endif()
  ...
  if(DEFINED arg_FILE_DISAMBIGUATOR AND NOT VCPKG_USE_HEAD_VERSION)
    set(downloaded_file_name
        "${org_name}-${repo_name}-${sanitized_ref}-${arg_FILE_DISAMBIGUATOR}.tar.gz"
    )
  else()
    set(downloaded_file_name "${org_name}-${repo_name}-${sanitized_ref}.tar.gz")
  endif()

  # exports VCPKG_HEAD_VERSION to the caller. This will get picked up by
  # ports.cmake after the build. When multiple vcpkg_from_gitlab's are used
  # after each other, only use the version from the first (hopefully the primary
  # one).
  if(VCPKG_USE_HEAD_VERSION AND NOT DEFINED VCPKG_HEAD_VERSION)
    z_uri_encode("${arg_REPO}" encoded_repo_path)
    set(version_url
        "${arg_GITLAB_URL}/api/v4/projects/${encoded_repo_path}/repository/branches/${arg_HEAD_REF}"
    )
    ...
    if(NOT version_contents MATCHES [["id":(\ *)"([a-f0-9]+)"]])
      message(
        FATAL_ERROR
          "Failed to parse API response from '${version_url}':\n${version_contents}\n"
      )
    endif()
    set(VCPKG_HEAD_VERSION
        "${CMAKE_MATCH_2}"
        PARENT_SCOPE)
  endif()
  ...
  vcpkg_download_distfile(
    archive
    URLS
    "${gitlab_link}/-/archive/${ref_to_use}/${repo_name}-${ref_to_use}.tar.gz"
    FILENAME
