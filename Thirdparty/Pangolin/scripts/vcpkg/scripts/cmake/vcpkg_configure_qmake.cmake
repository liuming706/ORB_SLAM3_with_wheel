function(vcpkg_configure_qmake)
  ...
  find_program(
    qmake_executable
    NAMES qmake
    PATHS "${CURRENT_HOST_INSTALLED_DIR}/tools/qt5/bin"
    NO_DEFAULT_PATH)
  ...
  if(NOT DEFINED VCPKG_BUILD_TYPE OR VCPKG_BUILD_TYPE STREQUAL "release")
    ...
    configure_file("${CURRENT_INSTALLED_DIR}/tools/qt5/qt_release.conf"
                   "${current_binary_dir}/qt.conf")
    ...
    qmake_add_flags("QMAKE_CFLAGS_RELEASE" "+="
                    "${VCPKG_DETECTED_CMAKE_C_FLAGS_RELEASE}")
    qmake_add_flags("QMAKE_CXXFLAGS_RELEASE" "+="
    ...
    qmake_add_flags("QMAKE_LIBFLAGS_RELEASE" "+="
    ...
    if(DEFINED arg_BUILD_OPTIONS OR DEFINED arg_BUILD_OPTIONS_RELEASE)
      vcpkg_list(SET build_opt_param -- ${arg_BUILD_OPTIONS}
                 ${arg_BUILD_OPTIONS_RELEASE})
    endif()
    ...
    if(EXISTS "${current_binary_dir}/config.log")
      file(
        REMOVE
        "${CURRENT_BUILDTREES_DIR}/internal-config-${TARGET_TRIPLET}-rel.log")
      file(
        RENAME "${current_binary_dir}/config.log"
        "${CURRENT_BUILDTREES_DIR}/internal-config-${TARGET_TRIPLET}-rel.log")
    endif()
  ...
  if(NOT DEFINED VCPKG_BUILD_TYPE OR VCPKG_BUILD_TYPE STREQUAL "debug")
    ...
    configure_file("${CURRENT_INSTALLED_DIR}/tools/qt5/qt_debug.conf"
                   "${current_binary_dir}/qt.conf")
    ...
    qmake_add_flags("QMAKE_CFLAGS_DEBUG" "+="
                    "${VCPKG_DETECTED_CMAKE_C_FLAGS_DEBUG}")
    qmake_add_flags("QMAKE_CXXFLAGS_DEBUG" "+="
    ...
    if(EXISTS "${current_binary_dir}/config.log")
      file(
        REMOVE
        "${CURRENT_BUILDTREES_DIR}/internal-config-${TARGET_TRIPLET}-dbg.log")
      file(
        RENAME "${current_binary_dir}/config.log"
        "${CURRENT_BUILDTREES_DIR}/internal-config-${TARGET_TRIPLET}-dbg.log")
    endif()
