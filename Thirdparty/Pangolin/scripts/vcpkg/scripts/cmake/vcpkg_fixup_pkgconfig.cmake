function(z_vcpkg_fixup_pkgconfig_check_files file config)
  ...
  if(NOT "${error_var}" EQUAL "0")
    message(
      FATAL_ERROR
        "${PKGCONFIG} --exists ${package_name} failed with error code: ${error_var}
...
function(vcpkg_fixup_pkgconfig)
  ...
  if(DEFINED arg_RELEASE_FILES AND NOT DEFINED arg_DEBUG_FILES)
    message(
      FATAL_ERROR
        "DEBUG_FILES must be specified if RELEASE_FILES was specified.")
  endif()
  if(NOT DEFINED arg_RELEASE_FILES AND DEFINED arg_DEBUG_FILES)
    message(
      FATAL_ERROR
        "RELEASE_FILES must be specified if DEBUG_FILES was specified.")
  endif()
  ...
  string(REGEX REPLACE "^([a-zA-Z]):/" [[/\1/]] unix_packages_dir
                       "${CURRENT_PACKAGES_DIR}")
  string(REGEX REPLACE "^([a-zA-Z]):/" [[/\1/]] unix_installed_dir
                       "${CURRENT_INSTALLED_DIR}")

  foreach(config IN ITEMS RELEASE DEBUG)
    debug_message("${config} Files: ${arg_${config}_FILES}")
    if("${VCPKG_BUILD_TYPE}" STREQUAL "debug" AND "${config}" STREQUAL
                                                  "RELEASE")
      continue()
    endif()
    if("${VCPKG_BUILD_TYPE}" STREQUAL "release" AND "${config}" STREQUAL
                                                    "DEBUG")
      continue()
    endif()
    foreach(file IN LISTS "arg_${config}_FILES")
      ...
      if("${config}" STREQUAL "DEBUG")
        ...
        cmake_path(RELATIVE_PATH relative_pc_path BASE_DIRECTORY
                   "${pkg_lib_search_path}")
      else()
      ...
      # this normalizes all files to end with a newline, and use LF instead of
      # CRLF; this allows us to use regex matches easier to modify these files.
      if(NOT "${contents}" MATCHES "\n$")
        string(APPEND contents "\n")
      endif()
      string(REPLACE "\r\n" "\n" contents "${contents}")

      string(REPLACE "${CURRENT_PACKAGES_DIR}" [[${prefix}]] contents
                     "${contents}")
      string(REPLACE "${CURRENT_INSTALLED_DIR}" [[${prefix}]] contents
                     "${contents}")
      string(REPLACE "${unix_packages_dir}" [[${prefix}]] contents
                     "${contents}")
      string(REPLACE "${unix_installed_dir}" [[${prefix}]] contents
                     "${contents}")
      ...
      if("${config}" STREQUAL "DEBUG")
        ...
        string(REPLACE [[${prefix}/include]] [[${prefix}/../include]] contents
                       "${contents}")
        string(REPLACE [[${prefix}/share]] [[${prefix}/../share]] contents
                       "${contents}")
      endif()
      # quote -L, -I, and -l paths starting with `${blah}`
      string(REGEX REPLACE " -([LIl])(\\\${[^}]*}[^ \n\t]*)" [[ -\1"\2"]]
                           contents "${contents}")
      # This section fuses XYZ.private and XYZ according to
      # VCPKG_LIBRARY_LINKAGE
      #
      # Pkgconfig searches Requires.private transitively for Cflags in the
      # dynamic case, which prevents us from removing it.
      #
      # Once this transformation is complete, users of vcpkg should never need
      # to pass --static.
      if("${VCPKG_LIBRARY_LINKAGE}" STREQUAL "static")
        # how this works: we want to transform: Libs: $1 Libs.private: $2 into
        # Libs: $1 $2 and the same thing for Requires and Requires.private
      ...
      file(WRITE "${file}"
           "prefix=\${pcfiledir}/${relative_pc_path}\n${contents}")
    endforeach()

    if(NOT arg_SKIP_CHECK) # The check can only run after all files have been
                           # corrected!
  ...
  set(Z_VCPKG_FIXUP_PKGCONFIG_CALLED
      TRUE
      CACHE INTERNAL "See below" FORCE)
  # Variable to check if this function has been called! Theoreotically vcpkg
  # could look for *.pc files and automatically call this function or check if
  # this function has been called if *.pc files are detected. The same is true
  # for vcpkg_fixup_cmake_targets
endfunction()
