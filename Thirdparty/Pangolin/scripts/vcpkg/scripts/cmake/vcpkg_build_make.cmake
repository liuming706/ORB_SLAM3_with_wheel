function(vcpkg_build_make)
  ...
  cmake_parse_arguments(
    PARSE_ARGV 0 arg "ADD_BIN_TO_PATH;ENABLE_INSTALL;DISABLE_PARALLEL"
    "LOGFILE_ROOT;BUILD_TARGET;SUBPATH;MAKEFILE;INSTALL_TARGET" "OPTIONS")

  if(DEFINED arg_UNPARSED_ARGUMENTS)
    message(
      WARNING
        "vcpkg_make_build was passed extra arguments: ${arg_UNPARSED_ARGUMENTS}"
    )
  endif()
  ...
  if(CMAKE_HOST_WIN32)
    ...
    string(REGEX REPLACE [[([a-zA-Z]):/]] [[/\1/]] vcpkg_package_prefix
                         "${vcpkg_package_prefix}")
    vcpkg_list(
      SET
      install_opts
      -j
      ...
      ${arg_MAKEFILE}
      ${arg_INSTALL_TARGET}
      DESTDIR=${vcpkg_package_prefix})
    vcpkg_list(
      SET
      no_parallel_install_opts
      -j
      1
      --trace
      -f
  ...
  # Since includes are buildtype independent those are setup by
  ...
  foreach(buildtype IN ITEMS "debug" "release")
    ...
    if(NOT DEFINED VCPKG_BUILD_TYPE OR VCPKG_BUILD_TYPE STREQUAL "${buildtype}")
      ...
      set(working_directory
          "${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}${short_buildtype}/${arg_SUBPATH}"
      )
      message(STATUS "Building ${TARGET_TRIPLET}${short_buildtype}")

      z_vcpkg_extract_cpp_flags_and_set_cflags_and_cxxflags(
        "${cmake_buildtype}")

      if(VCPKG_LIBRARY_LINKAGE STREQUAL "static")
        set(LINKER_FLAGS_${cmake_buildtype}
            "${VCPKG_DETECTED_CMAKE_STATIC_LINKER_FLAGS_${cmake_buildtype}}")
      else() # dynamic
        set(LINKER_FLAGS_${cmake_buildtype}
            "${VCPKG_DETECTED_CMAKE_SHARED_LINKER_FLAGS_${cmake_buildtype}}")
      endif()
      set(LDFLAGS_${cmake_buildtype} "")
      if(EXISTS "${Z_VCPKG_INSTALLED}${path_suffix}/lib")
        string(APPEND LDFLAGS_${cmake_buildtype}
               " -L${Z_VCPKG_INSTALLED}${path_suffix}/lib")
      endif()
      ...
      if(CMAKE_HOST_WIN32 AND VCPKG_DETECTED_CMAKE_C_COMPILER MATCHES "cl.exe")
        set(LINK_ENV_${cmake_buildtype}
            "$ENV{_LINK_} ${LINKER_FLAGS_${cmake_buildtype}}")
      else()
        string(APPEND LDFLAGS_${cmake_buildtype}
      ...
      foreach(lib_env_var IN LISTS lib_env_vars)
        if(EXISTS "${Z_VCPKG_INSTALLED}${path_suffix}/lib")
          vcpkg_host_path_list(PREPEND ENV{${lib_env_var}}
                               "${Z_VCPKG_INSTALLED}${path_suffix}/lib")
        endif()
        if(EXISTS "${Z_VCPKG_INSTALLED}${path_suffix}/lib/manual-link")
          vcpkg_host_path_list(
            PREPEND ENV{${lib_env_var}}
            "${Z_VCPKG_INSTALLED}${path_suffix}/lib/manual-link")
        endif()
      endforeach()
      unset(lib_env_vars)
      ...
      vcpkg_list(SET no_parallel_make_cmd_line ${make_command}
                 ${no_parallel_make_opts})
      ...
      file(
        READ
        "${CURRENT_BUILDTREES_DIR}/${arg_LOGFILE_ROOT}-${TARGET_TRIPLET}${short_buildtype}-out.log"
        logdata)
      if(logdata MATCHES
         "Warning: linker path does not have real file for library")
        message(
          FATAL_ERROR "libtool could not find a file being linked against!")
      endif()
      ...
      if(arg_ENABLE_INSTALL)
        ...
        vcpkg_execute_build_process(
          ...
          "${working_directory}"
          LOGNAME
          "install-${TARGET_TRIPLET}${short_buildtype}")
      endif()
  ...
  if(arg_ENABLE_INSTALL)
    ...
    file(RENAME "${CURRENT_PACKAGES_DIR}_tmp${Z_VCPKG_INSTALL_PREFIX}"
         "${CURRENT_PACKAGES_DIR}")
    file(REMOVE_RECURSE "${CURRENT_PACKAGES_DIR}_tmp")
  endif()

  # Remove libtool files since they contain absolute paths and are not
  # necessary.
