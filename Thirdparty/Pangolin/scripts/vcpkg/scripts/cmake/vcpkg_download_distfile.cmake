...
function(z_vcpkg_download_distfile_test_hash file_path kind error_advice sha512
         skip_sha512)
...
function(z_vcpkg_download_distfile_via_aria)
  cmake_parse_arguments(PARSE_ARGV 1 arg "SKIP_SHA512" "FILENAME;SHA512"
                        "URLS;HEADERS")
  ...
  foreach(URL IN LISTS arg_URLS)
    debug_message(
      "Download Command: ${ARIA2} ${URL} -o temp/${filename} -l download-${filename}-detailed.log ${headers_param}"
    )
    vcpkg_execute_in_download_mode(
      ...
      temp/${arg_FILENAME}
      -l
      download-${arg_FILENAME}-detailed.log
      ${headers_param}
      OUTPUT_FILE
      download-${arg_FILENAME}-out.log
      ERROR_FILE
      download-${arg_FILENAME}-err.log
      RESULT_VARIABLE
      error_code
      WORKING_DIRECTORY
      "${DOWNLOADS}")
  ...
  if(NOT "${error_code}" STREQUAL "0")
    message(
      STATUS "Downloading ${arg_FILENAME}... Failed.\n"
    ...
    z_vcpkg_download_distfile_test_hash(
      "${DOWNLOADS}/temp/${arg_FILENAME}" "downloaded file"
      "The file may have been corrupted in transit." "${arg_SHA512}"
      ${arg_SKIP_SHA512})
    file(REMOVE ${DOWNLOADS}/download-${arg_FILENAME}-out.log
         ${DOWNLOADS}/download-${arg_FILENAME}-err.log
         ${DOWNLOADS}/download-${arg_FILENAME}-detailed.log)
    get_filename_component(downloaded_file_dir "${downloaded_file_path}"
                           DIRECTORY)
...
function(vcpkg_download_distfile out_var)
  cmake_parse_arguments(
    PARSE_ARGV 1 arg "SKIP_SHA512;SILENT_EXIT;QUIET;ALWAYS_REDOWNLOAD"
    "FILENAME;SHA512" "URLS;HEADERS")
  ...
  if(arg_SILENT_EXIT)
    message(
      WARNING
        "SILENT_EXIT has been deprecated as an argument to vcpkg_download_distfile -- remove the argument to resolve this warning"
    )
  endif()
...
If you do not know the SHA512, add it as 'SHA512 0' and re-run this command.")
  elseif(arg_SKIP_SHA512 AND DEFINED arg_SHA512)
    message(
      FATAL_ERROR
        "vcpkg_download_distfile must not be passed both SHA512 and SKIP_SHA512."
    )
  endif()
  ...
  if(NOT arg_SKIP_SHA512)
    if("${arg_SHA512}" STREQUAL "0")
      ...
      if(NOT "${arg_SHA512_length}" EQUAL "128" OR NOT "${arg_SHA512}" MATCHES
                                                   "^[a-zA-Z0-9]*$")
  ...
  if(EXISTS "${downloaded_file_path}" AND NOT arg_SKIP_SHA512)
    ...
    if(NOT hash_match)
      ...
      set(arg_FILENAME
          "${directory_component}${filename_component}-${hash}${extension_component}"
      )
      set(downloaded_file_path "${DOWNLOADS}/${arg_FILENAME}")
    endif()
  endif()
  ...
  # Works around issue #3399 Delete "temp0" directory created by the old version
  # of vcpkg
  ...
  if(EXISTS "${downloaded_file_path}" AND NOT arg_ALWAYS_REDOWNLOAD)
    ...
    if(_VCPKG_NO_DOWNLOADS)
      string(
        APPEND
        advice_message
        " Downloads are disabled please provide a valid file at path ${downloaded_file_path} and retry."
      )
    else()
      string(
        APPEND
        advice_message
        " To re-download this file please delete cached file at path ${downloaded_file_path} and retry."
      )
    endif()

    z_vcpkg_download_distfile_test_hash(
      "${downloaded_file_path}" "cached file" "${advice_message}"
      "${arg_SHA512}" "${arg_SKIP_SHA512}")
  ...
  # vcpkg_download_distfile_ALWAYS_REDOWNLOAD only triggers when NOT
  # _VCPKG_NO_DOWNLOADS this could be de-morgan'd out but it's more clear this
  # way
  if(_VCPKG_NO_DOWNLOADS)
    if(NOT EXISTS "${downloaded_file_path}")
      message(
        FATAL_ERROR
          "Downloads are disabled, but '${downloaded_file_path}' does not exist."
      )
    endif()
  ...
  if(NOT arg_DISABLE_ARIA2
     AND _VCPKG_DOWNLOAD_TOOL STREQUAL "ARIA2"
     AND NOT EXISTS "${downloaded_file_path}")
    if(arg_SKIP_SHA512)
  ...
  if(NOT EXISTS "${downloaded_file_path}" OR arg_ALWAYS_REDOWNLOAD)
    vcpkg_execute_in_download_mode(
      ...
      ${headers_param}
      --debug
      --feature-flags=-manifests # there's a bug in vcpkg x-download when it
                                 # finds a manifest-root
      OUTPUT_VARIABLE
      output
      ERROR_VARIABLE
      output
