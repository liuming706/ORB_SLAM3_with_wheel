function(z_vcpkg_install_gn_get_target_type out_var)
  ...
  if(DEFINED arg_UNPARSED_ARGUMENTS)
    message(
      FATAL_ERROR
        "Internal error: get_target_type was passed extra arguments: ${arg_UNPARSED_ARGUMENTS}"
    )
  endif()
...
function(z_vcpkg_install_gn_get_desc out_var)
  cmake_parse_arguments(PARSE_ARGV 1 "arg" ""
                        "SOURCE_PATH;BUILD_DIR;TARGET;WHAT_TO_DISPLAY" "")
  if(DEFINED arg_UNPARSED_ARGUMENTS)
    message(
      FATAL_ERROR
        "Internal error: get_desc was passed extra arguments: ${arg_UNPARSED_ARGUMENTS}"
    )
  endif()
...
function(z_vcpkg_install_gn_install)
  cmake_parse_arguments(PARSE_ARGV 0 "arg" ""
                        "SOURCE_PATH;BUILD_DIR;INSTALL_DIR" "TARGETS")
  if(DEFINED arg_UNPARSED_ARGUMENTS)
    message(
      FATAL_ERROR
        "Internal error: install was passed extra arguments: ${arg_UNPARSED_ARGUMENTS}"
    )
  endif()

  foreach(target IN LISTS arg_TARGETS)
    # GN targets must start with a //
    z_vcpkg_install_gn_get_desc(
      outputs
      SOURCE_PATH
      "${arg_SOURCE_PATH}"
      BUILD_DIR
      ...
      WHAT_TO_DISPLAY
      outputs)
    z_vcpkg_install_gn_get_target_type(
      target_type
      SOURCE_PATH
      "${arg_SOURCE_PATH}"
      BUILD_DIR
    ...
    foreach(output IN LISTS outputs)
      ...
      if(NOT EXISTS "${output}")
        message(
          WARNING "Output for target `${target}` doesn't exist: ${output}.")
        continue()
      endif()
...
function(vcpkg_install_gn)
  if(Z_VCPKG_GN_INSTALL_GUARD)
    message(
      FATAL_ERROR
        "The ${PORT} port already depends on vcpkg-gn; using both vcpkg-gn and vcpkg_install_gn in the same port is unsupported."
    )
  endif()
  ...
  if(DEFINED arg_UNPARSED_ARGUMENTS)
    message(
      WARNING
        "vcpkg_install_gn was passed extra arguments: ${arg_UNPARSED_ARGUMENTS}"
    )
  endif()
