...
vcpkg_download_distfile(
  ARCHIVE
  URLS
  "https://ftp.mozilla.org/pub/security/nss/releases/NSS_${V_URL}_RTM/src/nss-${NSS_VERSION}.tar.gz"
  FILENAME
  "nss-${NSS_VERSION}.tar.gz"
  SHA512
...
if(VCPKG_TARGET_IS_WINDOWS)
  ...
  find_program(
    MOZBUILD_MAKE_COMMAND make
    PATHS "${MOZBUILD_MSYS_ROOT}/bin"
  ...
  find_program(
    MOZBUILD_PYTHON python
    PATHS "${MOZBUILD_ROOT}/python"
    NO_DEFAULT_PATH REQUIRED)
...
message(
  STATUS
    "Using headers from: ${VCPKG_INCLUDEDIR} arch: ${VCPKG_TARGET_ARCHITECTURE}"
)
...
message(
  STATUS
    "Using libraries from: ${VCPKG_LIBDIR} arch: ${VCPKG_TARGET_ARCHITECTURE}")
...
set(OPTIONS
    "-v" "-g" "--disable-tests"
    "--with-nspr=${VCPKG_INCLUDEDIR}/nspr:${VCPKG_LIBDIR}" "--system-sqlite"
    "-Dsign_libs=0")
...
if(NOT VCPKG_BUILD_TYPE OR VCPKG_BUILD_TYPE STREQUAL "debug")
  ...
  vcpkg_execute_required_process(
    ...
    ${OPTIONS}
    WORKING_DIRECTORY
    ${VCPKG_BINARY_DIR}-dbg/nss
    LOGNAME
    build-${TARGET_TRIPLET}${short_buildtype})
endif()
...
file(COPY "${VCPKG_BINARY_DIR}-rel/dist/public/nss"
     DESTINATION "${CURRENT_PACKAGES_DIR}/include")

file(COPY "${VCPKG_BINARY_DIR}-rel/dist/private/nss/"
     DESTINATION "${CURRENT_PACKAGES_DIR}/include/nss/private")

# Release libraries

file(GLOB LIB_RELEASE "${VCPKG_BINARY_DIR}-rel/dist/Release/lib/*.dll"
...
if(LIB_RELEASE_SIZE GREATER 0)
  ...
  file(COPY "${VCPKG_BINARY_DIR}-rel/dist/Release/lib"
...
vcpkg_copy_tools(
  ...
  "validation"
  SEARCH_DIR
  "${VCPKG_BINARY_DIR}-rel/dist/Release/bin/"
  DESTINATION
  "${CURRENT_PACKAGES_DIR}/tools/${PORT}")

# Debug libraries
if(NOT VCPKG_BUILD_TYPE OR VCPKG_BUILD_TYPE STREQUAL "debug")
  file(GLOB LIB_DEBUG "${VCPKG_BINARY_DIR}-dbg/dist/Debug/lib/*.dll"
  ...
  if(LIB_DEBUG_SIZE GREATER 0)
    ...
    file(COPY "${VCPKG_BINARY_DIR}-dbg/dist/Debug/lib"
...
# Copy license
file(
  INSTALL "${SOURCE_PATH}/nss/COPYING"
  DESTINATION "${CURRENT_PACKAGES_DIR}/share/nss"
  RENAME copyright)
