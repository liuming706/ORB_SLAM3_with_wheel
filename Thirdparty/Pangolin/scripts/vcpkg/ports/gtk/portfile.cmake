vcpkg_from_gitlab(
  ...
  GNOME/gtk
  REF
  73bea05c3386075528542c2714790199f2bb5861 # v4.6.8
  SHA512
  5e7b994d1e26a4d97bd5788b61ca3a43a04c96c10161d29e185d5854b925e07b1bba5f1e35509d4df078f918bb6cc7149bcb720aaec8c04e5d8254280c0076ed
  HEAD_REF
  master # branch name
  PATCHES
  0001-build.patch
...
vcpkg_add_to_path("${PKGCONFIG_DIR}") # Post install script runs pkg-config so
...
list(APPEND OPTIONS -Dx11-backend=${x11}) # Enable the X11 gdk backend (only
                                          # when building on Unix)
list(APPEND OPTIONS -Dbroadway-backend=false) # Enable the broadway (HTML5) gdk
                                              # backend
list(APPEND OPTIONS -Dwin32-backend=${win32}) # Enable the Windows gdk backend
                                              # (only when building on Windows)
list(APPEND OPTIONS -Dmacos-backend=${osx}) # Enable the macOS gdk backend (only
...
# If somebody finds out how to access and forward env variables to the meson
# install script be my guest. Nevertheless the script still needs manual
# execution in the crosscompiling case.
...
foreach(_config release debug)
  ...
  set(PKGCONFIG_INSTALLED_DIR
      "${CURRENT_INSTALLED_DIR}${_path_suffix}/lib/pkgconfig/")
  set(ENV{PKG_CONFIG_PATH} "${PKGCONFIG_INSTALLED_DIR}")
  # file(MAKE_DIRECTORY
  # "${CURRENT_PACKAGES_DIR}${_path_suffix}/lib/gtk-4.0/4.0.0/media")
  # file(MAKE_DIRECTORY
  # "${CURRENT_PACKAGES_DIR}${_path_suffix}/lib/gtk-4.0/4.0.0/immodules")
  # file(MAKE_DIRECTORY
  # "${CURRENT_PACKAGES_DIR}${_path_suffix}/lib/gtk-4.0/4.0.0/printbackends")
  vcpkg_execute_required_process(
    COMMAND
    "${PYTHON3}"
    "${SOURCE_PATH}/build-aux/meson/post-install.py"
    4.0
    4.0.0
    "${CURRENT_PACKAGES_DIR}${_path_suffix}/lib"
    "${CURRENT_PACKAGES_DIR}${_path_suffix}/share"
    "${CURRENT_PACKAGES_DIR}${_path_suffix}/bin"
    WORKING_DIRECTORY
    ${SOURCE_PATH}
    LOGNAME
    post-install-${TARGET_TRIPLET}-${_short})
  ...
  # file(RENAME "${CURRENT_PACKAGES_DIR}${_path_suffix}/lib/gtk-4.0/"
  # "${CURRENT_PACKAGES_DIR}${_path_suffix}/bin/gtk-4.0")
...
set(TOOL_NAMES gtk4-builder-tool gtk4-encode-symbolic-svg gtk4-query-settings
