vcpkg_from_github(
  ...
  v2020.06
  SHA512
  65b0c8a4520d1d282420c30ecd7c8525525d4dbb6e562e1e2e93d110f4eb686af43f098bf02460727fab1e1f9446dd00a99051e150c05ea40b1486a44fea1042
  HEAD_REF
  master
  PATCHES
...
# Remove platform-specific feature that are not available on current target
# platform from all features.
...
if(NOT VCPKG_USE_HEAD_VERSION)
  list(REMOVE_ITEM ALL_SUPPORTED_FEATURES anyshaderconverter shadertools
       shaderconverter vk-info)
  message(
    WARNING
      "Features anyshaderconverter, shadertools, shaderconverter and vk-info are not avaliable when building non-head version."
  )
endif()
...
set(_TOOLS al-info distancefieldconverter fontconverter gl-info imageconverter
           sceneconverter)
...
# the linker implicitly takes everything from the root lib/ folder, the static
# libraries have to be moved out of lib/magnum/blah/ directly to lib/. Possibly
# would be enough to do this just for Windows, doing it also on other platforms
# for consistency.
#
...
# AnyImageImporter). Thus the Any* plugin lib files are kept, but also not moved
# to the root lib/ folder, to prevent autolinking. A consequence of the *.lib
# file removal is that downstream projects can't implement Magnum plugins that
# would depend on (and thus link to) these, but that's considered a very rare
# use case and so it's fine.
#
...
if(VCPKG_LIBRARY_LINKAGE STREQUAL "static")
  ...
  if(WIN32)
    ...
    file(GLOB_RECURSE LIB_TO_REMOVE_DBG
         "${CURRENT_PACKAGES_DIR}/debug/lib/magnum-d/*")
    file(GLOB_RECURSE LIB_TO_KEEP_DBG
         "${CURRENT_PACKAGES_DIR}/debug/lib/magnum-d/*Any*")
    if(LIB_TO_KEEP_DBG)
    ...
    file(REMOVE_RECURSE
         "${CURRENT_PACKAGES_DIR}/debug/lib/magnum-d/fontconverters")
  endif()

  file(COPY "${CMAKE_CURRENT_LIST_DIR}/magnumdeploy.ps1"
       DESTINATION "${CURRENT_PACKAGES_DIR}/bin/magnum")
  file(COPY "${CMAKE_CURRENT_LIST_DIR}/magnumdeploy.ps1"
       DESTINATION "${CURRENT_PACKAGES_DIR}/debug/bin/magnum-d")
endif()
