...
vcpkg_from_sourceforge(
  ...
  remove_lame_init_old_from_symbol_list.patch # deprecated
                                              # https://github.com/zlargon/lame/blob/master/include/lame.h#L169
...
if(VCPKG_TARGET_IS_WINDOWS AND NOT VCPKG_TARGET_IS_MINGW)
  ...
  file(
    GLOB vcxprojs
    RELATIVE "${SOURCE_PATH}/vc_solution"
    "${SOURCE_PATH}/vc_solution/vc11_*.vcxproj")
  foreach(vcxproj ${vcxprojs})
    file(READ "${SOURCE_PATH}/vc_solution/${vcxproj}" vcxproj_con)

    if(NOT VCPKG_CRT_LINKAGE STREQUAL dynamic)
      string(REPLACE "DLL</RuntimeLibrary>" "</RuntimeLibrary>" vcxproj_con
                     "${vcxproj_con}")
    endif()

    string(REPLACE "/machine:x86" "/machine:${machine}" vcxproj_con
                   "${vcxproj_con}")
    string(REPLACE "<Platform>Win32</Platform>"
                   "<Platform>${platform}</Platform>" vcxproj_con
                   "${vcxproj_con}")
    string(REPLACE "|Win32" "|${platform}" vcxproj_con "${vcxproj_con}")
    string(REPLACE "Include=\"vc11_" "Include=\"${machine}_vc11_" vcxproj_con
                   "${vcxproj_con}")
    ...
    file(WRITE "${SOURCE_PATH}/vc_solution/${machine}_${vcxproj}"
         "${vcxproj_con}")
  endforeach()
  ...
  vcpkg_install_msbuild(
    ...
    "${SOURCE_PATH}"
    PROJECT_SUBPATH
    "vc_solution/${machine}_vc11_lame.sln"
    TARGET
    "lame"
    PLATFORM
    "${platform}")
...
file(COPY "${SOURCE_PATH}/include/lame.h"
     DESTINATION "${CURRENT_PACKAGES_DIR}/include/lame")
configure_file(
  "${CMAKE_CURRENT_LIST_DIR}/Config.cmake.in"
  "${CURRENT_PACKAGES_DIR}/share/${PORT}/mp3lame-config.cmake" @ONLY)
