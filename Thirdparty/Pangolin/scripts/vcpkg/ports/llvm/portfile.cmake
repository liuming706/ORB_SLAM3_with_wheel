...
vcpkg_from_github(
  ...
  main
  PATCHES
  0002-fix-install-paths.patch # This patch fixes paths in ClangConfig.cmake,
                               # LLVMConfig.cmake, LLDConfig.cmake etc.
...
# LLVM generates CMake error due to Visual Studio version 16.4 is known to
# miscompile part of LLVM. LLVM_TEMPORARILY_ALLOW_OLD_TOOLCHAIN=ON disables this
# error. See
...
set(llvm_external_libraries zlib libxml2)
...
if("enable-assertions" IN_LIST FEATURES)
  ...
  list(APPEND FEATURE_OPTIONS -DLLVM_ENABLE_ASSERTIONS=ON)
elseif("disable-assertions" IN_LIST FEATURES)
  # Force disable assertions for all configurations.
  list(APPEND FEATURE_OPTIONS -DLLVM_ENABLE_ASSERTIONS=OFF)
endif()
...
if("clang" IN_LIST FEATURES OR "clang-tools-extra" IN_LIST FEATURES)
  ...
  # 1) LLVM/Clang tools are relocated from ./bin/ to ./tools/llvm/
  # (LLVM_TOOLS_INSTALL_DIR=tools/llvm) 2) Clang resource files are relocated
  # from ./lib/clang/<version> to ./tools/llvm/lib/clang/<version> (see patch
  # 0007-fix-compiler-rt-install-path.patch) So, the relative path should be
  # changed from ../lib/clang/<version> to ./lib/clang/<version>
...
if("flang" IN_LIST FEATURES)
  if(VCPKG_DETECTED_CMAKE_CXX_COMPILER_ID STREQUAL "MSVC"
    ...
    message(
      FATAL_ERROR
        "Building Flang with MSVC is not supported on x86. Disable it until issues are fixed."
    )
  endif()
...
if("pstl" IN_LIST FEATURES)
  if(VCPKG_DETECTED_CMAKE_CXX_COMPILER_ID STREQUAL "MSVC")
    message(
      FATAL_ERROR
        "Building pstl with MSVC is not supported. Disable it until issues are fixed."
    )
  endif()
...
if("libcxx" IN_LIST FEATURES)
  if(VCPKG_DETECTED_CMAKE_CXX_COMPILER_ID STREQUAL "MSVC")
    message(
      FATAL_ERROR
        "Building libcxx with MSVC is not supported, as cl doesn't support the #include_next extension."
    )
  endif()
...
if("libcxxabi" IN_LIST FEATURES)
  if(VCPKG_DETECTED_CMAKE_CXX_COMPILER_ID STREQUAL "MSVC")
    message(
      FATAL_ERROR
        "Building libcxxabi with MSVC is not supported. Disable it until issues are fixed."
    )
  endif()
...
set(known_llvm_experimental_targets SPRIV)
...
vcpkg_cmake_configure(
  ...
  # Force TableGen to be built with optimization. This will significantly
  # improve build time.
  ...
  # Limit the maximum number of concurrent link jobs to 1. This should fix low
  # amount of memory issue for link.
...
function(llvm_cmake_package_config_fixup package_name)
  cmake_parse_arguments("arg" "DO_NOT_DELETE_PARENT_CONFIG_PATH" "FEATURE_NAME"
                        "" ${ARGN})
  if(NOT DEFINED arg_FEATURE_NAME)
    set(arg_FEATURE_NAME ${package_name})
  endif()
  if("${arg_FEATURE_NAME}" STREQUAL "${PORT}" OR "${arg_FEATURE_NAME}" IN_LIST
                                                 FEATURES)
    set(args)
...
llvm_cmake_package_config_fixup("ParallelSTL" FEATURE_NAME "pstl"
...
if(empty_dirs)
  foreach(empty_dir IN LISTS empty_dirs)
    if(NOT EXISTS "${empty_dir}")
      message(
        SEND_ERROR
          "Directory '${empty_dir}' is not exist. Please remove it from the checking."
      )
    else()
      ...
      if(files_in_dir)
        message(
          SEND_ERROR
            "Directory '${empty_dir}' is not empty. Please remove it from the checking."
        )
      else()
...
if("mlir" IN_LIST FEATURES)
  vcpkg_replace_string("${CURRENT_PACKAGES_DIR}/share/mlir/MLIRConfig.cmake"
                       "set(MLIR_MAIN_SRC_DIR \"${SOURCE_PATH}/mlir\")" "")
  vcpkg_replace_string("${CURRENT_PACKAGES_DIR}/share/mlir/MLIRConfig.cmake"
                       "${CURRENT_BUILDTREES_DIR}" "\${MLIR_INCLUDE_DIRS}")
endif()

# LLVM still generates a few DLLs in the static build: * LLVM-C.dll *
# libclang.dll * LTO.dll * Remarks.dll
