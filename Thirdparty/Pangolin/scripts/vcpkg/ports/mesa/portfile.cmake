# Build-Depends: From X Window PR: zstd, drm (!windows), elfutils (!windows),
# wayland (!windows), wayland-protocols (!windows), xdamage, xshmfence
# (!windows), x11, xcb, xfixes, xext, xxf86vm, xrandr, xv, xvmc (!windows),
# egl-registry, opengl-registry, tool-meson Required LLVM modules: LLVM
# (modules: bitwriter, core, coroutines, engine, executionengine, instcombine,
# mcdisassembler, mcjit, scalaropts, transformutils) found: YES
...
if(VCPKG_TARGET_IS_WINDOWS)
  set(VCPKG_POLICY_DLLS_IN_STATIC_LIBRARY enabled) # some parts of this port can
                                                   # only build as a shared
                                                   # library.
endif()
...
vcpkg_from_gitlab(
  ...
  mesa-22.1.7
  SHA512
  41302fc55ef429c14b1595832db3a898380230f86d2b84ac1ae3bd453d0aad87ec7ad310004dc64fcf34f58d8ea2736c13971c04eba056bcc549a4e3cc7c9470
  FILE_DISAMBIGUATOR
  1
  HEAD_REF
  master
...
if(WIN32) # WIN32 HOST probably has win_flex and win_bison!
  if(NOT EXISTS "${FLEX_DIR}/flex${VCPKG_HOST_EXECUTABLE_SUFFIX}")
    if(FLEX_DIR MATCHES "${DOWNLOADS}")
      file(CREATE_LINK "${FLEX}"
           "${FLEX_DIR}/flex${VCPKG_HOST_EXECUTABLE_SUFFIX}")
    else()
      message(
        FATAL_ERROR
          "${PORT} requires flex being named flex on windows and not win_flex!\n(Can be solved by creating a simple link from win_flex to flex)"
      )
    endif()
  endif()
  if(NOT EXISTS "${BISON_DIR}/BISON${VCPKG_HOST_EXECUTABLE_SUFFIX}")
    if(BISON_DIR MATCHES "${DOWNLOADS}")
      file(CREATE_LINK "${BISON}"
           "${BISON_DIR}/bison${VCPKG_HOST_EXECUTABLE_SUFFIX}")
    else()
      message(
        FATAL_ERROR
          "${PORT} requires bison being named bison on windows and not win_bison!\n(Can be solved by creating a simple link from win_bison to bison)"
      )
    endif()
  endif()
endif()

# For features https://github.com/pal1000/mesa-dist-win should be probably
# studied a bit more.
...
list(APPEND MESA_OPTIONS -Dshared-glapi=enabled) # shared GLAPI required when
                                                 # building two or more of the
                                                 # following APIs - opengl,
                                                 # gles1 gles2
...
vcpkg_configure_meson(
  SOURCE_PATH "${SOURCE_PATH}" OPTIONS -Dgles-lib-suffix=_mesa
  # -D egl-lib-suffix=_mesa
  -Dbuild-tests=false ${MESA_OPTIONS})
vcpkg_install_meson()
