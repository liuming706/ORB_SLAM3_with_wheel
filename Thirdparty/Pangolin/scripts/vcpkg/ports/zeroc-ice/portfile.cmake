vcpkg_from_github(
  ...
  v3.7.7
  SHA512
  73c3a2bb14c9e145383e4026206edd3e03b29c60a33af628611bfdab71d69a3aed108ce4e6cbfd67eb852560110e3495b4bd238c8cdf0de9d1f8e2f1088513ee
  PATCHES
  md5i_fix.patch
  slice2swift.patch
  mcppd_fix.patch)
...
set(WIN_DEBUG_BUILD_DIR
    ${CURRENT_BUILDTREES_DIR}/${DEBUG_TRIPLET}/${SOURCE_PATH_SUFFIX})
set(WIN_RELEASE_BUILD_DIR
    ${CURRENT_BUILDTREES_DIR}/${RELEASE_TRIPLET}/${SOURCE_PATH_SUFFIX})
...
function(install_slices ORIGINAL_PATH RELATIVE_PATHS)
  foreach(RELATIVE_PATH ${RELATIVE_PATHS})
    ...
    if(EXISTS ${ORIGINAL_PATH}/${RELATIVE_PATH})
      file(COPY ${HEADER_FILES}
           DESTINATION ${CURRENT_PACKAGES_DIR}/share/ice/slice/${RELATIVE_PATH})
    endif()
  endforeach()
endfunction()
...
if("glacier2router" IN_LIST FEATURES)
  ...
  vcpkg_list(APPEND ICE_OPTIONAL_COMPONENTS_MSBUILD
             "/t:C++98\\glacier2cryptpermissionsverifier")
  vcpkg_list(APPEND ICE_OPTIONAL_COMPONENTS_MAKE "glacier2router")
  vcpkg_list(APPEND ICE_OPTIONAL_COMPONENTS_MAKE
...
if(NOT VCPKG_TARGET_IS_WINDOWS)
  ...
  # Setting these as environment variables, as .d files aren't generated the
  # first time passing them as arguments to make.
  ...
  install_includes("${UNIX_BUILD_DIR}/cpp/include"
                   "${ICE_INCLUDE_SUB_DIRECTORIES}")
  install_includes("${UNIX_BUILD_DIR}/cpp/include/generated"
                   "${ICE_INCLUDE_SUB_DIRECTORIES}")
  ...
  file(COPY ${ICE_RELEASE_EXECUTABLES}
  ...
  vcpkg_execute_build_process(
    ...
    ${SOURCE_PATH}/cpp
    LOGNAME
    make-clean-after-build-${TARGET_TRIPLET})
  ...
  # Fix project files to prevent nuget restore of dependencies and remove hard
  # coded runtime linkage
  include("${CURRENT_PORT_DIR}/prepare_for_build.cmake")
  prepare_for_build("${SOURCE_PATH}")

  vcpkg_list(SET MSBUILD_OPTIONS "/p:UseVcpkg=yes" "/p:IceBuildingSrc=yes"
  ...
  vcpkg_install_msbuild(
    ...
    ${SOURCE_PATH}
    PROJECT_SUBPATH
    "cpp/msbuild/ice.${VCPKG_PLATFORM_TOOLSET}.sln"
    SKIP_CLEAN
    TARGET
    "C++11\\ice++11"
  ...
  if(EXISTS "${CURRENT_PACKAGES_DIR}/debug/bin/zeroc.icebuilder.msbuild.dll")
    file(REMOVE
         "${CURRENT_PACKAGES_DIR}/debug/bin/zeroc.icebuilder.msbuild.dll")
  endif()

  install_includes("${WIN_RELEASE_BUILD_DIR}/cpp/include"
                   "${ICE_INCLUDE_SUB_DIRECTORIES}")
  install_includes(
    "${WIN_RELEASE_BUILD_DIR}/cpp/include/generated/cpp11/${TRIPLET_SYSTEM_ARCH}/Release"
    "${ICE_INCLUDE_SUB_DIRECTORIES}")
...
file(
  ...
  DESTINATION ${CURRENT_PACKAGES_DIR}/share/zeroc-ice
  RENAME copyright)
