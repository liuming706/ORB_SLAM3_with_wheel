...
function(x_vcpkg_get_python_packages)
  cmake_parse_arguments(
    PARSE_ARGV 0 arg ""
    "PYTHON_VERSION;PYTHON_EXECUTABLE;REQUIREMENTS_FILE;OUT_PYTHON_VAR"
    "PACKAGES")
  ...
  if(NOT DEFINED arg_PYTHON_EXECUTABLE AND NOT DEFINED arg_PYTHON_VERSION)
    message(
      FATAL_ERROR "PYTHON_EXECUTABLE or PYTHON_VERSION must be specified.")
  ...
  if("${python_dir}" MATCHES "(${DOWNLOADS}|${CURRENT_HOST_INSTALLED_DIR})"
     AND CMAKE_HOST_WIN32) # inside vcpkg and windows host.
    # if(NOT EXISTS "${python_dir}/python310._pth" AND PYTHON_EXECUTABLE MATCHES
    # "python3") file(COPY
    # "${CURRENT_HOST_INSTALLED_DIR}/share/vcpkg-get-python-packages/python310._pth"
    # DESTINATION "${python_dir}") endif()
    if(NOT EXISTS "${python_dir}/easy_install${VCPKG_HOST_EXECUTABLE_SUFFIX}")
      if(NOT EXISTS "${python_dir}/Scripts/pip${VCPKG_HOST_EXECUTABLE_SUFFIX}")
        if(arg_PYTHON_VERSION STREQUAL 3)
          vcpkg_from_github(
            ...
            pypa/get-pip
            REF
            38e54e5de07c66e875c11a1ebbdb938854625dd8 # 2022-03-07
            SHA512
            431a9f66618a2f251db3a8c3311e7fc3af3ff7364ec1d14a99f1b9c237646b6146cef8b9471d83e1086dba2ed448bccc48d99b2bb375e4235d78e76d9970d3e5
          )
          vcpkg_execute_required_process(
            COMMAND
            "${arg_PYTHON_EXECUTABLE}"
            "${PYFILE_PATH}/public/get-pip.py"
            --no-warn-script-location
            WORKING_DIRECTORY
            "${CURRENT_BUILDTREES_DIR}"
            LOGNAME
            "get-pip-${TARGET_TRIPLET}")
        elseif(arg_PYTHON_VERSION STREQUAL 2)
          vcpkg_download_distfile(
            ...
            "get-pip.py"
            SHA512
            8c74bdaff57a2dcf2aa69c4c218b7d5f3bf4a470dbda2d7c8d1b53862c84e2a83cd04c3cd20cf80dc0e4076b113a734413e31d6a9853f41e894398e7f88f848e
          )
          vcpkg_execute_required_process(
            COMMAND
            "${arg_PYTHON_EXECUTABLE}"
      ...
      vcpkg_execute_required_process(
        COMMAND
        "${python_dir}/Scripts/pip${VCPKG_HOST_EXECUTABLE_SUFFIX}"
        install
        virtualenv
        --no-warn-script-location
        WORKING_DIRECTORY
        "${CURRENT_BUILDTREES_DIR}"
        LOGNAME
        "pip-install-virtualenv-${TARGET_TRIPLET}")
    else()
      vcpkg_execute_required_process(
        COMMAND
        "${python_dir}/easy_install${VCPKG_HOST_EXECUTABLE_SUFFIX}"
        virtualenv
        --no-warn-script-location # ${_package}
        WORKING_DIRECTORY
        "${CURRENT_BUILDTREES_DIR}"
        LOGNAME
        "easy-install-virtualenv-${TARGET_TRIPLET}")
    endif()
  endif()
  ...
  if(CMAKE_HOST_WIN32)
    ...
    if(arg_PYTHON_VERSION STREQUAL 3)
      file(COPY "${python_dir}/python310.zip"
           DESTINATION "${venv_path}/Scripts")
    endif()
  ...
  vcpkg_execute_required_process(
    ...
    "${CURRENT_BUILDTREES_DIR}"
    LOGNAME
    "venv-setup-${TARGET_TRIPLET}")
  vcpkg_add_to_path(PREPEND "${venv_path}${python_sub_path}")
  set(${arg_OUT_PYTHON_VAR}
      "${venv_path}${python_sub_path}/python${VCPKG_HOST_EXECUTABLE_SUFFIX}")
  ...
  if(DEFINED arg_REQUIREMENTS_FILE)
    ...
    vcpkg_execute_required_process(
      ...
      "${CURRENT_BUILDTREES_DIR}"
      LOGNAME
      "pip-install-requirements-file-${TARGET_TRIPLET}")
  endif()
