...
string(COMPARE EQUAL "${VCPKG_LIBRARY_LINKAGE}" "static"
...
vcpkg_from_github(
  ...
  STEllAR-GROUP/hpx
  REF
  f48c81865800ae72618b40b4e79d4168cfb0bd56
  SHA512
  06d1384615b327194d871145c1899317bd350c43a072be2cbfdc773f7869b71aafce3e9cabf835a8fe902a13d7050d5e0400a76f74023985575347a645196b1d
  HEAD_REF
  stable
  PATCHES
...
vcpkg_check_features(
  ...
  FEATURES
  "zlib"
  HPX_WITH_COMPRESSION_ZLIB
  "snappy"
  HPX_WITH_COMPRESSION_SNAPPY
  "bzip2"
  HPX_WITH_COMPRESSION_BZIP2
  "cuda"
  HPX_WITH_CUDA
  "mpi"
  HPX_WITH_PARCELPORT_MPI
  "mpi"
  HPX_WITH_PARCELPORT_MPI_MULTITHREADED)
...
foreach(CMAKE_FILE IN LISTS CMAKE_FILES)
  ...
  string(REGEX REPLACE "lib/([A-Za-z0-9_.-]+\\.dll)" "bin/\\1" _contents
                       "${_contents}")
  string(REGEX REPLACE "lib/hpx/([A-Za-z0-9_.-]+\\.dll)" "bin/hpx/\\1"
                       _contents "${_contents}")
...
vcpkg_replace_string(
  ...
  "set(CMAKE_MODULE_PATH \${CMAKE_MODULE_PATH}" "list(APPEND CMAKE_MODULE_PATH")
...
file(REMOVE "${CURRENT_PACKAGES_DIR}/bin/hpxcxx"
     "${CURRENT_PACKAGES_DIR}/debug/bin/hpxcxx")

file(MAKE_DIRECTORY "${CURRENT_PACKAGES_DIR}/tools/${PORT}")
file(RENAME "${CURRENT_PACKAGES_DIR}/bin/hpxrun.py"
     "${CURRENT_PACKAGES_DIR}/tools/${PORT}/hpxrun.py")
vcpkg_replace_string(
  "${CURRENT_PACKAGES_DIR}/tools/${PORT}/hpxrun.py"
  "'${CURRENT_INSTALLED_DIR}/tools/openmpi/bin/mpiexec'" "'mpiexec'")
