...
function(vcpkg_cmake_configure)
  ...
  if(NOT arg_Z_CMAKE_GET_VARS_USAGE AND DEFINED CACHE{Z_VCPKG_CMAKE_GENERATOR})
    message(
      WARNING
        "${CMAKE_CURRENT_FUNCTION} already called; this function should only be called once."
    )
  endif()
  if(arg_PREFER_NINJA)
    message(
      WARNING
        "PREFER_NINJA has been deprecated in ${CMAKE_CURRENT_FUNCTION}. Please remove it from the portfile!"
    )
  endif()
  ...
  if(host_architecture STREQUAL "x86" OR DEFINED
  ...
  if(arg_WINDOWS_USE_MSBUILD
     AND VCPKG_HOST_IS_WINDOWS
    ...
    if(NOT generator)
      message(
        FATAL_ERROR
          "Unable to determine appropriate Visual Studio generator for triplet ${TARGET_TRIPLET}:
  ...
  if(NOT generator)
    ...
    message(
      FATAL_ERROR
        "Unable to determine appropriate generator for: "
  ...
  if(generator STREQUAL "Ninja")
    ...
    # If we use Ninja, it must be on PATH for CMake's ExternalProject, cf.
  ...
  file(REMOVE_RECURSE "${build_dir_release}" "${build_dir_debug}")
  ...
  z_vcpkg_cmake_configure_both_set_or_unset(VCPKG_CXX_FLAGS_DEBUG
                                            VCPKG_C_FLAGS_DEBUG)
  z_vcpkg_cmake_configure_both_set_or_unset(VCPKG_CXX_FLAGS_RELEASE
  ...
  foreach(config_var IN ITEMS INSTALL_NAME_DIR OSX_DEPLOYMENT_TARGET
                              OSX_SYSROOT OSX_ARCHITECTURES)
    if(DEFINED VCPKG_${config_var})
      vcpkg_list(APPEND arg_OPTIONS
                 "-DCMAKE_${config_var}=${VCPKG_${config_var}}")
    endif()
  endforeach()
  ...
  if(NOT arg_DISABLE_PARALLEL_CONFIGURE)
    ...
    if(NOT DEFINED VCPKG_BUILD_TYPE OR "${VCPKG_BUILD_TYPE}" STREQUAL "release")
      z_vcpkg_configure_cmake_build_cmakecache(ninja_configure_contents ".."
                                               "rel")
    endif()
    if(NOT DEFINED VCPKG_BUILD_TYPE OR "${VCPKG_BUILD_TYPE}" STREQUAL "debug")
      z_vcpkg_configure_cmake_build_cmakecache(
        ninja_configure_contents "../../${TARGET_TRIPLET}-dbg" "dbg")
    endif()

    file(MAKE_DIRECTORY "${build_dir_release}/vcpkg-parallel-configure")
    file(WRITE "${build_dir_release}/vcpkg-parallel-configure/build.ninja"
    ...
    vcpkg_execute_required_process(
      ...
      -v
      WORKING_DIRECTORY
      "${build_dir_release}/vcpkg-parallel-configure"
      LOGNAME
      "${arg_LOGFILE_BASE}"
      SAVE_LOG_FILES
      ../../${TARGET_TRIPLET}-dbg/CMakeCache.txt
      ../CMakeCache.txt)
  ...
  foreach(config_log IN LISTS config_logs)
    ...
    string(STRIP "${CMAKE_MATCH_1}" unused_variables) # remove leading `    `
                                                      # and trailing `\n`
    ...
    foreach(unused_variable IN LISTS unused_variables)
      if(unused_variable IN_LIST manually_specified_variables)
        ...
        debug_message(
          "unused variable (not manually specified): ${unused_variable}")
      endif()
    endforeach()
  endforeach()
...
If these options should still be passed for whatever reason, please use the `MAYBE_UNUSED_VARIABLES` argument."
  ...
  if(NOT arg_Z_CMAKE_GET_VARS_USAGE)
    set(Z_VCPKG_CMAKE_GENERATOR
        "${generator}"
        CACHE INTERNAL "The generator which was used to configure CMake.")
  endif()
endfunction()
