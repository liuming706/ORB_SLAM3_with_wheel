vcpkg_from_github(
  ...
  xianyi/OpenBLAS
  REF
  b89fb708caa5a5a32de8f4306c4ff132e0228e9a # v0.3.21
  SHA512
...
elseif(NOT (VCPKG_TARGET_IS_WINDOWS AND NOT VCPKG_TARGET_IS_MINGW))
  string(APPEND VCPKG_C_FLAGS " -DNEEDBUNDERSCORE") # Required to get common
                                                    # BLASFUNC to append extra _
  string(APPEND VCPKG_CXX_FLAGS " -DNEEDBUNDERSCORE")
  list(
    APPEND OPENBLAS_EXTRA_OPTIONS -DNOFORTRAN=ON -DBU=_ # required for all blas
                                                        # functions to append
                                                        # extra _ using NAME
  )
endif()
...
if(EXISTS
   "${CURRENT_PACKAGES_DIR}/bin/getarch_2nd${VCPKG_HOST_EXECUTABLE_SUFFIX}")
...
if(EXISTS "${pcfile}")
  ...
  # file(CREATE_LINK "${pcfile}" "${CURRENT_PACKAGES_DIR}/lib/pkgconfig/blas.pc"
  # COPY_ON_ERROR)
endif()
...
# maybe we need also to write a wrapper inside share/blas to search implicitly
# for openblas, whenever we feel it's ready for its own -config.cmake file

# openblas do not make the config file , so I manually made this but I think in
# most case, libraries will not include these files, they define their own used
# function prototypes this is only to quite vcpkg
file(COPY "${CMAKE_CURRENT_LIST_DIR}/openblas_common.h"
     DESTINATION "${CURRENT_PACKAGES_DIR}/include")

vcpkg_replace_string("${SOURCE_PATH}/cblas.h" "#include \"common.h\""
