function(ignition_modular_build_library)
  ...
  set(oneValueArgs NAME MAJOR_VERSION SOURCE_PATH CMAKE_PACKAGE_NAME
                   DEFAULT_CMAKE_PACKAGE_NAME)
  set(multiValueArgs OPTIONS)
  cmake_parse_arguments(IML "${options}" "${oneValueArgs}" "${multiValueArgs}"
                        ${ARGN})
  ...
  if(EXISTS "${CURRENT_PACKAGES_DIR}/lib/cmake")
    # Some ignition libraries install library subcomponents, that are
    # effectively additional cmake packages with name
    # ${IML_CMAKE_PACKAGE_NAME}-${COMPONENT_NAME}, so it is needed to call
    # vcpkg_fixup_cmake_targets for them as well
    file(
      GLOB COMPONENTS_CMAKE_PACKAGE_NAMES
      LIST_DIRECTORIES TRUE
      RELATIVE "${CURRENT_PACKAGES_DIR}/lib/cmake/"
      "${CURRENT_PACKAGES_DIR}/lib/cmake/*")

    foreach(COMPONENT_CMAKE_PACKAGE_NAME IN
            LISTS COMPONENTS_CMAKE_PACKAGE_NAMES)
      vcpkg_fixup_cmake_targets(
        CONFIG_PATH "lib/cmake/${COMPONENT_CMAKE_PACKAGE_NAME}" TARGET_PATH
        "share/${COMPONENT_CMAKE_PACKAGE_NAME}"
    ...
    file(COPY ${CMAKE_RELEASE_FILES}
         DESTINATION "${CURRENT_PACKAGES_DIR}/share/${IML_CMAKE_PACKAGE_NAME}/")
  endif()

  # Remove unused files files
  file(
    REMOVE_RECURSE ${CURRENT_PACKAGES_DIR}/lib/cmake
    ${CURRENT_PACKAGES_DIR}/debug/include
    ${CURRENT_PACKAGES_DIR}/debug/lib/cmake ${CURRENT_PACKAGES_DIR}/debug/share)
...
#
# Download and build a library from the Ignition Robotics project (
...
# ~~~
#
# ## Parameters: ### NAME The name of the specific ignition library, i.e.
# `cmake` for `ignition-cmake0`, `math` for `ignition-math4`.
#
# ### VERSION The complete version number.
#
# ### SHA512 The SHA512 hash that should match the downloaded  archive. This is
# forwarded to the `vcpkg_from_github` command.
#
# ### REF Reference to the tag of the desired release. This is forwarded to the
# `vcpkg_from_github` command. If not specified, defaults to
# `ignition-${NAME}${MAJOR_VERSION}_${VERSION}`.
#
# ### HEAD_REF Reference (tag) to the desired release. This is forwarded to the
# `vcpkg_from_github` command. If not specified, defaults to
# `ign-${NAME}${MAJOR_VERSION}`.
#
# ### PATCHES A list of patches to be applied to the extracted sources. This is
# forwarded to the `vcpkg_from_github` command.
#
# ### CMAKE_PACKAGE_NAME The name of the CMake package for the port. If not
# specified, defaults to `ignition-${NAME}${MAJOR_VERSION}`.
#
# ### DISABLE_PKGCONFIG_INSTALL If present, disable installation of .pc
