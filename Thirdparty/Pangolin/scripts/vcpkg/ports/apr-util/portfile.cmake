...
if(VCPKG_TARGET_IS_WINDOWS)
  ...
  # endif
  # ~~~
  # When building, BUILD_SHARED_LIBS sets APU_DECLARE_STATIC to 0 and
  # APU_DECLARE_EXPORT to 1 Not BUILD_SHARED_LIBS sets APU_DECLARE_STATIC to 1
  # and APU_DECLARE_EXPORT to 0 When consuming APU_DECLARE_EXPORT is always 0
  # (assumed), so we need only embed the static or not setting into the
  # resulting headers:
  if(VCPKG_LIBRARY_LINKAGE STREQUAL dynamic)
    vcpkg_replace_string("${CURRENT_PACKAGES_DIR}/include/apu.h"
                         "defined(APU_DECLARE_STATIC)" "0")
  else()
    vcpkg_replace_string("${CURRENT_PACKAGES_DIR}/include/apu.h"
                         "defined(APU_DECLARE_STATIC)" "1")
  endif()
else()
  vcpkg_extract_source_archive(SOURCE_PATH ARCHIVE "${ARCHIVE}")

  if("crypto" IN_LIST FEATURES)
    set(CRYPTO_OPTIONS "--with-crypto=yes"
  ...
  if(CONFIGURE_PARAMETER_1)
    message(
      STATUS
        "Configuring apr-util with ${CONFIGURE_PARAMETER_1} ${CONFIGURE_PARAMETER_2} ${CONFIGURE_PARAMETER_3}"
    )
  else()
  ...
  vcpkg_replace_string("${CURRENT_PACKAGES_DIR}/tools/apr-util/bin/apu-1-config"
                       "${CURRENT_INSTALLED_DIR}" "`dirname $0`/../../..")
  vcpkg_replace_string("${CURRENT_PACKAGES_DIR}/tools/apr-util/bin/apu-1-config"
                       "${CURRENT_BUILDTREES_DIR}" "not/existing")
  if(NOT VCPKG_BUILD_TYPE)
    vcpkg_replace_string(
      "${CURRENT_PACKAGES_DIR}/tools/apr-util/debug/bin/apu-1-config"
      "${CURRENT_INSTALLED_DIR}" "`dirname $0`/../../../..")
    vcpkg_replace_string(
      "${CURRENT_PACKAGES_DIR}/tools/apr-util/debug/bin/apu-1-config"
      "${CURRENT_BUILDTREES_DIR}" "not/existing")
