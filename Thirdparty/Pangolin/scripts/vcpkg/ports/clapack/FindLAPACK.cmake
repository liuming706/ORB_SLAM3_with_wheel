...
#
# ``CLAPACK_FOUND`` True if clapack library found
#
# ``CLAPACK_VERSION`` Containing the clapack version tag (manually defined)
#
# ``CLAPACK_INCLUDE_DIR`` Location of clapack headers
#
# ``CLAPACK_LIBRARY`` List of libraries to link with when using clapack
#
# Result Targets
# ^^^^^^^^^^^^^^
#
...
#
# ``clapack::clapack`` Target to use clapack
#
...
#
# ``F2C_FOUND`` True if f2c (fortran-to-c wrap layer) library found
#
# ``F2C_INCLUDE_DIR`` Location of clapack headers
#
# ``F2C_LIBRARY`` Library containing the fortran-to-c wrap layer, necessary for
# clapack and automatically included when used
#
# ``LAPACK_FOUND`` True if clapack library found
#
# ``LAPACK_VERSION`` Containing the clapack version tag (manually defined)
#
# ``LAPACK_INCLUDE_DIR`` Location of clapack headers
#
# ``LAPACK_LIBRARY`` List of libraries to link with when using clapack
#
...
set(CLAPACK_INCLUDE_DIR
    "${LAPACK_INCLUDE_DIR}"
    CACHE PATH "" FORCE)
set(CLAPACK_INCLUDE_DIRS
    "${LAPACK_INCLUDE_DIR}"
    CACHE PATH "" FORCE)
set(F2C_INCLUDE_DIRS
    "${F2C_INCLUDE_DIR}"
    CACHE PATH "" FORCE)
...
if(WIN32)
  find_file(
    LAPACK_LIBRARY_RELEASE_DLL
    NAMES lapack.dll
    PATHS ${LAPACK_DLL_DIR})
  find_file(
    LAPACK_LIBRARY_DEBUG_FOLDER
    NAMES lapackd.dll
    PATHS ${LAPACK_DLL_DIR})
  find_file(
    F2C_LIBRARY_RELEASE_DLL
    NAMES f2c.dll libf2c.dll
    PATHS ${LAPACK_DLL_DIR})
  find_file(
    F2C_LIBRARY_DEBUG_DLL
    NAMES f2cd.dll libf2cd.dll
    PATHS ${LAPACK_DLL_DIR})
endif()

set(LAPACK_BLAS_LIBRARY
    "${LAPACK_BLAS_LIBRARY}"
    CACHE STRING "" FORCE)
set(F2C_LIBRARIES
    "${F2C_LIBRARY}"
...
set(F2C_LIBRARY
    "${F2C_LIBRARY}"
    CACHE STRING "" FORCE)
set(LAPACK_LIBRARY_RELEASE
    "${LAPACK_LIBRARY_RELEASE}"
    CACHE STRING "" FORCE)
set(LAPACK_LIBRARY_DEBUG
    "${LAPACK_LIBRARY_DEBUG}"
    CACHE STRING "" FORCE)
set(F2C_LIBRARY_RELEASE
    "${F2C_LIBRARY_RELEASE}"
    CACHE STRING "" FORCE)
set(F2C_LIBRARY_DEBUG
    "${F2C_LIBRARY_DEBUG}"
    CACHE STRING "" FORCE)
  ...
  if(EXISTS "${LAPACK_LIBRARY_RELEASE_DLL}")
    ...
    set_target_properties(
      ...
      PROPERTIES
        ...
        "$<$<NOT:$<CONFIG:DEBUG>>:${oF2C_LIBRARY_RELEASE}>;$<$<CONFIG:DEBUG>:${oF2C_LIBRARY_DEBUG}>;$<$<NOT:$<CONFIG:DEBUG>>:${LAPACK_BLAS_LIBRARY_RELEASE}>;$<$<CONFIG:DEBUG>:${LAPACK_BLAS_LIBRARY_DEBUG}>;$<LINK_ONLY:${ADDITIONAL_LAPACK_LIBRARY}>;$<LINK_ONLY:${PTHREAD_LINK_NAME}>"
        IMPORTED_CONFIGURATIONS Release
        IMPORTED_LINK_INTERFACE_LANGUAGES "C")
    if(EXISTS "${LAPACK_LIBRARY_DEBUG_DLL}")
      set_property(
        TARGET clapack::clapack
        APPEND
        PROPERTY IMPORTED_CONFIGURATIONS Debug)
      set_target_properties(
        clapack::clapack
        PROPERTIES IMPORTED_LOCATION_DEBUG "${LAPACK_LIBRARY_DEBUG_DLL}"
    ...
    if(EXISTS "${LAPACK_LIBRARY_DEBUG}")
      set_property(
        ...
        PROPERTY IMPORTED_CONFIGURATIONS Debug)
      set_target_properties(
        clapack::clapack PROPERTIES IMPORTED_LOCATION_DEBUG
                                    "${oLAPACK_LIBRARY_DEBUG}")
    endif()
  endif()
endif()

# Ensure consistency with both CMake's vanilla as well as lapack-reference's
# FindLAPACK.cmake module and register the LAPACK::LAPACK target
  ...
  if(EXISTS "${LAPACK_LIBRARY_RELEASE_DLL}")
    ...
    if(EXISTS "${LAPACK_LIBRARY_DEBUG}")
      set_property(
        ...
        PROPERTY IMPORTED_CONFIGURATIONS Debug)
      set_target_properties(
        LAPACK::LAPACK PROPERTIES IMPORTED_LOCATION_DEBUG
                                  "${oLAPACK_LIBRARY_DEBUG}")
    endif()
  endif()
endif()
