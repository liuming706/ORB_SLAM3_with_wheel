...
function(vcpkg_cmake_config_fixup)
  cmake_parse_arguments(
    PARSE_ARGV 0 "arg" "DO_NOT_DELETE_PARENT_CONFIG_PATH;NO_PREFIX_CORRECTION"
    "PACKAGE_NAME;CONFIG_PATH;TOOLS_PATH" "")

  if(DEFINED arg_UNPARSED_ARGUMENTS)
    message(
      FATAL_ERROR
        "vcpkg_cmake_config_fixup was passed extra arguments: ${arg_UNPARSED_ARGUMENTS}"
    )
  endif()
  ...
  string(REPLACE "." "\\." EXECUTABLE_SUFFIX
  ...
  if(NOT arg_CONFIG_PATH STREQUAL "share/${arg_PACKAGE_NAME}")
    if(arg_CONFIG_PATH STREQUAL "share")
      ...
      file(RENAME "${CURRENT_PACKAGES_DIR}/debug/share"
           "${CURRENT_PACKAGES_DIR}/debug/${arg_CONFIG_PATH}")
      file(RENAME "${CURRENT_PACKAGES_DIR}/share"
           "${CURRENT_PACKAGES_DIR}/${arg_CONFIG_PATH}")
    endif()
    ...
    if(NOT DEFINED VCPKG_BUILD_TYPE OR VCPKG_BUILD_TYPE STREQUAL "debug")
      ...
      if(debug_config_dir_name STREQUAL "cmake"
         AND NOT arg_DO_NOT_DELETE_PARENT_CONFIG_PATH)
        file(REMOVE_RECURSE "${debug_config}")
      else()
        get_filename_component(debug_config_parent_dir "${debug_config}"
                               DIRECTORY)
        get_filename_component(debug_config_dir_name
                               "${debug_config_parent_dir}" NAME)
        string(TOLOWER "${debug_config_dir_name}" debug_config_dir_name)
        if(debug_config_dir_name STREQUAL "cmake"
           AND NOT arg_DO_NOT_DELETE_PARENT_CONFIG_PATH)
    ...
    if(release_config_dir_name STREQUAL "cmake"
      ...
      get_filename_component(release_config_parent_dir "${release_config}"
                             DIRECTORY)
      get_filename_component(release_config_dir_name
                             "${release_config_parent_dir}" NAME)
      string(TOLOWER "${release_config_dir_name}" release_config_dir_name)
      if(release_config_dir_name STREQUAL "cmake"
         AND NOT arg_DO_NOT_DELETE_PARENT_CONFIG_PATH)
  ...
  file(GLOB_RECURSE release_targets "${release_share}/*-release.cmake")
  foreach(release_target IN LISTS release_targets)
    file(READ "${release_target}" contents)
    string(REPLACE "${CURRENT_INSTALLED_DIR}" "\${_IMPORT_PREFIX}" contents
                   "${contents}")
    string(
      REGEX
      REPLACE "\\\${_IMPORT_PREFIX}/bin/([^ \"]+${EXECUTABLE_SUFFIX})"
              "\${_IMPORT_PREFIX}/${arg_TOOLS_PATH}/\\1" contents "${contents}")
    file(WRITE "${release_target}" "${contents}")
  endforeach()

  if(NOT DEFINED VCPKG_BUILD_TYPE OR VCPKG_BUILD_TYPE STREQUAL "debug")
    file(GLOB_RECURSE debug_targets "${debug_share}/*-debug.cmake")
    foreach(debug_target IN LISTS debug_targets)
      ...
      string(
        REGEX
        REPLACE "\\\${_IMPORT_PREFIX}/bin/([^ \";]+${EXECUTABLE_SUFFIX})"
                "\${_IMPORT_PREFIX}/${arg_TOOLS_PATH}/\\1" contents
                "${contents}")
  ...
  # Fix ${_IMPORT_PREFIX} and absolute paths in cmake generated targets and
  # configs; Since those can be renamed we have to check in every *.cmake, but
  # only once.
  ...
  set(Z_VCPKG_CMAKE_CONFIG_ALREADY_FIXED_UP
      "${Z_VCPKG_CMAKE_CONFIG_ALREADY_FIXED_UP}"
      CACHE INTERNAL "")

  foreach(main_cmake IN LISTS main_cmakes)
    file(READ "${main_cmake}" contents)
    # Note: I think the following comment is no longer true, since we now
    # require the path to be `share/blah` however, I don't know it for sure. -
    # nimazzuc

    # This correction is not correct for all cases. To make it correct for all
    # cases it needs to consider original folder deepness to
    # CURRENT_PACKAGES_DIR in comparison to the moved to folder deepness which
    # is always at least (>=) 2, e.g. share/${PORT}. Currently the code assumes
    # it is always 2 although this requirement is only true for the
    # *Config.cmake. The targets are not required to be in the same folder as
    # the *Config.cmake!
...
get_filename_component(_IMPORT_PREFIX "${_IMPORT_PREFIX}" PATH)]]
          contents
          "${contents}") # see #1044 for details why this replacement is
                         # necessary. See #4782 why it must be a regex.
      string(
        REGEX
        REPLACE
      ...
      string(
        ...
        REPLACE
          ...
          [[get_filename_component(PACKAGE_PREFIX_DIR "${CMAKE_CURRENT_LIST_DIR}/../../" ABSOLUTE)]]
          contents
          "${contents}") # This is a meson-related workaround, see
                         # https://github.com/mesonbuild/meson/issues/6955
    endif()

    # Merge release and debug configurations of target property
    # INTERFACE_LINK_LIBRARIES.
    string(REPLACE "${release_share}/" "${debug_share}/" debug_cmake
                   "${main_cmake}")
    if(DEFINED VCPKG_BUILD_TYPE)
      # Skip. Warning: A release-only port in a dual-config installation may
      # pull release dependencies into the debug configuration.
    ...
    elseif(NOT EXISTS "${debug_cmake}")
      message(
        SEND_ERROR "Did not find a debug import file matching '${main_cmake}'")
    else()
      ...
      while(contents MATCHES
        ...
        if(NOT debug_contents MATCHES
           "set_target_properties\\(${target} PROPERTIES[^)]*\\)")
          message(
            SEND_ERROR
              "Did not find a debug configuration for target '${target}'.")
        endif()
        set(debug_command "${CMAKE_MATCH_0}")
        string(REGEX MATCH "  INTERFACE_LINK_LIBRARIES \"([^\"]*)\""
                     release_line "${matched_command}")
        set(release_libs "${CMAKE_MATCH_1}")
        string(REGEX MATCH "  INTERFACE_LINK_LIBRARIES \"([^\"]*)\"" debug_line
                     "${debug_command}")
        set(debug_libs "${CMAKE_MATCH_1}")
        z_vcpkg_cmake_config_fixup_merge(merged_libs release_libs debug_libs)
        string(REPLACE "${release_line}"
                       "  INTERFACE_LINK_LIBRARIES \"${merged_libs}\""
                       updated_command "${matched_command}")
        string(REPLACE "set_target_properties" "set_target_properties::done"
                       updated_command "${updated_command}") # Prevend 2nd match
        string(REPLACE "${matched_command}" "${updated_command}" contents
                       "${contents}")
      endwhile()
      string(REPLACE "set_target_properties::done" "set_target_properties"
                     contents "${contents}") # Restore original command
    endif()

    # Fix absolute paths to installed dir with ones relative to
    # ${CMAKE_CURRENT_LIST_DIR} This happens if vcpkg built libraries are
    # directly linked to a target instead of using an imported target.
    string(REPLACE "${CURRENT_INSTALLED_DIR}" [[${VCPKG_IMPORT_PREFIX}]]
                   contents "${contents}")
    file(TO_CMAKE_PATH "${CURRENT_PACKAGES_DIR}" cmake_current_packages_dir)
    string(REPLACE "${cmake_current_packages_dir}" [[${VCPKG_IMPORT_PREFIX}]]
                   contents "${contents}")
    ...
    if(NOT index STREQUAL "-1")
      ...
      file(RELATIVE_PATH relative "${main_cmake_dir}"
  ...
  file(
    GLOB_RECURSE unused_files "${debug_share}/*[Tt]argets.cmake"
    "${debug_share}/*[Cc]onfig.cmake" "${debug_share}/*[Cc]onfigVersion.cmake"
...
# Merges link interface library lists for release and debug into a single
# expression which use generator expression as necessary.
function(z_vcpkg_cmake_config_fixup_merge out_var release_var debug_var)
  set(release_libs "VCPKG;${${release_var}}")
  string(REGEX REPLACE ";optimized;([^;]*)" ";\\1" release_libs
                       "${release_libs}")
  ...
  list(TRANSFORM release_libs
       REPLACE [[^\\[$]<\\[$]<NOT:\\[$]<CONFIG:DEBUG>>:(.*)>$]] "\\1")
