...
vcpkg_download_distfile(
  ...
  FILENAME
  "msmpisdk-${MSMPI_VERSION}-b0087dfd.msi"
  SHA512
  b0087dfd21423bf87b94b17d7cb03576838585371bbf8b03cca95c3ad73670108c7bc6517b0de852ef595072cc4143be2011636e7242bcb080394d94294848a7
)

# to enable CI, you should modify the following URL also in
...
# We always want the ProgramFiles folder even on a 64-bit machine (not the
# ProgramFilesx86 folder)
vcpkg_get_program_files_platform_bitness(PROGRAM_FILES_PLATFORM_BITNESS)
set(SYSTEM_MPIEXEC_FILEPATH
    "${PROGRAM_FILES_PLATFORM_BITNESS}/Microsoft MPI/Bin/mpiexec.exe")

if(EXISTS "${SYSTEM_MPIEXEC_FILEPATH}")
  set(MPIEXEC_VERSION_LOGNAME "mpiexec-version")
  vcpkg_execute_required_process(
    COMMAND ${SYSTEM_MPIEXEC_FILEPATH} WORKING_DIRECTORY
    "${CURRENT_BUILDTREES_DIR}" LOGNAME ${MPIEXEC_VERSION_LOGNAME})
  file(READ "${CURRENT_BUILDTREES_DIR}/${MPIEXEC_VERSION_LOGNAME}-out.log"
       MPIEXEC_OUTPUT)
...
file(TO_NATIVE_PATH "${CURRENT_BUILDTREES_DIR}/msiexec-${TARGET_TRIPLET}.log"
     MSIEXEC_LOG_PATH)
...
# Write the command out to a script file and run that to avoid weird escaping
# behavior when spaces are present
file(WRITE ${SCRIPT_FILE}
     "msiexec ${PARAM_MSI} /qn ${PARAM_LOG} ${PARAM_TARGET_DIR}")

vcpkg_execute_required_process(COMMAND ${SCRIPT_FILE} WORKING_DIRECTORY
                               "${CURRENT_BUILDTREES_DIR}" LOGNAME extract-sdk)
...
# NOTE: since the binary distribution does not include any debug libraries we
# always install the release libraries
...
file(INSTALL "${SOURCE_LIB_PATH}/${TRIPLET_SYSTEM_ARCH}/msmpi.lib"
...
# Handle copyright
file(
  COPY "${SOURCE_PATH}/sdk/PFiles/Microsoft SDKs/MPI/License/MicrosoftMPI-SDK-EULA.rtf"
  DESTINATION "${CURRENT_PACKAGES_DIR}/share/${PORT}")
file(COPY "${SOURCE_PATH}/sdk/PFiles/Microsoft SDKs/MPI/License/MPI-SDK-TPN.txt"
     DESTINATION "${CURRENT_PACKAGES_DIR}/share/${PORT}")
file(WRITE "${CURRENT_PACKAGES_DIR}/share/${PORT}/copyright"
     "See the accompanying MicrosoftMPI-SDK-EULA.rtf and MPI-SDK-TPN.txt")
