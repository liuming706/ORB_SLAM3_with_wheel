vcpkg_from_github(
  ...
  REF
  bb368e271e24f63078129283148ce83db6b9670a # V7.6.2
  SHA512
  500c7ff804eb6b202bef48e1be904fe43a3c0137e9a402affe128b3b75a1adbb20bfe383cee82503b13efc083a95eb97425f1afb1f66bae38543d29f871a91f9
  HEAD_REF
  master
  PATCHES
  fix-pdb-find.patch
...
# VTK option in opencascade not currently supported because only 6.1.0 is
# supported but vcpkg has >= 9.0

# We turn off BUILD_MODULE_Draw as it requires TCL 8.6 and TK 8.6 specifically
# which conflicts with vcpkg only having TCL 9.0 And pre-built ActiveTCL
# binaries are behind a marketing wall :( We use the Unix install layout for
# Windows as it matches vcpkg
...
file(GLOB files
     "${CURRENT_PACKAGES_DIR}/include/opencascade/[a-zA-Z0-9_]*\.[hgl]xx")
foreach(file_name IN LISTS files)
  file(READ "${file_name}" filedata)
  string(REGEX REPLACE "# *include \<([a-zA-Z0-9_]*\.[hgl]xx)\>"
                       "#include \"\\1\"" filedata "${filedata}")
  foreach(extra_header IN LISTS ADDITIONAL_HEADERS)
    string(REGEX REPLACE "# *include \<${extra_header}\>"
                         "#include \"${extra_header}\"" filedata "${filedata}")
  endforeach()
...
if(WIN32)
  ...
  file(RENAME "${CURRENT_PACKAGES_DIR}/debug/libd"
...
if(VCPKG_LIBRARY_LINKAGE STREQUAL dynamic)
  ...
  file(RENAME "${CURRENT_PACKAGES_DIR}/debug/bind"
...
file(
  INSTALL "${SOURCE_PATH}/OCCT_LGPL_EXCEPTION.txt"
