# For very old ports whose upstream do not properly set the minimum CMake
# version.
...
list(REMOVE_ITEM CMAKE_PROGRAM_PATH
     "${_VCPKG_INSTALLED_DIR}/${VCPKG_TARGET_TRIPLET}/tools/python3")
if(@PythonFinder_NO_OVERRIDE@)
  _find_package(${ARGS})
  return()
endif()

# CMake's FindPython's separation of concerns is very muddy. We only want to
# force vcpkg's Python if the consumer is using the development component. What
# we don't want to do is break detection of the system Python, which may have
# certain packages the user expects. But - if the user is embedding Python or
# using both the development and interpreter components, then we need the
# interpreter matching vcpkg's Python libraries. Note that the "Development"
# component implies both "Development.Module" and "Development.Embed"
...
if(_PythonFinder_WantLibs)
  ...
  # Don't set the public facing hint or the finder will be unable to detect the
  # debug library. Internally, it uses the same value with an underscore
  # prepended.
  find_library(
  ...
  find_library(
    ...
    NAMES "python@PYTHON_VERSION_MAJOR@@PYTHON_VERSION_MINOR@_d"
  ...
  if(@VCPKG_LIBRARY_LINKAGE@ STREQUAL static)
    ...
    # Python for Windows embeds the zlib module into the core, so we have to
    # link against it. This is a separate extension module on Unix-like
    # platforms.
    if(WIN32)
    ...
    if(APPLE)
      ...
      if(TARGET @PythonFinder_PREFIX@::Python)
        get_target_property(
          _PYTHON_INTERFACE_LIBS @PythonFinder_PREFIX@::Python
        ...
        list(
          APPEND _PYTHON_INTERFACE_LIBS Iconv::Iconv
          "$<IF:$<CONFIG:Debug>,${Intl_LIBRARY_DEBUG},${Intl_LIBRARY_RELEASE}>")
        set_property(
          TARGET @PythonFinder_PREFIX@::Python
          PROPERTY INTERFACE_LINK_LIBRARIES ${_PYTHON_INTERFACE_LIBS})
        unset(_PYTHON_INTERFACE_LIBS)
      endif()
      if(TARGET @PythonFinder_PREFIX@::Module)
        get_target_property(
          _PYTHON_INTERFACE_LIBS @PythonFinder_PREFIX@::Module
          INTERFACE_LINK_LIBRARIES)
        if(NOT _PYTHON_INTERFACE_LIBS)
          set(_PYTHON_INTERFACE_LIBS "")
        endif()
      ...
      if(DEFINED @PythonFinder_PREFIX@_LIBRARIES)
        list(APPEND @PythonFinder_PREFIX@_LIBRARIES "-framework CoreFoundation"
