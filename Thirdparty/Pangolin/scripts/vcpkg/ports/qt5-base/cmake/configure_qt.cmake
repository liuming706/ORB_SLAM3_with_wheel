function(configure_qt)
  cmake_parse_arguments(
    _csc "" "SOURCE_PATH;TARGET_PLATFORM;HOST_PLATFORM;HOST_TOOLS_ROOT"
    "OPTIONS;OPTIONS_DEBUG;OPTIONS_RELEASE" ${ARGN})
  ...
  if(DEFINED _csc_HOST_TOOLS_ROOT)
    ...
    message(
      STATUS
        "Building Qt with prepared host tools from ${_csc_HOST_TOOLS_ROOT}!")
    vcpkg_add_to_path("${_csc_HOST_TOOLS_ROOT}/bin")
    vcpkg_add_to_path("${_csc_HOST_TOOLS_ROOT}")
    set(EXT_BIN_DIR -external-hostbindir ${_csc_HOST_TOOLS_ROOT}/bin) # we only
                                                                      # use
                                                                      # release
                                                                      # binaries
                                                                      # for
    ...
    find_program(
      QMAKE_COMMAND
      NAMES qmake
      PATHS ${_csc_HOST_TOOLS_ROOT}/bin
      NO_DEFAULT_PATH)
  ...
  if(NOT VCPKG_TARGET_IS_WINDOWS OR VCPKG_DETECTED_CMAKE_AR MATCHES "ar$")
    # This is the reason why users should probably use a customized qt.conf with
  ...
  foreach(_buildname ${BUILDTYPES})
    set(PKGCONFIG_INSTALLED_DIR
        "${CURRENT_INSTALLED_DIR}${_path_suffix_${_buildname}}/lib/pkgconfig")
    set(PKGCONFIG_INSTALLED_SHARE_DIR
        "${CURRENT_INSTALLED_DIR}/share/pkgconfig")
    set(PKGCONFIG_PACKAGES_DIR
        "${CURRENT_PACKAGES_DIR}${_path_suffix_${_buildname}}/lib/pkgconfig")
    ...
    if(DEFINED ENV{PKG_CONFIG_PATH})
      ...
      set(ENV{PKG_CONFIG_PATH}
          "${PKGCONFIG_INSTALLED_DIR}${VCPKG_HOST_PATH_SEPARATOR}${PKGCONFIG_INSTALLED_SHARE_DIR}${VCPKG_HOST_PATH_SEPARATOR}${PKGCONFIG_PACKAGES_DIR}${VCPKG_HOST_PATH_SEPARATOR}${PKGCONFIG_PACKAGES_SHARE_DIR}${VCPKG_HOST_PATH_SEPARATOR}$ENV{PKG_CONFIG_PATH}"
      )
    else()
      set(ENV{PKG_CONFIG_PATH}
          "${PKGCONFIG_INSTALLED_DIR}${VCPKG_HOST_PATH_SEPARATOR}${PKGCONFIG_INSTALLED_SHARE_DIR}${VCPKG_HOST_PATH_SEPARATOR}${PKGCONFIG_PACKAGES_DIR}${VCPKG_HOST_PATH_SEPARATOR}${PKGCONFIG_PACKAGES_SHARE_DIR}"
      )
    endif()
    ...
    # Note sure about these. VCPKG_QMAKE_OPTIONS offers a way to opt out of
    # these. (earlier values being overwritten by later values; = set +=append
    # *=append unique -=remove)
    ...
    qmake_add_flags("QMAKE_CFLAGS_${_buildname}" "+="
                    "${VCPKG_COMBINED_C_FLAGS_${_buildname}}")
    qmake_add_flags("QMAKE_CXXFLAGS_${_buildname}" "+="
                    "${VCPKG_COMBINED_CXX_FLAGS_${_buildname}}")
    qmake_add_flags("QMAKE_LFLAGS" "+="
                    "${VCPKG_COMBINED_SHARED_LINKER_FLAGS_${_buildname}}")
    qmake_add_flags("QMAKE_LFLAGS_SHLIB" "+="
                    "${VCPKG_COMBINED_SHARED_LINKER_FLAGS_${_buildname}}")
    qmake_add_flags("QMAKE_LFLAGS_PLUGIN" "+="
                    "${VCPKG_COMBINED_MODULE_LINKER_FLAGS_${_buildname}}")
    qmake_add_flags("QMAKE_LIBFLAGS_${_buildname}" "+="
                    "${VCPKG_COMBINED_STATIC_LINKER_FLAGS_${_buildname}}")

    # These paths get hardcoded into qmake. So point them into the
    # CURRENT_INSTALLED_DIR instead of CURRENT_PACKAGES_DIR makefiles will be
    # fixed to install into CURRENT_PACKAGES_DIR in install_qt
    set(BUILD_OPTIONS
        ${_csc_OPTIONS}
        ...
        ${EXT_BIN_DIR}
        -hostprefix
        ${CURRENT_INSTALLED_DIR}/tools/qt5${_path_suffix_${_buildname}}
        # -hostprefix ${CURRENT_INSTALLED_DIR}/tools/qt5
        -hostlibdir
        ${CURRENT_INSTALLED_DIR}/tools/qt5${_path_suffix_${_buildname}}/lib # could
                                                                            # probably
                                                                            # be move to
                                                                            # manual-link
        -hostbindir
        ${CURRENT_INSTALLED_DIR}/tools/qt5${_path_suffix_${_buildname}}/bin
        # -hostbindir ${CURRENT_INSTALLED_DIR}/tools/qt5/bin Qt VS Plugin
        # requires a /bin subfolder with the executables in the root dir. But to
        # use the wizard a correctly setup lib folder is also required So with
        # the vcpkg layout there is no way to make it work unless all dll are
        # are copied to tools/qt5/bin and all libs to tools/qt5/lib
        -archdatadir
        ${CURRENT_INSTALLED_DIR}/tools/qt5${_path_suffix_${_buildname}}
        -datadir
        ${CURRENT_INSTALLED_DIR}${_path_suffix}/share/qt5${_path_suffix_${_buildname}}
        -plugindir
        ${CURRENT_INSTALLED_DIR}${_path_suffix_${_buildname}}/plugins
        -qmldir
        ${CURRENT_INSTALLED_DIR}${_path_suffix_${_buildname}}/qml
        -headerdir
        ${CURRENT_INSTALLED_DIR}/include/qt5
        -libexecdir
        ${CURRENT_INSTALLED_DIR}/tools/qt5${_path_suffix_${_buildname}}
        -bindir
        ${CURRENT_INSTALLED_DIR}${_path_suffix_${_buildname}}/bin
        -libdir
        ${CURRENT_INSTALLED_DIR}${_path_suffix_${_buildname}}/lib
        -I
        ...
        ${CURRENT_INSTALLED_DIR}${_path_suffix_${_buildname}}/lib
        -L
        ${CURRENT_INSTALLED_DIR}${_path_suffix_${_buildname}}/lib/manual-link
        -platform
        ${_csc_TARGET_PLATFORM})
    ...
    # Note archdatadir and datadir are required to be prefixed with the
    # hostprefix?
    ...
    file(TO_CMAKE_PATH "${CURRENT_PACKAGES_DIR}"
         CMAKE_CURRENT_PACKAGES_DIR_PATH)
    file(TO_CMAKE_PATH "${CURRENT_INSTALLED_DIR}"
         CMAKE_CURRENT_INSTALLED_DIR_PATH)
    file(READ "${CURRENT_BUILDTREES_DIR}/${_build_triplet}/bin/qt.conf"
         _contents)
    string(REPLACE "${CMAKE_CURRENT_PACKAGES_DIR_PATH}"
                   "\${CURRENT_INSTALLED_DIR}" _contents ${_contents})
    string(REPLACE "${CMAKE_CURRENT_INSTALLED_DIR_PATH}"
                   "\${CURRENT_INSTALLED_DIR}" _contents ${_contents})
    # string(REPLACE "HostPrefix=\${CURRENT_PACKAGES_DIR}"
    # "HostPrefix=\${CURRENT_INSTALLED_DIR}" _contents ${_contents})
    ...
    file(
      WRITE
      "${CURRENT_PACKAGES_DIR}/tools/qt5/qt_${_build_type_${_buildname}}.conf"
      "${_contents}")
  endforeach()
