...
function(install_qt)
  ...
  if(CMAKE_HOST_WIN32)
    ...
    file(COPY ${FLEX_EXE_PATH}
         DESTINATION "${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-tools")
    set(FLEX_TEMP
        "${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-tools/${FLEX_DIR}")
  ...
  foreach(_buildname ${BUILDTYPES})
    ...
    set(_installed_prefix_
        "${CURRENT_INSTALLED_DIR}${_path_suffix_${_buildname}}")
    set(_installed_libpath_
        "${_installed_prefix_}/lib/${VCPKG_HOST_PATH_SEPARATOR}${_installed_prefix_}/lib/manual-link/"
    )

    vcpkg_add_to_path(PREPEND "${_installed_prefix_}/bin")
    vcpkg_add_to_path(PREPEND "${_installed_prefix_}/lib")

    # We set LD_LIBRARY_PATH ENV variable to allow executing Qt tools (rcc,...)
    # even with dynamic linking
    if(CMAKE_HOST_UNIX)
      if(DEFINED ENV{LD_LIBRARY_PATH})
        set(_ld_library_path_defined_ TRUE)
        set(_ld_library_path_backup_ $ENV{LD_LIBRARY_PATH})
        set(ENV{LD_LIBRARY_PATH}
            "${_installed_libpath_}${VCPKG_HOST_PATH_SEPARATOR}${_ld_library_path_backup_}"
        )
      else()
    ...
    vcpkg_execute_build_process(
      ...
      ${INVOKE_SINGLE}
      WORKING_DIRECTORY
      ${CURRENT_BUILDTREES_DIR}/${_build_triplet}
      LOGNAME
    ...
    if(VCPKG_TARGET_IS_OSX)
      ...
      vcpkg_execute_required_process(
        ...
        clean
        WORKING_DIRECTORY
        ${CURRENT_BUILDTREES_DIR}/${_build_triplet}/qmake
        LOGNAME
        cleaning-after-build-${_build_triplet})
    endif()
