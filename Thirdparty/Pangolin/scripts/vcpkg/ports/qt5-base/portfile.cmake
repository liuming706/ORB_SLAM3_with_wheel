...
if(VCPKG_TARGET_IS_LINUX)
  message(
    WARNING
      "qt5-base currently requires some packages from the system package manager, see https://doc.qt.io/qt-5/linux-requirements.html"
  )
  message(
    WARNING
      [[
...
if(WITH_MYSQL_PLUGIN AND NOT VCPKG_TARGET_IS_WINDOWS)
  message(
    WARNING
      "${PORT} is currently not setup to support feature 'mysqlplugin' on platforms other than windows. Feel free to open up a PR to fix it!"
  )
endif()
...
qt_download_submodule(
  ...
  patches/qt_app.patch # Moves the target location of qt5 host apps to always
                       # install into the host dir.
  patches/gui_configure.patch # Patches the gui configure.json to break
                              # freetype/fontconfig autodetection because it
                              # does not include its dependencies.
  patches/icu.patch # Help configure find static icu builds in vcpkg on windows
  patches/xlib.patch # Patches Xlib check to actually use Pkgconfig instead of
                     # makeSpec only
  ...
  patches/cocoa.patch # Fix missing include on macOS Monterrey,
                      # https://code.qt.io/cgit/qt/qtbase.git/commit/src/plugins/platforms/cocoa?id=dece6f5840463ae2ddf927d65eb1b3680e34a547
  # patches/static_opengl.patch       #Use this patch if you really want to
  # statically link angle on windows (e.g. using -opengl es2 and -static). Be
  # carefull since it requires definining _GDI32_ for all dependent projects due
  # to redefinition errors in the the windows supplied gl.h header and the angle
  # gl.h otherwise. CMake fixes
  patches/Qt5BasicConfig.patch
  patches/Qt5PluginTarget.patch
  patches/create_cmake.patch
  patches/Qt5GuiConfigExtras.patch # Patches the library search behavior for EGL
                                   # since angle is not build with Qt
  patches/fix_angle.patch # Failed to create OpenGL context for format
                          # QSurfaceFormat ...
)
...
set(CORE_OPTIONS
    -confirm-license
    -opensource
    # -simulator_and_device -ltcg -combined-angle-lib ENV ANGLE_DIR to external
    # angle source dir. (Will always be compiled with Qt) -optimized-tools
    # -force-debug-info
    -verbose)
...
find_library(
  ...
  NAMES pq libpq
  PATHS "${CURRENT_INSTALLED_DIR}/lib"
  NO_DEFAULT_PATH) # Depends on openssl and zlib(linux)
find_library(
  PSQL_DEBUG
  NAMES pq libpq pqd libpqd
  PATHS "${CURRENT_INSTALLED_DIR}/debug/lib"
  NO_DEFAULT_PATH)
...
if(NOT (PSQL_RELEASE MATCHES ".*\.so") AND NOT (PSQL_DEBUG MATCHES ".*\.so"))
  ...
  find_library(
    PSQL_COMMON_DEBUG
    NAMES pgcommon libpgcommon pgcommond libpgcommond
    PATHS "${CURRENT_INSTALLED_DIR}/debug/lib"
    NO_DEFAULT_PATH)
  find_library(
    PSQL_PORT_RELEASE
  ...
  find_library(
    PSQL_PORT_DEBUG
    NAMES pgport libpgport pgportd libpgportd
    PATHS "${CURRENT_INSTALLED_DIR}/debug/lib"
    NO_DEFAULT_PATH)
endif()
find_library(
...
find_library(
  MYSQL_DEBUG
  NAMES libmysql libmysqld mysqlclient mysqlclientd
  PATHS "${CURRENT_INSTALLED_DIR}/debug/lib"
  NO_DEFAULT_PATH)
...
find_library(
  PCRE2_DEBUG
  NAMES pcre2-16 pcre2-16-static pcre2-16d pcre2-16-staticd
  PATHS "${CURRENT_INSTALLED_DIR}/debug/lib"
  NO_DEFAULT_PATH)
find_library(
  FREETYPE_RELEASE
  NAMES freetype
  PATHS "${CURRENT_INSTALLED_DIR}/lib"
  NO_DEFAULT_PATH) # zlib, bzip2, libpng
find_library(
  FREETYPE_DEBUG
  ...
  NO_DEFAULT_PATH)
find_library(
  DOUBLECONVERSION_RELEASE
  NAMES double-conversion
  PATHS "${CURRENT_INSTALLED_DIR}/lib"
  NO_DEFAULT_PATH)
find_library(
  DOUBLECONVERSION_DEBUG
  NAMES double-conversion
  PATHS "${CURRENT_INSTALLED_DIR}/debug/lib"
  NO_DEFAULT_PATH)
find_library(
...
find_library(
  ICUUC_DEBUG
  NAMES icuucd libicuucd icuuc libicuuc
  PATHS "${CURRENT_INSTALLED_DIR}/debug/lib"
  NO_DEFAULT_PATH)
find_library(
  ICUTU_RELEASE
...
find_library(
  ICUTU_DEBUG
  NAMES icutud libicutud icutu libicutu
  PATHS "${CURRENT_INSTALLED_DIR}/debug/lib"
  NO_DEFAULT_PATH)

# Was installed in WSL but not on CI machine find_library(ICULX_RELEASE NAMES
# iculx libiculx PATHS "${CURRENT_INSTALLED_DIR}/lib" NO_DEFAULT_PATH)
# find_library(ICULX_DEBUG NAMES iculxd libiculxd iculx libiculx PATHS
# "${CURRENT_INSTALLED_DIR}/debug/lib" NO_DEFAULT_PATH)
...
find_library(
  ICUIO_DEBUG
  NAMES icuiod libicuiod icuio libicuio
  PATHS "${CURRENT_INSTALLED_DIR}/debug/lib"
  NO_DEFAULT_PATH)
find_library(
  ICUIN_RELEASE
  NAMES icui18n libicui18n icuin
  PATHS "${CURRENT_INSTALLED_DIR}/lib"
  NO_DEFAULT_PATH)
find_library(
  ICUIN_DEBUG
  NAMES icui18nd libicui18nd icui18n libicui18n icuin icuind
  PATHS "${CURRENT_INSTALLED_DIR}/debug/lib"
  NO_DEFAULT_PATH)
find_library(
  ICUDATA_RELEASE
  NAMES icudata libicudata icudt
  PATHS "${CURRENT_INSTALLED_DIR}/lib"
  NO_DEFAULT_PATH)
find_library(
  ICUDATA_DEBUG
  NAMES icudatad libicudatad icudata libicudata icudtd
  PATHS "${CURRENT_INSTALLED_DIR}/debug/lib"
  NO_DEFAULT_PATH)
set(ICU_RELEASE
    "${ICUIN_RELEASE} ${ICUTU_RELEASE} ${ICULX_RELEASE} ${ICUUC_RELEASE} ${ICUIO_RELEASE} ${ICUDATA_RELEASE}"
)
set(ICU_DEBUG
    "${ICUIN_DEBUG} ${ICUTU_DEBUG} ${ICULX_DEBUG} ${ICUUC_DEBUG} ${ICUIO_DEBUG} ${ICUDATA_DEBUG}"
)
...
find_library(
  ...
  NO_DEFAULT_PATH)
find_library(
  SSL_RELEASE ssl ssleay32
  PATHS "${CURRENT_INSTALLED_DIR}/lib"
  NO_DEFAULT_PATH)
find_library(
  SSL_DEBUG ssl ssleay32 ssld ssleay32d
  PATHS "${CURRENT_INSTALLED_DIR}/debug/lib"
  NO_DEFAULT_PATH)
find_library(
  EAY_RELEASE libeay32 crypto libcrypto
  PATHS "${CURRENT_INSTALLED_DIR}/lib"
  NO_DEFAULT_PATH)
find_library(
  EAY_DEBUG
...
set(FREETYPE_RELEASE_ALL
    "${FREETYPE_RELEASE} ${BZ2_RELEASE} ${LIBPNG_RELEASE} ${ZLIB_RELEASE} ${BROTLI_DEC_RELEASE} ${BROTLI_COMMON_RELEASE}"
)
set(FREETYPE_DEBUG_ALL
    "${FREETYPE_DEBUG} ${BZ2_DEBUG} ${LIBPNG_DEBUG} ${ZLIB_DEBUG} ${BROTLI_DEC_DEBUG} ${BROTLI_COMMON_DEBUG}"
)
...
if(VCPKG_TARGET_IS_WINDOWS)
  ...
  if(NOT ${VCPKG_LIBRARY_LINKAGE} STREQUAL "static")
    list(APPEND CORE_OPTIONS -opengl dynamic) # other options are "-no-opengl",
                                              # "-opengl angle", and "-opengl
                                              # desktop" and "-opengel es2"
  else()
    list(APPEND CORE_OPTIONS -opengl dynamic) # other possible option without
                                              # moving angle dlls: "-opengl
                                              # desktop". "-opengel es2" only
                                              # works with commented patch
  endif()
  list(
    APPEND
    RELEASE_OPTIONS
  ...
  if(WITH_MYSQL_PLUGIN)
    list(
      APPEND
      RELEASE_OPTIONS
      "MYSQL_LIBS=${MYSQL_RELEASE} ${SSL_RELEASE} ${EAY_RELEASE} ${ZLIB_RELEASE} ws2_32.lib secur32.lib advapi32.lib shell32.lib crypt32.lib user32.lib gdi32.lib"
    )
    list(
      APPEND
      DEBUG_OPTIONS
      "MYSQL_LIBS=${MYSQL_DEBUG} ${SSL_DEBUG} ${EAY_DEBUG} ${ZLIB_DEBUG} ws2_32.lib secur32.lib advapi32.lib shell32.lib crypt32.lib user32.lib gdi32.lib"
    )
...
elseif(VCPKG_TARGET_IS_OSX)
  ...
  if("${VCPKG_TARGET_ARCHITECTURE}" MATCHES "arm64")
    ...
    string(REPLACE "QMAKE_APPLE_DEVICE_ARCHS = x86_64"
                   "QMAKE_APPLE_DEVICE_ARCHS = arm64" _tmp_contents
                   ${_tmp_contents})
  ...
  if(DEFINED VCPKG_OSX_DEPLOYMENT_TARGET)
    ...
    file(STRINGS "${CURRENT_BUILDTREES_DIR}/OSX_SDK_VER.txt" OSX_SDK_VERSION
         REGEX "^[0-9][0-9]\.[0-9][0-9]*")
    message(STATUS "Detected OSX SDK Version: ${OSX_SDK_VERSION}")
    string(REGEX MATCH "^[0-9][0-9]\.[0-9][0-9]*" OSX_SDK_VERSION
                 ${OSX_SDK_VERSION})
    ...
    execute_process(
      ...
      WORKING_DIRECTORY ${CURRENT_BUILDTREES_DIR})
    file(STRINGS "${CURRENT_BUILDTREES_DIR}/OSX_SYS_VER.txt"
         VCPKG_OSX_DEPLOYMENT_TARGET REGEX "^[0-9][0-9]\.[0-9][0-9]*")
    message(
      STATUS "Detected OSX system Version: ${VCPKG_OSX_DEPLOYMENT_TARGET}")
    string(REGEX MATCH "^[0-9][0-9]\.[0-9][0-9]*" VCPKG_OSX_DEPLOYMENT_TARGET
                 ${VCPKG_OSX_DEPLOYMENT_TARGET})
    message(
      STATUS "Major.Minor OSX system Version: ${VCPKG_OSX_DEPLOYMENT_TARGET}")

    set(ENV{QMAKE_MACOSX_DEPLOYMENT_TARGET} ${VCPKG_OSX_DEPLOYMENT_TARGET})
    if(${VCPKG_OSX_DEPLOYMENT_TARGET} GREATER "10.15"
    )# Max Version supported by QT. This version is defined in
     # mkspecs/common/macx.conf as QT_MAC_SDK_VERSION_MAX
      message(
        STATUS
          "Qt ${QT_MAJOR_MINOR_VER}.${QT_PATCH_VER} only support OSX_DEPLOYMENT_TARGET up to 10.15"
      )
      set(VCPKG_OSX_DEPLOYMENT_TARGET "10.15")
    endif()
    set(ENV{QMAKE_MACOSX_DEPLOYMENT_TARGET} ${VCPKG_OSX_DEPLOYMENT_TARGET})
    message(
      STATUS "Enviromnent OSX SDK Version: $ENV{QMAKE_MACOSX_DEPLOYMENT_TARGET}"
    )
    file(READ "${SOURCE_PATH}/mkspecs/common/macx.conf" _tmp_contents)
    string(
      REPLACE "QMAKE_MACOSX_DEPLOYMENT_TARGET = 10.13"
...
list(APPEND CORE_OPTIONS -nomake examples -nomake tests)
...
if(QT_UPDATE_VERSION)
  ...
  # ############################################################################
  # TODO: Make this a function since it is also done by modular scripts! e.g. by
  # patching mkspecs/features/qt_tools.prf somehow
  file(
    GLOB_RECURSE
    PRL_FILES
    "${CURRENT_PACKAGES_DIR}/lib/*.prl"
    "${CURRENT_PACKAGES_DIR}/tools/qt5/lib/*.prl"
    "${CURRENT_PACKAGES_DIR}/tools/qt5/mkspecs/*.pri"
    "${CURRENT_PACKAGES_DIR}/debug/lib/*.prl"
    "${CURRENT_PACKAGES_DIR}/tools/qt5/debug/lib/*.prl"
    "${CURRENT_PACKAGES_DIR}/tools/qt5/debug/mkspecs/*.pri")
  ...
  if(NOT DEFINED VCPKG_BUILD_TYPE OR VCPKG_BUILD_TYPE STREQUAL "release")
    ...
    file(COPY ${CMAKE_CURRENT_LIST_DIR}/qtdeploy.ps1
         DESTINATION ${CURRENT_PACKAGES_DIR}/plugins)
  endif()

  if(NOT DEFINED VCPKG_BUILD_TYPE OR VCPKG_BUILD_TYPE STREQUAL "debug")
    qt_fix_prl("${CURRENT_INSTALLED_DIR}/debug" "${PRL_FILES}")
    file(COPY ${CMAKE_CURRENT_LIST_DIR}/qtdeploy.ps1
         DESTINATION ${CURRENT_PACKAGES_DIR}/debug/plugins)
  endif()

  file(MAKE_DIRECTORY ${CURRENT_PACKAGES_DIR}/share)
  file(RENAME ${CURRENT_PACKAGES_DIR}/lib/cmake
       ${CURRENT_PACKAGES_DIR}/share/cmake)
  file(REMOVE_RECURSE ${CURRENT_PACKAGES_DIR}/debug/lib/cmake) # TODO: check if
                                                               # important debug
                                                               # information for
                                                               # cmake is lost

  # This needs a new VCPKG policy or a static angle build (ANGLE needs to be
  # fixed in VCPKG!)
  if(VCPKG_TARGET_IS_WINDOWS AND ${VCPKG_LIBRARY_LINKAGE} MATCHES "static"
  )# Move angle dll libraries
    if(EXISTS "${CURRENT_PACKAGES_DIR}/bin")
      message(
        STATUS
          "Moving ANGLE dlls from /bin to /tools/qt5-angle/bin. In static builds dlls are not allowed in /bin"
      )
      file(MAKE_DIRECTORY ${CURRENT_PACKAGES_DIR}/tools/qt5-angle)
      file(RENAME ${CURRENT_PACKAGES_DIR}/bin
           ${CURRENT_PACKAGES_DIR}/tools/qt5-angle/bin)
      if(EXISTS ${CURRENT_PACKAGES_DIR}/debug/bin)
        file(MAKE_DIRECTORY ${CURRENT_PACKAGES_DIR}/tools/qt5-angle/debug)
        file(RENAME ${CURRENT_PACKAGES_DIR}/debug/bin
             ${CURRENT_PACKAGES_DIR}/tools/qt5-angle/debug/bin)
      endif()
    endif()
  endif()

  # Fix location of qtmain(d).lib. Has been moved into manual-link. Add debug
  # version
  set(cmakefile
      "${CURRENT_PACKAGES_DIR}/share/cmake/Qt5Core/Qt5CoreConfigExtras.cmake")
  ...
  if(EXISTS ${CURRENT_PACKAGES_DIR}/tools/qt5/bin)
    file(COPY ${CURRENT_PACKAGES_DIR}/tools/qt5/bin
         DESTINATION ${CURRENT_PACKAGES_DIR}/tools/${PORT})
    vcpkg_copy_tool_dependencies(${CURRENT_PACKAGES_DIR}/tools/${PORT}/bin)
    vcpkg_copy_tool_dependencies(${CURRENT_PACKAGES_DIR}/tools/qt5/bin)
  endif()
  # This should be removed if possible! (Currently debug build of
  # qt5-translations requires it.)
  if(EXISTS ${CURRENT_PACKAGES_DIR}/debug/tools/qt5/bin)
    file(COPY ${CURRENT_PACKAGES_DIR}/tools/qt5/bin
         DESTINATION ${CURRENT_PACKAGES_DIR}/tools/qt5/debug)
  ...
  set(CURRENT_INSTALLED_DIR "./../../..") # Making the qt.conf relative and not
                                          # absolute
  configure_file(
    ${CURRENT_PACKAGES_DIR}/tools/qt5/qt_release.conf
    ${CURRENT_PACKAGES_DIR}/tools/qt5/bin/qt.conf) # This makes the tools at
                                                   # least useable for release
...
file(
  COPY ${CMAKE_CURRENT_LIST_DIR}/cmake/qt_port_hashes.cmake
  ...
  DESTINATION ${CURRENT_PACKAGES_DIR}/share/qt5)

# Fix Qt5GuiConfigExtras EGL path
if(VCPKG_TARGET_IS_LINUX)
  set(_file
      "${CURRENT_PACKAGES_DIR}/share/cmake/Qt5Gui/Qt5GuiConfigExtras.cmake")
  file(READ "${_file}" _contents)
  string(
    REGEX
    REPLACE
      "_qt5gui_find_extra_libs\\\(EGL[^\\\n]+"
...
if(VCPKG_TARGET_IS_OSX)
  file(GLOB _debug_files
       "${CURRENT_PACKAGES_DIR}/debug/lib/pkgconfig/*_debug.pc")
...
# #Code to get generated CMake files from CI file(RENAME
# "${CURRENT_PACKAGES_DIR}/share/cmake/Qt5Core/Qt5CoreConfig.cmake"
# "${CURRENT_BUILDTREES_DIR}/Qt5CoreConfig.cmake.log") file(GLOB_RECURSE
# CMAKE_GUI_FILES "${CURRENT_PACKAGES_DIR}/share/cmake/Qt5Gui/*.cmake" )
# foreach(cmake_file ${CMAKE_GUI_FILES}) get_filename_component(cmake_filename
# "${cmake_file}" NAME) file(COPY "${cmake_file}" DESTINATION
# "${CURRENT_BUILDTREES_DIR}") file(RENAME
# "${CURRENT_BUILDTREES_DIR}/${cmake_filename}"
# "${CURRENT_BUILDTREES_DIR}/${cmake_filename}.log") endforeach() #Copy
# config.log from buildtree/triplet to buildtree to get the log in CI in case of
# failure if(EXISTS
# "${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-rel/config.log") file(RENAME
# "${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-rel/config.log"
# "${CURRENT_BUILDTREES_DIR}/config-rel.log") endif() if(EXISTS
# "${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-dbg/config.log") file(RENAME
# "${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-dbg/config.log"
# "${CURRENT_BUILDTREES_DIR}/config-dbg.log") endif() message(FATAL_ERROR "Need
# Info from CI!")
