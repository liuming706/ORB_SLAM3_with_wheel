...
vcpkg_download_distfile(
  ARCHIVE
  URLS
  "https://ftp.postgresql.org/pub/source/v${PORT_VERSION}/postgresql-${PORT_VERSION}.tar.bz2"
  FILENAME
  "postgresql-${PORT_VERSION}.tar.bz2"
  SHA512
  dd2f80248684e331d2ffb1e26cd2a285df1fb18710807a0c31aedabf917912ce9267f8ca26318e5371d916c6fe476f8a17886d82d3ff86a974e6f24c19a6aafb
)
...
if(VCPKG_TARGET_ARCHITECTURE MATCHES "arm")
  ...
  list(APPEND PATCHES patches/windows/host_skip_openssl.patch) # Skip
                                                               # openssl.exe
                                                               # version check
                                                               # since it cannot
                                                               # be executed by
...
vcpkg_extract_source_archive_ex(OUT_SOURCE_PATH SOURCE_PATH ARCHIVE
                                "${ARCHIVE}" PATCHES ${PATCHES})
...
# Setup build types
if(NOT DEFINED VCPKG_BUILD_TYPE OR VCPKG_BUILD_TYPE MATCHES
                                   "[Rr][Ee][Ll][Ee][Aa][Ss][Ee]")
  set(_buildtype RELEASE)
  set(_short rel)
  list(APPEND port_config_list ${_buildtype})
  set(INSTALL_PATH_SUFFIX_${_buildtype} "")
  set(BUILDPATH_${_buildtype}
      "${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-${_short}")
...
if(NOT DEFINED VCPKG_BUILD_TYPE OR VCPKG_BUILD_TYPE MATCHES
  ...
  set(BUILDPATH_${_buildtype}
      "${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-${_short}")
  file(REMOVE_RECURSE "${BUILDPATH_${_buildtype}}") # Clean old builds
  set(PACKAGE_DIR_${_buildtype}
      "${CURRENT_PACKAGES_DIR}${INSTALL_PATH_SUFFIX_${_buildtype}}")
  unset(_short)
  unset(_buildtype)
endif()
...
if(VCPKG_TARGET_IS_WINDOWS AND NOT VCPKG_TARGET_IS_MINGW)
  ...
  foreach(_buildtype ${port_config_list})
    # Copy libpq sources.
    message(
      STATUS "Copying libpq source files to ${BUILDPATH_${_buildtype}}...")
    ...
    vcpkg_apply_patches(
      SOURCE_PATH "${BUILDPATH_${_buildtype}}" PATCHES
      patches/windows/Solution_${_buildtype}.patch
      patches/windows/python3_build_${_buildtype}.patch)
    message(STATUS "Patches applied!")
    file(COPY "${CURRENT_PORT_DIR}/config.pl"
         DESTINATION "${BUILDPATH_${_buildtype}}/src/tools/msvc")

    set(MSPROJ_PERL
        "${BUILDPATH_${_buildtype}}/src/tools/msvc/MSBuildProject.pm")
    ...
    # ldap      => undef,    # --with-ldap extraver  => undef,    #
    # --with-extra-version=<string> gss       => undef,    #
    # --with-gssapi=<path> icu       => undef,    # --with-icu=<path> ##done nls
    # => undef,    # --enable-nls=<path> ##done tap_tests => undef,    #
    # --enable-tap-tests tcl       => undef, # --with-tcl=<path> #done perl =>
    # undef,    # --with-perl python    => undef,    # --with-python=<path>
    # ##done openssl   => undef,    # --with-openssl=<path> ##done uuid      =>
    # undef,    # --with-ossp-uuid xml       => undef,    # --with-libxml=<path>
    # ##done xslt      => undef,    # --with-libxslt=<path> ##done iconv     =>
    # undef,    # (not in configure, path to iconv) ##done (needed by xml) zlib
    # => undef     # --with-zlib=<path> ##done

    # Setup external dependencies "-DFEATURES=core;openssl;zlib"
    # "-DALL_FEATURES=openssl;zlib;readline;libedit;python;tcl;nls;systemd;llvm;icu;bonjour;uuid;xml;xslt;"
    if("${FEATURES}" MATCHES "icu")
      string(REPLACE "icu       => undef"
                     "icu      => \"${CURRENT_INSTALLED_DIR}\"" _contents
                     "${_contents}")
    endif()
    if("${FEATURES}" MATCHES "nls")
      string(REPLACE "nls       => undef"
                     "nls      => \"${CURRENT_INSTALLED_DIR}\"" _contents
                     "${_contents}")
    ...
    if("${FEATURES}" MATCHES "openssl")
      ...
      # ';${CURRENT_INSTALLED_DIR}/tools/openssl';")
      string(REPLACE "openssl   => undef"
                     "openssl   => \"${CURRENT_INSTALLED_DIR}\"" _contents
                     "${_contents}")
    endif()
    if("${FEATURES}" MATCHES "python")
      ...
      string(REPLACE "python    => undef"
                     "python    => \"${CURRENT_INSTALLED_DIR}\"" _contents
                     "${_contents}")
    endif()
    if("${FEATURES}" MATCHES "tcl")
      string(REPLACE "tcl       => undef"
                     "tcl       => \"${CURRENT_INSTALLED_DIR}\"" _contents
                     "${_contents}")
    endif()
    if("${FEATURES}" MATCHES "xml")
      string(REPLACE "xml       => undef"
                     "xml      => \"${CURRENT_INSTALLED_DIR}\"" _contents
                     "${_contents}")
      string(REPLACE "iconv     => undef"
                     "iconv      => \"${CURRENT_INSTALLED_DIR}\"" _contents
                     "${_contents}")
    endif()
    if("${FEATURES}" MATCHES "xslt")
      string(REPLACE "xslt      => undef"
                     "xslt      => \"${CURRENT_INSTALLED_DIR}\"" _contents
                     "${_contents}")
    endif()
    if("${FEATURES}" MATCHES "zlib")
      string(REPLACE "zlib      => undef"
                     "zlib      => \"${CURRENT_INSTALLED_DIR}\"" _contents
                     "${_contents}")
    endif()
    if("${FEATURES}" MATCHES "lz4")
      string(REPLACE "lz4       => undef"
                     "lz4       => \"${CURRENT_INSTALLED_DIR}\"" _contents
                     "${_contents}")
    endif()

    file(WRITE "${CONFIG_FILE}" "${_contents}")
    file(WRITE "${BUILDPATH_${_buildtype}}/src/tools/msvc/buildenv.pl"
         "${buildenv_contents}")
    configure_file(
      "${CURRENT_PORT_DIR}/libpq.props.in"
      "${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}/libpq.props" @ONLY)
    ...
    if(HAS_TOOLS)
      ...
      vcpkg_execute_required_process(
        ...
        ${_buildtype}
        WORKING_DIRECTORY
        "${BUILDPATH_${_buildtype}}/src/tools/msvc"
        LOGNAME
        "build-${TARGET_TRIPLET}-${_buildtype}")
      ...
      foreach(build_lib ${build_libs})
        message(
          STATUS "Building ${build_lib} ${TARGET_TRIPLET}-${_buildtype}...")
        vcpkg_execute_required_process(
          COMMAND
          ${PERL}
          build.pl
          ...
          "${BUILDPATH_${_buildtype}}/src/tools/msvc"
          LOGNAME
          "build-${build_lib}-${TARGET_TRIPLET}-${_buildtype}")
        message(
          STATUS "Building ${build_lib} ${TARGET_TRIPLET}-${_buildtype}... done"
        )
      endforeach()
    endif()
    ...
    vcpkg_execute_required_process(
      ...
      "${BUILDPATH_${_buildtype}}/src/tools/msvc"
      LOGNAME
      "install-${TARGET_TRIPLET}-${_buildtype}")
  ...
  if("nls" IN_LIST FEATURES)
    ...
    set(ENV{MSGFMT}
        "${CURRENT_HOST_INSTALLED_DIR}/tools/gettext/bin/msgfmt${VCPKG_HOST_EXECUTABLE_SUFFIX}"
    )
  else()
  ...
  if(VCPKG_TARGET_IS_MINGW AND VCPKG_LIBRARY_LINKAGE STREQUAL "dynamic")
    if(NOT VCPKG_BUILD_TYPE OR VCPKG_BUILD_TYPE STREQUAL "release")
      ...
      file(RENAME "${CURRENT_PACKAGES_DIR}/lib/libpq.a"
           "${CURRENT_PACKAGES_DIR}/lib/libpq.dll.a")
      file(RENAME "${CURRENT_PACKAGES_DIR}/lib/libpq.dll"
           "${CURRENT_PACKAGES_DIR}/bin/libpq.dll")
    endif()
    if(NOT VCPKG_BUILD_TYPE OR VCPKG_BUILD_TYPE STREQUAL "debug")
      file(MAKE_DIRECTORY "${CURRENT_PACKAGES_DIR}/debug/bin")
      file(RENAME "${CURRENT_PACKAGES_DIR}/debug/lib/libpq.a"
           "${CURRENT_PACKAGES_DIR}/debug/lib/libpq.dll.a")
      file(RENAME "${CURRENT_PACKAGES_DIR}/debug/lib/libpq.dll"
           "${CURRENT_PACKAGES_DIR}/debug/bin/libpq.dll")
    endif()
  endif()
  if(VCPKG_TARGET_IS_MINGW)
    set(USE_DL OFF)
