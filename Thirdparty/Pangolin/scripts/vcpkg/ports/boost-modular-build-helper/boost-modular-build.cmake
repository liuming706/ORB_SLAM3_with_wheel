...
get_filename_component(BOOST_BUILD_INSTALLED_DIR "${CMAKE_CURRENT_LIST_DIR}"
                       DIRECTORY)
get_filename_component(BOOST_BUILD_INSTALLED_DIR "${BOOST_BUILD_INSTALLED_DIR}"
                       DIRECTORY)

set(BOOST_VERSION 1.81.0)
string(REGEX MATCH "^([0-9]+)\\.([0-9]+)\\.([0-9]+)" BOOST_VERSION_MATCH
             "${BOOST_VERSION}")
if("${CMAKE_MATCH_3}" GREATER 0)
  set(BOOST_VERSION_ABI_TAG
...
function(boost_modular_build)
  ...
  if(NOT DEFINED _bm_SOURCE_PATH)
    message(
      FATAL_ERROR "SOURCE_PATH is a required argument to boost_modular_build.")
  endif()

  # The following variables are used in the Jamroot.jam
  set(B2_REQUIREMENTS)

  # Some CMake variables may be overridden in the file specified in
  # ${_bm_BOOST_CMAKE_FRAGMENT}
  ...
  if(_jamfile)
    ...
    string(REGEX REPLACE "\.\./\.\./([^/ ]+)/build//(boost_[^/ ]+)"
                         "/boost/\\1//\\2" _contents "${_contents}")
    string(REGEX REPLACE "/boost//([^/ ]+)" "/boost/\\1//boost_\\1" _contents
                         "${_contents}")
  ...
  if("python2" IN_LIST FEATURES)
    ...
    string(REGEX REPLACE ".*python([0-9\.]+).*" "\\1" python2_version
                         "${python2_include_dir}")
  ...
  if("python3" IN_LIST FEATURES)
    ...
    string(REGEX REPLACE ".*python([0-9\.]+).*" "\\1" python3_version
                         "${python3_include_dir}")
    string(REPLACE "." "" PYTHON_VERSION_TAG "${python3_version}")
  endif()

  configure_file(${BOOST_BUILD_INSTALLED_DIR}/share/boost-build/Jamroot.jam.in
                 ${_bm_SOURCE_PATH}/Jamroot.jam @ONLY)

  set(configure_options)
  if(_bm_BOOST_CMAKE_FRAGMENT)
    list(APPEND configure_options
         "-DBOOST_CMAKE_FRAGMENT=${_bm_BOOST_CMAKE_FRAGMENT}")
  endif()
  ...
  vcpkg_cmake_configure(
    SOURCE_PATH
    "${BOOST_BUILD_INSTALLED_DIR}/share/boost-build"
    GENERATOR
    Ninja
    OPTIONS
    "-DPORT=${PORT}"
  ...
  file(GLOB INSTALLED_LIBS "${CURRENT_PACKAGES_DIR}/debug/lib/*.lib"
  ...
  foreach(LIB IN LISTS INSTALLED_LIBS)
    ...
    string(REPLACE "-vc141-" "-vc140-" NEW_FILENAME ${NEW_FILENAME}
    )# To merge VS2017 and VS2015 binaries
    string(REPLACE "-vc142-" "-vc140-" NEW_FILENAME ${NEW_FILENAME}
    )# To merge VS2019 and VS2015 binaries
    string(REPLACE "-vc143-" "-vc140-" NEW_FILENAME ${NEW_FILENAME}
    )# To merge VS2022 and VS2015 binaries
    string(REPLACE "-sgd-" "-gd-" NEW_FILENAME ${NEW_FILENAME}) # For Debug libs
    string(REPLACE "-sgyd-" "-gyd-" NEW_FILENAME ${NEW_FILENAME}) # For Debug
                                                                  # libs
    string(REPLACE "-gyd-" "-gd-" NEW_FILENAME ${NEW_FILENAME}) # For Debug libs
                                                                # with python
                                                                # debugging
    string(REPLACE "-x32-" "-" NEW_FILENAME ${NEW_FILENAME}) # To enable CMake
                                                             # 3.10 and earlier
                                                             # to locate the
                                                             # binaries
    string(REPLACE "-x64-" "-" NEW_FILENAME ${NEW_FILENAME}) # To enable CMake
                                                             # 3.10 and earlier
                                                             # to locate the
                                                             # binaries
    string(REPLACE "-a32-" "-" NEW_FILENAME ${NEW_FILENAME}) # To enable CMake
                                                             # 3.10 and earlier
                                                             # to locate the
                                                             # binaries
    string(REPLACE "-a64-" "-" NEW_FILENAME ${NEW_FILENAME}) # To enable CMake
                                                             # 3.10 and earlier
                                                             # to locate the
                                                             # binaries
    string(REPLACE "-${BOOST_VERSION_ABI_TAG}" "" NEW_FILENAME ${NEW_FILENAME}
    )# To enable CMake > 3.10 to locate the binaries
    if("${DIRECTORY_OF_LIB_FILE}/${NEW_FILENAME}" STREQUAL
       "${DIRECTORY_OF_LIB_FILE}/${OLD_FILENAME}")
      # nothing to do
    elseif(EXISTS "${DIRECTORY_OF_LIB_FILE}/${NEW_FILENAME}")
      file(REMOVE "${DIRECTORY_OF_LIB_FILE}/${OLD_FILENAME}")
    else()
      file(RENAME "${DIRECTORY_OF_LIB_FILE}/${OLD_FILENAME}"
           "${DIRECTORY_OF_LIB_FILE}/${NEW_FILENAME}")
    endif()
  endforeach()
  # Similar for mingw
  file(GLOB INSTALLED_LIBS "${CURRENT_PACKAGES_DIR}/debug/lib/*-mgw*-*.a"
       "${CURRENT_PACKAGES_DIR}/lib/*-mgw*-*.a")
  foreach(LIB IN LISTS INSTALLED_LIBS)
    get_filename_component(OLD_FILENAME "${LIB}" NAME)
    get_filename_component(DIRECTORY_OF_LIB_FILE "${LIB}" DIRECTORY)
    string(REGEX REPLACE "-mgw[0-9]+-.*[0-9](\\.dll\\.a|\\.a)$" "\\1"
  ...
  if(NOT EXISTS "${CURRENT_PACKAGES_DIR}/lib")
    message(
      FATAL_ERROR
        "No libraries were produced. This indicates a failure while building the boost library."
    )
  endif()

  configure_file(${BOOST_BUILD_INSTALLED_DIR}/share/boost-build/usage
