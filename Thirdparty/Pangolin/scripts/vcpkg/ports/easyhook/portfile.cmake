message(
  WARNING
    ".Net framework 4.7.2 is required, please install it before installing easyhook."
)
...
vcpkg_from_github(
  ...
  v2.7.7097.0
  SHA512
  D0CA5B64E77F6281B2DD7EE0DC492A9B07DDB60A9F514037938CC3E3FFA5DD57C95CB630E18C02C984A89070839E4188044896D4EE57A21E43E6EA3A4918255A
  HEAD_REF
  master
  PATCHES
  fix-build.patch)
...
# Use /Z7 rather than /Zi to avoid "fatal error C1090: PDB API call failed,
# error code '23': (0x00000006)"
foreach(VCXPROJ IN
        ITEMS "${SOURCE_PATH}/EasyHookDll/EasyHookDll.vcxproj"
  ...
  vcpkg_replace_string(
    ...
    "<DebugInformationFormat>OldStyle</DebugInformationFormat>")
  vcpkg_replace_string("${VCXPROJ}" "<MinimalRebuild>true</MinimalRebuild>" "")
endforeach()
...
foreach(
  ...
  vcpkg_replace_string(
    "${CSPROJ}" "<TargetFrameworkVersion>v4.0</TargetFrameworkVersion>"
    "<TargetFrameworkVersion>4.7.2</TargetFrameworkVersion>")
  vcpkg_replace_string(
    "${CSPROJ}" "<TargetFrameworkProfile>Client</TargetFrameworkProfile>" "")
endforeach()
...
if(NOT VCPKG_BUILD_TYPE OR VCPKG_BUILD_TYPE STREQUAL "debug")
  file(REMOVE "${CURRENT_PACKAGES_DIR}/bin/EasyHook.dll"
       "${CURRENT_PACKAGES_DIR}/bin/EasyHook.pdb")
endif()
if(NOT VCPKG_BUILD_TYPE OR VCPKG_BUILD_TYPE STREQUAL "release")
  file(REMOVE "${CURRENT_PACKAGES_DIR}/debug/bin/EasyHook.dll"
       "${CURRENT_PACKAGES_DIR}/debug/bin/EasyHook.pdb")
endif()

# Install includes
file(INSTALL "${SOURCE_PATH}/Public/easyhook.h"
     DESTINATION "${CURRENT_PACKAGES_DIR}/include/easyhook")
