...
vcpkg_from_github(
  ...
  ${PACKAGE_VERSION}
  SHA512
  8a3ff480e065cf517468fac9d8d4474cfa6ed354fa83ae60de224580f359f8dcfbfed6cf640d33783779174ade0bca0fbe1c529097ee103af2b02546fc2acaec
  HEAD_REF
  master
  PATCHES
...
vcpkg_download_distfile(
  ...
  FILENAME
  "gnulib-${GNULIB_REF}.tar.gz"
  SHA512
  6e534b3a623efa5f473977deeed4d24669ef0e0e3ac5fcadc88c5cf2d6ad0852a07c68cd70ac748d7f9a3793704ce1a54a7d17114458a8c1f2e42d410681c340
)

vcpkg_extract_source_archive_ex(OUT_SOURCE_PATH GNULIB_SOURCE_PATH ARCHIVE
                                "${GNULIB_SNAPSHOT}" REF ${GNULIB_REF})

file(REMOVE_RECURSE "${SOURCE_PATH}/gnulib")
file(RENAME "${GNULIB_SOURCE_PATH}" "${SOURCE_PATH}/gnulib")

include(
  "${CURRENT_HOST_INSTALLED_DIR}/share/yasm-tool-helper/yasm-tool-helper.cmake")
...
string(
  REPLACE
    ...
    "${_contents}")
string(REGEX
       REPLACE "${VCPKG_ROOT_DIR}/installed/[^/]+/share"
               "${CURRENT_HOST_INSTALLED_DIR}/share" _contents "${_contents}"
)# Above already
...
foreach(PROPS IN ITEMS "${SOURCE_PATH}/SMP/smp_deps.props"
                       "${SOURCE_PATH}/SMP/smp_winrt_deps.props")
  vcpkg_replace_string("${PROPS}" [=[_winrt</TargetName>]=] [=[</TargetName>]=])
  vcpkg_replace_string("${PROPS}" [=[<TargetName>lib$(RootNamespace)]=]
                       [=[<TargetName>$(RootNamespace)]=])
endforeach()

# patch hogweed, gpm, nettle, zlib libraries file names; inject RuntimeLibrary
# property to control CRT linkage
foreach(VCXPROJ IN ITEMS "${SOURCE_PATH}/SMP/libgnutls.vcxproj"
                         "${SOURCE_PATH}/SMP/libgnutls_winrt.vcxproj")
  vcpkg_replace_string("${VCXPROJ}" "_winrt.lib" ".lib")
  vcpkg_replace_string("${VCXPROJ}" "libhogweed" "hogweed")
  vcpkg_replace_string("${VCXPROJ}" "hogweedd" "hogweed")
  vcpkg_replace_string("${VCXPROJ}" "libgmp" "gmp")
  vcpkg_replace_string("${VCXPROJ}" "gmpd" "gmp")
  vcpkg_replace_string("${VCXPROJ}" "libnettle" "nettle")
  vcpkg_replace_string("${VCXPROJ}" "nettled" "nettle")
  vcpkg_replace_string("${VCXPROJ}" "libzlib" "zlib")
...
vcpkg_install_msbuild(
  ...
  /p:OutDir=..\\msvc
  OPTIONS_DEBUG
  /p:RuntimeLibrary=MultiThreadedDebug${RuntimeLibraryExt}
  OPTIONS_RELEASE
  /p:RuntimeLibrary=MultiThreaded${RuntimeLibraryExt})
...
file(
  INSTALL
  "${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-rel/${SOURCE_PATH_SUFFIX}/msvc/${WINRT_SUBFOLDER}/include"
...
configure_file("${SOURCE_PATH}/lib/gnutls.pc.in"
               "${CURRENT_PACKAGES_DIR}/lib/pkgconfig/gnutls.pc" @ONLY)
...
configure_file("${SOURCE_PATH}/lib/gnutls.pc.in"
               "${CURRENT_PACKAGES_DIR}/debug/lib/pkgconfig/gnutls.pc" @ONLY)
