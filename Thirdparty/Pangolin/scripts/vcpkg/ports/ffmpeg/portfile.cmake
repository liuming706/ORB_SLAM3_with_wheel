if(VCPKG_TARGET_IS_WINDOWS)
  set(PATCHES 0017-Patch-for-ticket-9019-CUDA-Compile-Broken-Using-MSVC.patch
  )# https://trac.ffmpeg.org/ticket/9019
endif()
vcpkg_from_github(
  OUT_SOURCE_PATH
  SOURCE_PATH
  ...
  n4.4.3
  SHA512
  46bb03d690bdf0d1ce55bfe0582edf1f50b938efcdd5708a6e5ba04154fa50ac8cff4a9e44544cfa50e7f52392c88cde9455aacd6b6e51d4944f123bdcc9297a
  HEAD_REF
  master
  PATCHES
...
set(OPTIONS
    "--enable-pic --disable-doc --enable-debug --enable-runtime-cpudetect")
...
if(VCPKG_DETECTED_MSVC)
  ...
  string(REGEX REPLACE "(^| )-RTC1( |$)" " " VCPKG_COMBINED_C_FLAGS_DEBUG
                       "${VCPKG_COMBINED_C_FLAGS_DEBUG}")
  string(REGEX REPLACE "(^| )-Od( |$)" " " VCPKG_COMBINED_C_FLAGS_DEBUG
                       "${VCPKG_COMBINED_C_FLAGS_DEBUG}")
  string(REGEX REPLACE "(^| )-Ob0( |$)" " " VCPKG_COMBINED_C_FLAGS_DEBUG
                       "${VCPKG_COMBINED_C_FLAGS_DEBUG}")
endif()

string(APPEND VCPKG_COMBINED_C_FLAGS_DEBUG
       " -I \"${CURRENT_INSTALLED_DIR}/include\"")
string(APPEND VCPKG_COMBINED_C_FLAGS_RELEASE
       " -I \"${CURRENT_INSTALLED_DIR}/include\"")
...
if("nvcodec" IN_LIST FEATURES)
  # Note: the --enable-cuda option does not actually require the cuda sdk or
  # toolset port dependency as ffmpeg uses runtime detection and dynamic loading
  set(OPTIONS
...
if("qsv" IN_LIST FEATURES)
  set(OPTIONS
      "${OPTIONS} --enable-libmfx --enable-encoder=h264_qsv --enable-decoder=h264_qsv"
  )
endif()
...
# ffmpeg needs --cross-prefix option to use appropriate tools for
# cross-compiling.
...
if(VCPKG_TARGET_IS_UWP)
  ...
  string(APPEND OPTIONS
         " --extra-ldflags=-APPCONTAINER --extra-ldflags=WindowsApp.lib")
endif()

# Note: --disable-optimizations can't be used due to
# https://ffmpeg.org/pipermail/libav-user/2013-March/003945.html
...
if(NOT VCPKG_BUILD_TYPE OR VCPKG_BUILD_TYPE STREQUAL "release")
  ...
  set(ENV{${LIB_PATH_VAR}}
      "${CURRENT_INSTALLED_DIR}/lib${VCPKG_HOST_PATH_SEPARATOR}${ENV_LIB_PATH}")
  ...
  set(ENV{CFLAGS} "@${crsp}")
  # All tools except the msvc arm{,64} assembler accept @... as response file
  # syntax. For that assembler, there is no known way to pass in flags. We must
  # hope that not passing flags will work acceptably.
  ...
  configure_file("${CMAKE_CURRENT_LIST_DIR}/build.sh.in"
                 "${BUILD_DIR}/build.sh" @ONLY)
...
if(NOT VCPKG_BUILD_TYPE OR VCPKG_BUILD_TYPE STREQUAL "debug")
  ...
  set(ENV{${LIB_PATH_VAR}}
      "${CURRENT_INSTALLED_DIR}/debug/lib${VCPKG_HOST_PATH_SEPARATOR}${ENV_LIB_PATH}"
  )
...
if(VCPKG_TARGET_IS_WINDOWS)
  file(GLOB DEF_FILES "${CURRENT_PACKAGES_DIR}/lib/*.def"
  ...
  if(NOT VCPKG_TARGET_IS_MINGW)
    ...
    foreach(DEF_FILE ${DEF_FILES})
      ...
      string(REGEX
             REPLACE "-[0-9]*\\.def" "${VCPKG_TARGET_STATIC_LIBRARY_SUFFIX}"
                     OUT_FILE_NAME "${DEF_FILE_NAME}")
  ...
  file(GLOB EXP_FILES "${CURRENT_PACKAGES_DIR}/lib/*.exp"
  ...
  if(VCPKG_TARGET_IS_MINGW)
    file(GLOB LIB_FILES_2 "${CURRENT_PACKAGES_DIR}/bin/*.lib"
...
if(VCPKG_TARGET_IS_WINDOWS)
  ...
  foreach(_debug IN LISTS _dirs)
    foreach(PKGCONFIG_MODULE IN LISTS FFMPEG_PKGCONFIG_MODULES)
      set(PKGCONFIG_FILE
          "${CURRENT_PACKAGES_DIR}${_debug}lib/pkgconfig/${PKGCONFIG_MODULE}.pc"
      )
      ...
      vcpkg_replace_string(
        "${PKGCONFIG_FILE}"
        "-libpath:${CYG_INSTALLED_DIR}${_debug}lib/pkgconfig/../../lib " "")
      # transform libdir, includedir, and prefix paths from cygwin style to
      # windows style
      ...
      foreach(PATH_NAME prefix libdir includedir)
        ...
        vcpkg_replace_string("${PKGCONFIG_FILE}" "${PATH_NAME}=${PATH_VALUE}"
                             "${PATH_NAME}=${FIXED_PATH}")
      endforeach()
      # list libraries with -l flag (so pkgconf knows they are libraries and not
      # just linker flags)
      foreach(LIBS_ENTRY Libs Libs.private)
        ...
        if(LIBS_VALUE)
          ...
          string(REGEX REPLACE "([^ ]+)[.]lib" "-l\\1" LIBS_VALUE
                               "${LIBS_VALUE}")
          set(LIBS_VALUE_NEW "${LIBS_VALUE}")
          vcpkg_replace_string(
            "${PKGCONFIG_FILE}" "${LIBS_ENTRY}: ${LIBS_VALUE_OLD}"
            "${LIBS_ENTRY}: ${LIBS_VALUE_NEW}")
        endif()
      endforeach()
    endforeach()
  endforeach()
...
x_vcpkg_pkgconfig_get_modules(PREFIX FFMPEG_PKGCONFIG MODULES
                              ${FFMPEG_PKGCONFIG_MODULES} LIBS)
...
append_dependencies_from_libs(FFMPEG_DEPENDENCIES_RELEASE LIBS
                              "${FFMPEG_PKGCONFIG_LIBS_RELEASE}")
append_dependencies_from_libs(FFMPEG_DEPENDENCIES_DEBUG LIBS
                              "${FFMPEG_PKGCONFIG_LIBS_DEBUG}")
...
function(extract_regex_from_file out)
  ...
  if(contents MATCHES "${arg_REGEX}")
    ...
    message(
      FATAL_ERROR
        "Could not find line matching \"${arg_REGEX}\" in file \"${arg_FILE}\"")
  endif()
  set("${out}"
      "${CMAKE_MATCH_1}"
      PARENT_SCOPE)
...
function(extract_version_from_component out)
  ...
  extract_regex_from_file(
    major_version FILE "${SOURCE_PATH}/${component_lower}/version.h" REGEX
    "#define ${component_upper}_VERSION_MAJOR[ ]+([0-9]+)")
  extract_regex_from_file(
    minor_version FILE "${SOURCE_PATH}/${component_lower}/version.h" REGEX
    "#define ${component_upper}_VERSION_MINOR[ ]+([0-9]+)")
  extract_regex_from_file(
    micro_version FILE "${SOURCE_PATH}/${component_lower}/version.h" REGEX
    "#define ${component_upper}_VERSION_MICRO[ ]+([0-9]+)")
  set("${out}"
...
extract_regex_from_file(
  FFMPEG_VERSION FILE
  "${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-rel/libavutil/ffversion.h" REGEX
...
extract_version_from_component(LIBAVRESAMPLE_VERSION COMPONENT libavresample)
...
# Handle copyright
file(
  STRINGS "${CURRENT_BUILDTREES_DIR}/build-${TARGET_TRIPLET}-rel-out.log"
  LICENSE_STRING
  REGEX "License: .*"
  LIMIT_COUNT 1)
...
configure_file("${CMAKE_CURRENT_LIST_DIR}/FindFFMPEG.cmake.in"
               "${CURRENT_PACKAGES_DIR}/share/${PORT}/FindFFMPEG.cmake" @ONLY)
