...
if(VCPKG_TARGET_IS_WINDOWS)
  list(
    ...
    --with-included-glib
    # This is required. For some reason these do not get correctly identified
    # for release builds.
...
function(build_libintl_only)
  ...
  vcpkg_configure_make(
    SOURCE_PATH
    "${SOURCE_PATH}/gettext-runtime"
    DETERMINE_BUILD_TRIPLET
    USE_WRAPPERS
    ADD_BIN_TO_PATH # So configure can check for working iconv
    OPTIONS
    ${OPTIONS})
  vcpkg_install_make(MAKEFILE "${CMAKE_CURRENT_LIST_DIR}/Makefile" BUILD_TARGET
                     build-intl INSTALL_TARGET install-intl)
endfunction()

if("tools" IN_LIST FEATURES)
  # Minimization of gettext tools build time by: - building tools only for
  # release configuration - custom top-level Makefile - configuration cache
  list(APPEND OPTIONS "--cache-file=../config.cache-${TARGET_TRIPLET}")
  file(REMOVE_RECURSE
       "${CURRENT_BUILDTREES_DIR}/config.cache-${TARGET_TRIPLET}")
  ...
  elseif(NOT DEFINED VCPKG_BUILD_TYPE OR VCPKG_BUILD_TYPE STREQUAL "debug")
    ...
    file(READ "${CURRENT_BUILDTREES_DIR}/config.cache-${TARGET_TRIPLET}"
         config_cache)
    string(REGEX REPLACE "\nac_cv_env[^\n]*" "" config_cache "${config_cache}"
    )# Eliminate build type flags
    file(WRITE "${CURRENT_BUILDTREES_DIR}/config.cache-${TARGET_TRIPLET}"
         "${config_cache}")
    build_libintl_only(BUILD_TYPE "debug")
    file(RENAME "${CURRENT_PACKAGES_DIR}/debug"
         "${CURRENT_PACKAGES_DIR}.release/debug")
...
configure_file("${SOURCE_PATH}/COPYING"
               "${CURRENT_PACKAGES_DIR}/share/gettext/copyright" COPYONLY)
...
if(NOT VCPKG_TARGET_IS_LINUX)
  file(COPY "${CMAKE_CURRENT_LIST_DIR}/vcpkg-cmake-wrapper.cmake"
       DESTINATION "${CURRENT_PACKAGES_DIR}/share/intl")
endif()
if("tools" IN_LIST FEATURES AND NOT VCPKG_CROSSCOMPILING)
  file(COPY "${CMAKE_CURRENT_LIST_DIR}/vcpkg-port-config.cmake"
       DESTINATION "${CURRENT_PACKAGES_DIR}/share/gettext")
endif()
