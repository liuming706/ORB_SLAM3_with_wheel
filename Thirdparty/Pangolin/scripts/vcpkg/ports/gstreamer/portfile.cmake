...
vcpkg_from_gitlab(
  ...
  1.20.5
  SHA512
  2a996d8ac0f70c34dbbc02c875026df6e89346f0844fbaa25475075bcb6e57c81ceb7d71e729c3259eace851e3d7222cb3fe395e375d93eb45b1262a6ede1fdb
  HEAD_REF
  master
  PATCHES
  fix-clang-cl.patch
...
if(VCPKG_TARGET_IS_OSX)
  # In Darwin platform, there can be an old version of `bison`, Which can't be
  # used for `gst-build`. It requires 2.4+
...
#
# Rationale for added options Common options are added below systematically
# Feature options are added below only if the feature needs an external
# dependency Feature options that are dependent on the operating system type
# (like wasapi or osxaudio) are set to auto Every other feature options are made
# available if the dependency is available on vcpkg and if the plugin has
# managed to build during tests
#
...
vcpkg_configure_meson(
  ...
  -Dnls=${NLS}
  -Dorc=disabled # gstreamer requires a specific version of orc which is not
                 # available in vcpkg
  -Ddoc=disabled
  -Dgtk_doc=disabled
  # gstreamer
  ...
  -Dgst-plugins-bad:gs=disabled # Error during plugin configuration (abseil
  ...
  -Dgst-plugins-bad:lv2=disabled # Error during plugin configuration (lilv
                                 # pkg-config file missing)
...
if(NOT VCPKG_TARGET_IS_LINUX)
  file(RENAME
       "${CURRENT_PACKAGES_DIR}/lib/gstreamer-1.0/include/gst/gl/gstglconfig.h"
       "${CURRENT_PACKAGES_DIR}/include/gstreamer-1.0/gst/gl/gstglconfig.h")
endif()

list(APPEND GST_BIN_TOOLS gst-inspect-1.0 gst-launch-1.0 gst-stats-1.0
     gst-typefind-1.0)
list(APPEND GST_LIBEXEC_TOOLS gst-plugin-scanner)

if("plugins-base" IN_LIST FEATURES)
  list(APPEND GST_BIN_TOOLS gst-device-monitor-1.0 gst-discoverer-1.0
       gst-play-1.0)
endif()
...
vcpkg_copy_tools(TOOL_NAMES ${GST_LIBEXEC_TOOLS} SEARCH_DIR
                 "${CURRENT_PACKAGES_DIR}/libexec/gstreamer-1.0" AUTO_CLEAN)
