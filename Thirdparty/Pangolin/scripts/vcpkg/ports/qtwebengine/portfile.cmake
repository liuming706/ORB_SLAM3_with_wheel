...
set(${PORT}_PATCHES add-missing-string.patch)
...
vcpkg_check_features(
  ...
  "geolocation"
  CMAKE_REQUIRE_FIND_PACKAGE_Qt6Positioning
  "webchannel"
  CMAKE_REQUIRE_FIND_PACKAGE_Qt6WebChannel
  INVERTED_FEATURES
  "geolocation"
  CMAKE_DISABLE_FIND_PACKAGE_Qt6Positioning
  "webchannel"
  CMAKE_DISABLE_FIND_PACKAGE_Qt6WebChannel)
...
# webengine-extensions webengine-printing-and-pdf webengine-pepper-plugins
...
if(VCPKG_TARGET_IS_LINUX)
  ...
  # qt_configure_add_summary_entry(ARGS "webengine-system-libpci") + ALSA and
  # PULSEAUDIO
  set(system_libs
      re2
      icu
...
find_program(
  NODEJS
  NAMES node
  PATHS "${CURRENT_HOST_INSTALLED_DIR}/tools/node" "bin" NO_DEFAULT_PATHS)
...
set(GPERF
    "${CURRENT_HOST_INSTALLED_DIR}/tools/gperf/gperf${VCPKG_HOST_EXECUTABLE_SUFFIX}"
)
...
# We know that C:/buildrees/${PORT} is to long to build Release. Debug works
# however. Means 24 length is too much but 23 might work.
if(buildtree_length GREATER 22
   AND VCPKG_TARGET_IS_WINDOWS
   AND NOT VCPKG_TARGET_ARCHITECTURE MATCHES "arm64")
  message(
    WARNING
      "Buildtree path '${CURRENT_BUILDTREES_DIR}' is too long.\nConsider passing --x-buildtrees-root=<shortpath> to vcpkg!\nTrying to use '${CURRENT_BUILDTREES_DIR}/../tmp'"
  )
  set(CURRENT_BUILDTREES_DIR "${CURRENT_BUILDTREES_DIR}/../tmp") # activly avoid
                                                                 # long path
                                                                 # issues in CI.
                                                                 # -> Means CI
                                                                 # will not
  ...
  if(buildtree_length_new GREATER 22)
    message(
      FATAL_ERROR
        "Buildtree path is too long. Build will fail! Pass --x-buildtrees-root=<shortpath> to vcpkg!"
    )
  endif()
