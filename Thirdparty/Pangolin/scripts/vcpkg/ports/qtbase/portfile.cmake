# Reminder for myself and everybody else: Qt cross module dependency information
# within the Qt respository is wrong and/or incomplete. Always check the
# toplevel CMakeLists.txt for the find_package call and search for linkage
# against the Qt:: targets Often enough certain (bigger) dependencies are only
# used to build examples and/or tests. As such getting the correct dependency
# information relevant for vcpkg requires a manual search/check
set(QT_IS_LATEST ON)
...
set(${PORT}_PATCHES
    ...
    fix_egl.patch
    fix_egl_2.patch
    clang-cl_QGADGET_fix.diff # Upstream is still figuring out if this is a
                              # compiler bug or not.
    installed_dir.patch
...
# Features can be found via searching for qt_feature in all configure.cmake
# files in the source: The files also contain information about the Platform for
# which it is searched Always use FEATURE_<feature> in vcpkg_cmake_configure
# (using QT_FEATURE_X overrides Qts condition check for the feature.)
# Theoretically there is a feature for every widget to enable/disable it but
# that is way to much for vcpkg
...
foreach(_input IN LISTS input_vars)
  ...
  elseif(_input STREQUAL "libmd4c")
    string(APPEND INPUT_OPTIONS qt) # libmd4c is not yet in VCPKG (but required
                                    # by qtdeclarative)
  else()
...
vcpkg_check_features(
  ...
  # Required "pcre2"                 CMAKE_DISABLE_FIND_PACKAGE_WrapSystemPCRE2
  # # Bug in qt cannot be deactivated
  "icu"
...
list(APPEND FEATURE_CORE_OPTIONS
...
vcpkg_check_features(
  ...
  # CMAKE_REQUIRE_FIND_PACKAGE_WrapSystemHarfbuzz "jpeg"
  # CMAKE_REQUIRE_FIND_PACKAGE_JPEG "png" CMAKE_REQUIRE_FIND_PACKAGE_PNG "xlib"
  ...
  # "freetype"            CMAKE_DISABLE_FIND_PACKAGE_WrapSystemFreetype # Bug in
  # qt cannot be deactivated
  "harfbuzz"
  ...
  # There are more X features but I am unsure how to safely disable them! Most
  # of them seem to be found automaticall with find_package(X11)
)
...
foreach(_db IN LISTS DB_LIST)
  list(APPEND FEATURE_SQLDRIVERS_OPTIONS
       -DCMAKE_DISABLE_FIND_PACKAGE_${_db}:BOOL=ON)
endforeach()

# printsupport features: vcpkg_check_features(OUT_FEATURE_OPTIONS
# FEATURE_PRINTSUPPORT_OPTIONS
# )
list(APPEND FEATURE_PRINTSUPPORT_OPTIONS
     -DCMAKE_DISABLE_FIND_PACKAGE_CUPS:BOOL=ON)

# widgets features: vcpkg_check_features(OUT_FEATURE_OPTIONS
# FEATURE_WIDGETS_OPTIONS
# "gtk3"             FEATURE_gtk3 There are a lot of additional features here to
# deactivate parts of widgets. )
...
set(other_files qt-cmake qt-cmake-private qt-cmake-standalone-test
                qt-configure-module qt-internal-configure-tests)
if(CMAKE_HOST_WIN32)
...
foreach(_config debug release)
  ...
  foreach(other_file IN LISTS other_files)
    if(EXISTS "${CURRENT_PACKAGES_DIR}/${path_suffix}bin/${other_file}")
      set(target_file
          "${CURRENT_PACKAGES_DIR}/tools/Qt6/bin/${path_suffix}${other_file}")
      file(RENAME "${CURRENT_PACKAGES_DIR}/${path_suffix}bin/${other_file}"
           "${target_file}")
      file(READ "${target_file}" _contents)
      if(_config MATCHES "debug")
        string(REPLACE "..\\share\\" "..\\..\\..\\..\\share\\" _contents
                       "${_contents}")
        string(REPLACE "../share/" "../../../../share/" _contents
                       "${_contents}")
      else()
...
string(REGEX REPLACE "set\\\(__qt_initial_cxx_compiler [^\\\n]+\\\n" ""
                     toolchain_contents "${toolchain_contents}")
string(REPLACE "${CURRENT_HOST_INSTALLED_DIR}"
               "\${vcpkg_installed_dir}/${HOST_TRIPLET}" toolchain_contents
               "${toolchain_contents}")
...
if(VCPKG_LIBRARY_LINKAGE STREQUAL "static" OR NOT VCPKG_TARGET_IS_WINDOWS)
  if(VCPKG_CROSSCOMPILING)
    file(REMOVE_RECURSE "${CURRENT_PACKAGES_DIR}/bin/qmake"
         "${CURRENT_PACKAGES_DIR}/debug/bin/qmake") # qmake has been moved so
                                                    # this is the qmake helper
                                                    # script
  endif()
...
file(RELATIVE_PATH installed_to_host "${CURRENT_INSTALLED_DIR}"
     "${CURRENT_HOST_INSTALLED_DIR}")
file(RELATIVE_PATH host_to_installed "${CURRENT_HOST_INSTALLED_DIR}"
...
configure_file("${_file}" "${CURRENT_PACKAGES_DIR}/tools/Qt6/qt_release.conf"
               @ONLY) # For vcpkg-qmake
...
set(REL_HOST_TO_DATA "\${CURRENT_INSTALLED_DIR}/")
configure_file("${_file}" "${CURRENT_PACKAGES_DIR}/tools/Qt6/qt_debug.conf"
               @ONLY) # For vcpkg-qmake
...
set(hostinfofile
    "${CURRENT_PACKAGES_DIR}/share/Qt6HostInfo/Qt6HostInfoConfig.cmake")
file(READ "${hostinfofile}" _contents)
string(REPLACE [[set(QT6_HOST_INFO_LIBEXECDIR "bin")]]
               [[set(QT6_HOST_INFO_LIBEXECDIR "tools/Qt6/bin")]] _contents
               "${_contents}")
string(REPLACE [[set(QT6_HOST_INFO_BINDIR "bin")]]
               [[set(QT6_HOST_INFO_BINDIR "tools/Qt6/bin")]] _contents
               "${_contents}")
...
if(EXISTS "${configfile}")
  ...
  if(EXISTS "${CURRENT_PACKAGES_DIR}/tools/Qt6/bin/qmake.exe")
    ...
    string(REPLACE [[ "${_IMPORT_PREFIX}/tools/Qt6/bin/qmake.exe"]]
  ...
  if(EXISTS "${CURRENT_PACKAGES_DIR}/tools/Qt6/bin/qtpaths.exe")
    file(INSTALL "${CMAKE_CURRENT_LIST_DIR}/qtpaths.debug.bat"
         DESTINATION "${CURRENT_PACKAGES_DIR}/tools/Qt6/bin")
    string(REPLACE [[ "${_IMPORT_PREFIX}/tools/Qt6/bin/qtpaths.exe"]]
  ...
  if(EXISTS "${CURRENT_PACKAGES_DIR}/tools/Qt6/bin/windeployqt.exe")
    file(INSTALL "${CMAKE_CURRENT_LIST_DIR}/windeployqt.debug.bat"
         DESTINATION "${CURRENT_PACKAGES_DIR}/tools/Qt6/bin")
    string(REPLACE [[ "${_IMPORT_PREFIX}/tools/Qt6/bin/windeployqt.exe"]]
...
if(VCPKG_CROSSCOMPILING)
  vcpkg_replace_string(
    ...
    "${CURRENT_HOST_INSTALLED_DIR}"
    "\${CMAKE_CURRENT_LIST_DIR}/../../../${HOST_TRIPLET}")
endif()
