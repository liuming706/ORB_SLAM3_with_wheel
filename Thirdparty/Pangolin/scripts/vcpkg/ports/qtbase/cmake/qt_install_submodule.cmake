...
include(
  "${CURRENT_HOST_INSTALLED_DIR}/share/vcpkg-cmake/vcpkg-port-config.cmake")
include(
  "${CURRENT_HOST_INSTALLED_DIR}/share/vcpkg-cmake-config/vcpkg-port-config.cmake"
)
...
function(qt_download_submodule_impl)
  ...
  if("${_qarg_SUBMODULE}" IN_LIST QT_FROM_QT_GIT)
    # qtinterfaceframework is not available in the release, so we fall back to a
    # `git clone`.
    vcpkg_from_git(
      OUT_SOURCE_PATH
      SOURCE_PATH
    ...
    if(PORT STREQUAL "qttools") # Keep this for beta & rc's
      vcpkg_from_git(
        ...
        URL
        git://code.qt.io/playground/qlitehtml.git # git://code.qt.io/playground/qlitehtml.git
        REF
        "${${PORT}_qlitehtml_REF}"
        FETCH_REF
        master
      ...
      file(
        COPY "${SOURCE_PATH_LITEHTML}/"
        DESTINATION
          "${SOURCE_PATH}/src/assistant/qlitehtml/src/3rdparty/litehtml")
    elseif(PORT STREQUAL "qtwebengine")
      vcpkg_from_git(
        OUT_SOURCE_PATH SOURCE_PATH_WEBENGINE URL
        git://code.qt.io/qt/qtwebengine-chromium.git REF
        "${${PORT}_chromium_REF}")
      ...
      if(NOT EXISTS "${SOURCE_PATH}/src/3rdparty/gn")
        file(RENAME "${SOURCE_PATH_WEBENGINE}/gn"
             "${SOURCE_PATH}/src/3rdparty/gn")
      endif()
    endif()
  else()
...
function(qt_cmake_configure)
  cmake_parse_arguments(
    PARSE_ARGV 0 "_qarg" "DISABLE_NINJA;DISABLE_PARALLEL_CONFIGURE" ""
    "TOOL_NAMES;OPTIONS;OPTIONS_DEBUG;OPTIONS_RELEASE;OPTIONS_MAYBE_UNUSED")

  vcpkg_find_acquire_program(PERL) # Perl is probably required by all qt ports
                                   # for syncqt
  get_filename_component(PERL_PATH ${PERL} DIRECTORY)
  vcpkg_add_to_path(${PERL_PATH})
  if(NOT PORT STREQUAL "qtwebengine" OR QT_IS_LATEST) # qtwebengine requires
                                                      # python2; since 6.3
                                                      # python3
  ...
  string(REGEX MATCHALL "CMAKE_DISABLE_FIND_PACKAGE_[^:=]+"
               disabled_find_package "${_qarg_OPTIONS}")
  list(APPEND _qarg_OPTIONS_MAYBE_UNUSED ${disabled_find_package})

  string(REGEX MATCHALL "CMAKE_REQUIRE_FIND_PACKAGE_[^:=]+(:BOOL)?=OFF"
               require_find_package "${_qarg_OPTIONS}")
  list(TRANSFORM require_find_package REPLACE "(:BOOL)?=OFF" "")
  list(APPEND _qarg_OPTIONS_MAYBE_UNUSED ${require_find_package})

  # Disable unused warnings for disabled features. Qt might decide to not emit
  # the feature variables if other features are deactivated.
  string(REGEX MATCHALL "(QT_)?FEATURE_[^:=]+(:BOOL)?=OFF" disabled_features
               "${_qarg_OPTIONS}")
  ...
  vcpkg_cmake_configure(
    ...
    ${disable_parallel}
    OPTIONS
    -DQT_USE_DEFAULT_CMAKE_OPTIMIZATION_FLAGS:BOOL=ON # We don't want Qt to
                                                      # screw with users
                                                      # toolchain settings.
    # -DQT_HOST_PATH=<somepath> # For crosscompiling
    # -DQT_PLATFORM_DEFINITION_DIR=mkspecs/win32-msvc
    # -DQT_QMAKE_TARGET_MKSPEC=win32-msvc -DQT_USE_CCACHE
...
function(qt_fix_prl_files)
  ...
  foreach(prl_file IN LISTS prl_files)
    ...
    string(REPLACE "${lib_path}" "\$\$[QT_INSTALL_LIBS]" _contents
                   "${_contents}")
    string(REPLACE "${include_path}" "\$\$[QT_INSTALL_HEADERS]" _contents
                   "${_contents}")
    string(REPLACE "${install_prefix}" "\$\$[QT_INSTALL_PREFIX]" _contents
                   "${_contents}")
    string(REPLACE "[QT_INSTALL_PREFIX]/lib/objects-Debug"
                   "[QT_INSTALL_LIBS]/objects-Debug" _contents "${_contents}")
    string(REPLACE "[QT_INSTALL_PREFIX]/Qt6/qml" "[QT_INSTALL_QML]" _contents
                   "${_contents}")
...
function(qt_fixup_and_cleanup)
  cmake_parse_arguments(PARSE_ARGV 0 "_qarg" "" "" "TOOL_NAMES")
  vcpkg_copy_pdbs()
  ...
  foreach(_glob IN LISTS COMPONENTS_OR_FILES)
    if(IS_DIRECTORY "${_glob}")
      string(REPLACE "${CURRENT_PACKAGES_DIR}/share/Qt6" "" _component
                     "${_glob}")
  ...
  foreach(_comp IN LISTS COMPONENTS)
    if(EXISTS "${CURRENT_PACKAGES_DIR}/share/Qt6${_comp}")
      vcpkg_cmake_config_fixup(PACKAGE_NAME "Qt6${_comp}" CONFIG_PATH
                               "share/Qt6${_comp}" TOOLS_PATH "tools/Qt6/bin")
      # Would rather put it into share/cmake as before but the import_prefix
      # correction in vcpkg_cmake_config_fixup is working against that.
    else()
      message(STATUS "WARNING: Qt component ${_comp} not found/built!")
    endif()
  endforeach()
  # fix debug plugin paths (should probably be fixed in
  ...
  foreach(_debug_target IN LISTS DEBUG_CMAKE_TARGETS)
    vcpkg_replace_string("${_debug_target}" "{_IMPORT_PREFIX}/${qt_plugindir}"
                         "{_IMPORT_PREFIX}/debug/${qt_plugindir}")
    vcpkg_replace_string("${_debug_target}" "{_IMPORT_PREFIX}/${qt_qmldir}"
  ...
  foreach(_plugin_target IN LISTS STATIC_CMAKE_TARGETS)
    # restore a single get_filename_component which was remove by
    # vcpkg_cmake_config_fixup
  ...
  foreach(_tool IN LISTS _qarg_TOOL_NAMES)
    if(NOT EXISTS
      ...
      debug_message(
        "Removed '${_tool}' from copy tools list since it was not found!")
  ...
  if(_qarg_TOOL_NAMES)
    ...
    vcpkg_copy_tools(
      ...
      "${qt_tooldest}"
      AUTO_CLEAN)
    if(EXISTS "${CURRENT_PACKAGES_DIR}/${qt_plugindir}" AND NOT PORT STREQUAL
                                                            "qtdeclarative"
    )# qmllint conflict
      file(COPY "${CURRENT_PACKAGES_DIR}/${qt_plugindir}/"
           DESTINATION "${qt_tooldest}")
    endif()
  endif()
  ...
  file(
    REMOVE_RECURSE "${CURRENT_PACKAGES_DIR}/debug/lib/cmake/"
    "${CURRENT_PACKAGES_DIR}/debug/share" "${CURRENT_PACKAGES_DIR}/lib/cmake/"
