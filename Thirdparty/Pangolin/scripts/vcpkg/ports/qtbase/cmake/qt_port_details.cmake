# Steps to update the qt6 ports 1. Change QT_VERSION below to the new version 2.
# Set QT_UPDATE_VERSION to 1 3. Add any new Qt modules to QT_PORTS 4. Run a
# build of `qtbase` 5. Fix any intermediate failures by adding the module into
# QT_FROM_GITHUB, QT_FROM_GITHUB_BRANCH, or QT_FROM_QT_GIT as appropriate 6. The
# build should fail with "Done downloading version and emitting hashes." This
# will have changed out the vcpkg.json versions of the qt ports and rewritten
# qt_port_data.cmake 7. Set QT_UPDATE_VERSION back to 0
...
if(QT_VERSION VERSION_GREATER_EQUAL 6.2.2)
  list(APPEND QT_PORTS
       # New in 6.2.2
       qtinterfaceframework qtapplicationmanager)
endif()
if(QT_VERSION VERSION_GREATER_EQUAL 6.4.0)
  list(APPEND QT_PORTS
       # New in 6.4.0
       qthttpserver qtquick3dphysics)
endif()
...
# QT_FROM_GITHUB, QT_FROM_GITHUB_BRANCH, QT_FROM_QT_GIT set(QT_FROM_QT_GIT
# ${QT_PORTS}) list(POP_FRONT QT_FROM_QT_GIT)
...
function(qt_get_url_filename qt_port out_url out_filename)
  ...
  elseif("${qt_port}" IN_LIST QT_FROM_GITHUB_BRANCH)
    ...
    set(url
        "https://download.qt.io/archive/qt/${qt_major_minor}/${QT_VERSION}/submodules/${qt_port}-everywhere-src-${QT_VERSION}.tar.xz"
    )
...
if(QT_UPDATE_VERSION)
  if(NOT PORT STREQUAL "qtbase")
    message(
      FATAL_ERROR
        "QT_UPDATE_VERSION must be used from the root 'qtbase' package")
  endif()
  ...
  foreach(qt_port IN LISTS QT_PORTS)
    ...
    if("${qt_port}" IN_LIST QT_FROM_QT_GIT)
      ...
      execute_process(
        COMMAND "${GIT}" ls-remote -t
                "https://code.qt.io/cgit/qt/${qt_port}.git" "v${QT_VERSION}"
        OUTPUT_VARIABLE out)
