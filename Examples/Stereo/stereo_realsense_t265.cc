...
int main(int argc, char **argv)
{
    ...
    while (b_continue_session) {
        ...
        if (rs2::video_frame image_frame = frame_set.first_or_default(RS2_STREAM_FISHEYE)) {
            ...
            imLeft =
                cv::Mat(cv::Size(width_left, height_left), CV_8UC1, (void *)(frame_left.get_data()), cv::Mat::AUTO_STEP);
            imRight = cv::Mat(cv::Size(width_right, height_right), CV_8UC1, (void *)(frame_right.get_data()),
                              cv::Mat::AUTO_STEP);
...
#endif
                t_resize =
                    std::chrono::duration_cast<std::chrono::duration<double, std::milli> >(t_End_Resize - t_Start_Resize)
                        .count();
