...
int main(int argc, char **argv)
{
    ...
    auto imu_callback = [&](const rs2::frame &frame) {
        ...
        if (rs2::frameset fs = frame.as<rs2::frameset>()) {
            ...
            imCV_left = cv::Mat(cv::Size(width_img, height_img), CV_8U, (void *)(color_frame_left.get_data()),
                                cv::Mat::AUTO_STEP);
            imCV_right = cv::Mat(cv::Size(width_img, height_img), CV_8U, (void *)(color_frame_right.get_data()),
                                 cv::Mat::AUTO_STEP);
    ...
    while (b_continue_session) {
        ...
        for (int i = 0; i < vAccel.size(); ++i) {
            accFile << std::setprecision(15) << vAccel_times[i] << "," << vAccel[i].x << "," << vAccel[i].y << ","
                    << vAccel[i].z << endl;
        }
