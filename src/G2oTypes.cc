...
void EdgeInertial::linearizeOplus()
{
    ...
    _jacobianOplus[0].block<3, 3>(6, 0) =
        Sophus::SO3d::hat(Rbw1 * (VP2->estimate().twb - VP1->estimate().twb - VV1->estimate() * dt - 0.5 * g * dt * dt));  // OK
    // translation
...
// localmapping中imu初始化所用的边，除了正常的几个优化变量外还优化了重力方向与尺度
EdgeInertialGSE::EdgeInertialGSE(IMU::Preintegrated *pInt) :
    JRg(pInt->JRg.cast<double>()),
    JVg(pInt->JVg.cast<double>()),
    JPg(pInt->JPg.cast<double>()),
    JVa(pInt->JVa.cast<double>()),
...
void EdgeInertialGSE::computeError()
{
    ...
    const Eigen::Vector3d ee =
        VP1->estimate().Rwb.transpose() *
        (s * (VP2->estimate().twb - VP1->estimate().twb) - mpInt->Tbo +
...
void EdgeInertialGSE::linearizeOplus()
{
    ...
    //        - dV; const Eigen::Vector3d ep = VP1->estimate().Rwb.transpose()*(s*(VP2->estimate().twb -
    //        VP1->estimate().twb - VV1->estimate()*dt) - g*dt*dt/2) - dP; const Eigen::Vector3d ee =
    //        VP1->estimate().Rwb.transpose()*(s*(VP2->estimate().twb - VP1->estimate().twb)-mpInt->Tbo
    //        + VP1->estimate().Rwb.transpose()*VP2->estimate().Rwb*mpInt->Tbo-mpInt->encoder_velocity.cast<double>());
    ...
    _jacobianOplus[0].block<3, 3>(9, 0) =
        Sophus::SO3d::hat(Rbw1 * ((s * (VP2->estimate().twb - VP1->estimate().twb)) + VP2->estimate().Rwb * mpInt->Tbo));
    ...
    _jacobianOplus[4].block<3, 3>(9, 0) =
        -VP1->estimate().Rwb.transpose() * VP2->estimate().Rwb * Sophus::SO3d::hat(mpInt->Tbo);
    // translation
    _jacobianOplus[4].block<3, 3>(6, 3) = s * Rbw1 * Rwb2;
