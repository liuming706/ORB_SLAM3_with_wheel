    ...
    for (int i = 0; i < n; i++) {
        ...
        if ((i == 0) && (i < (n - 1))) {
            ...
            encoder_v = (mvImuFromLastFrame[i].encoder_v + mvImuFromLastFrame[i + 1].encoder_v -
                         (mvImuFromLastFrame[i + 1].encoder_v - mvImuFromLastFrame[i].encoder_v) * (tini / tab)) *
                        0.5f;
        } else if (i < (n - 1)) {
            // 中间的数据不存在帧的干扰，正常计算
        ...
        // 直到倒数第二个imu时刻时，计算过程跟第一时刻类似，都需要考虑帧与imu时刻的关系
            ...
            angVel = (mvImuFromLastFrame[i].w + mvImuFromLastFrame[i + 1].w -
                     ...
                     0.5f;
            encoder_v = (mvImuFromLastFrame[i].encoder_v + mvImuFromLastFrame[i + 1].encoder_v -
                         (mvImuFromLastFrame[i + 1].encoder_v - mvImuFromLastFrame[i].encoder_v) * (tend / tab)) *
                        0.5f;
...
void Tracking::Track()
{
    ...
    if ((mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD) && !mbCreatedMap) {
        ...
        double timePreImu =
            std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(time_EndPreIMU - time_StartPreIMU).count();
    ...
    if (mState == NOT_INITIALIZED) {
        ...
        if (!mbOnlyTracking) {
            ...
            if (mState == OK) {
                ...
                if ((!mbVelocity && !pCurrentMap->isImuInitialized()) || mCurrentFrame.mnId < mnLastRelocFrameId + 2) {
                    ...
                    if (mCurrentFrame.mTimeStamp > mLastFrame.mTimeStamp + 1.0) return;
                    if (!bOK) {
                ...
                if (mState == RECENTLY_LOST) {
                    ...
                    if ((mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD)) {
                        ...
                        if (mCurrentFrame.mTimeStamp - mTimeStampLost > time_recently_lost) {
                            cerr << "如果IMU模式下当前帧距离跟丢帧超过5s还没有找回（time_recently_lost默认为5s）,"
                                    "RECENTLY_LOST状态改为LOST"
                                 << endl;
                            mState = LOST;
        ...
        double timePosePred =
            std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(time_EndPosePred - time_StartPosePred).count();
        ...
        if (!mbOnlyTracking) {
            ...
            if (!bOK) cerr << "Fail to track local map!" << endl;
        } else {
        ...
        // once mCurrFrame is completely modified) 这段貌似没啥作用
        ...
        double timeLMTrack =
            std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(time_EndLMTrack - time_StartLMTrack).count();
        ...
        if (bOK || mState == RECENTLY_LOST) {
            ...
            double timeNewKF =
                std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(time_EndNewKF - time_StartNewKF).count();
