    ...
    mbShutDown(false)
{
    ...
    //  创建并开启闭环线程
    mpLoopCloser =
        new LoopClosing(mpAtlas, mpKeyFrameDatabase, mpVocabulary, mSensor != MONOCULAR, activeLC);  // mSensor!=MONOCULAR);
                                    ...
                                    string filename)
{
    ...
    // Check reset
    {
        ...
        // 如果检测到重置活动地图的标志为true,将重置地图
            ...
            if (!mpAtlas->GetCurrentMap()->isImuInitialized()) mpTracker->ResetActiveMap();
    ...
    if (mSensor == System::IMU_MONOCULAR)
        for (size_t i_imu = 0; i_imu < vImuMeas.size(); i_imu++) mpTracker->GrabImuData(vImuMeas[i_imu]);  //存储IMU数据
