...
void Preintegrated::Reintegrate()
{
    ...
    buseencoder = true;
    if (buseencoder) {
        for (size_t i = 0; i < aux.size(); i++) IntegrateNewMeasurement(aux[i].a, aux[i].w, aux[i].t, aux[i].enc);
    } else {
...
void Preintegrated::IntegrateNewMeasurement(const Eigen::Vector3f &acceleration, const Eigen::Vector3f &angVel,
                                            const float &dt, const double &encoder_v)
{
    // 保存imu数据，利用中值积分的结果构造一个预积分类保存在mvMeasurements中
    ...
    if (buseencoder) {
        ...
        covariance_enc.block<12, 12>(0, 0) =
            F * covariance_enc.block<12, 12>(0, 0) * F.transpose() + V * Nga_en * V.transpose();
...
void Preintegrated::MergePrevious(Preintegrated *pPrev)
{
    ...
    // TODO ::这里需要修改
    Initialize(bav);
    for (size_t i = 0; i < aux1.size(); i++) IntegrateNewMeasurement(aux1[i].a, aux1[i].w, aux1[i].t, aux1[i].enc);
    for (size_t i = 0; i < aux2.size(); i++) IntegrateNewMeasurement(aux2[i].a, aux2[i].w, aux2[i].t, aux2[i].enc);
}
